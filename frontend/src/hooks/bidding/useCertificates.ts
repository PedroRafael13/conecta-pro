...
/**
 * Hook para validar certidão no órgão emissor (Bidding)
 */
export function useValidarCertidaoBidding() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (certificateId: string) =>
      certificatesService.validarCertidao(certificateId),
    onSuccess: (data, certificateId) => {
      ...
      if (data.valida) {
        ...
        toast.warning('Certidão inválida ou vencida');
      }
    },
    onError: (error: any) => {
...
/**
 * Hook para download de certidão (PDF)
 */
export function useDownloadCertidao() {
  return useMutation({
    mutationFn: (certificateId: string) =>
      certificatesService.downloadCertidao(certificateId),
    onSuccess: (blob, certificateId) => {
      // Criar URL temporário para o blob
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `certidao-${certificateId}.pdf`;
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.detail || 'Erro ao fazer download da certidão'
      );
    },
  });
}
