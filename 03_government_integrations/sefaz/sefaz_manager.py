...
class SEFAZManager:
    ...
    async def sign(self, document_id: UUID) -> str:
        """
        Assina documento com certificado digital.
        ...
        # Simulacao de assinatura
        signature = base64.b64encode(
            hashlib.sha256(doc.xml_content.encode()).digest()
        ).decode()

        signed_xml = doc.xml_content.replace(
            "</NFe>",
            f"""<Signature xmlns="http://www.w3.org/2000/09/xmldsig#">
    <SignedInfo><CanonicalizationMethod Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315"/>
    <SignatureMethod Algorithm="http://www.w3.org/2000/09/xmldsig#rsa-sha1"/>
    <Reference URI="#NFe{doc.chave_acesso}">
    <Transforms><Transform Algorithm="http://www.w3.org/2000/09/xmldsig#enveloped-signature"/>
    <Transform Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315"/></Transforms>
    <DigestMethod Algorithm="http://www.w3.org/2000/09/xmldsig#sha1"/>
    <DigestValue>{signature[:28]}</DigestValue></Reference></SignedInfo>
    <SignatureValue>{signature}</SignatureValue>
        ...
        logger.info("Documento assinado: %s", document_id)
...
def init_sefaz_manager(
    ...
    global _sefaz_manager
    _sefaz_manager = SEFAZManager(emitente, certificate_path, certificate_password, ambiente)
    return _sefaz_manager
