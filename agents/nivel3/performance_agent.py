...
Mede: tempo de resposta, detecta degradação,
monitora uso de memória, alerta N+1 queries.
"""
import json
import subprocess
import time
...
class PerformanceAgent:
    ...
    def verificar_memoria_container(self) -> dict:
        """Verifica uso de memória do container backend."""
        try:
    ...
    def auditar(self) -> dict:
            ...
            "bugs": [
                {
                    ...
                    "tempo_ms": m["tempo_ms"],
                    "descricao": (
                        f"Endpoint {m['classificacao']}: "
                        f"{m['endpoint']} ({m['tempo_ms']}ms)"
                    ),
        ...
        print(f"  Tempo médio: {tempo_medio}ms")
        print(f"  Endpoints lentos: {len(lentos)}")
