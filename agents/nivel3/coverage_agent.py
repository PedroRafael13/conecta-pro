...
- Mismatch de campos entre payload frontend e schema backend
"""
import json
import re
...
class CoverageAgent:
    ...
    def auditar(self) -> dict:
        ...
        for path in paths_backend:
            if not any(
                self._paths_match(path, fp) for fp in paths_frontend
            ):
                if not any(
                    x in path
                    for x in ["/health", "/docs", "/openapi", "/auth"]
                ):
                    sem_frontend.append(path)

        sem_backend = []
        for path in paths_frontend:
            if not any(
                self._paths_match(path, bp) for bp in paths_backend
            ):
                sem_backend.append(path)
        ...
        if sem_frontend:
            resultado["bugs"].append({
                ...
                "descricao": (
                    f"{len(sem_frontend)} endpoints backend "
                    "sem chamada no frontend"
                ),
                "detalhes": sem_frontend[:10],
            })
        ...
        if sem_backend:
            resultado["bugs"].append({
                ...
                "descricao": (
                    f"{len(sem_backend)} chamadas frontend "
                    "para endpoints inexistentes"
                ),
                "detalhes": sem_backend[:5],
            })
    ...
    def _paths_match(self, path1: str, path2: str) -> bool:
        """Compara paths normalizando IDs e parâmetros."""
