...
Detecta: endpoints sem auth, headers ausentes, IDOR básico.
...
class SecurityAgent:
    ...
    def _request(self, path: str, token: str = None, method: str = "GET", data: bytes = None) -> tuple:
    ...
    def testar_registro_sem_auth(self) -> list:
        ...
        if status in [200, 201]:
            return [{"endpoint": "/api/v1/auth/register",
    ...
    def auditar(self) -> dict:
        ...
        score = max(0, min(10.0, 10.0 - len(criticos) * 3.0 - len(altos) * 1.5 - len(todos_bugs) * 0.3))
        resultado = {
            "agente": "security",
            "score": round(score, 1),
            ...
            "altas": len(altos),
            "bugs": [{"tipo": "vulnerabilidade", "severidade": b.get("severidade"),
                      "descricao": b.get("descricao"), "autocorrigivel": False, "acao_jordan": True}
                     for b in todos_bugs],
        }
        print(f"  Vulns: {len(todos_bugs)} ({len(criticos)} críticas), Score: {resultado['score']}/10")
        return resultado
