...
OBRIGACOES = [
    {"nome": "CRF FGTS", "endpoint": "/api/v1/government/fgts/status", "alerta_dias": 15, "tipo": "certidao"},
    {"nome": "CND Federal", "endpoint": "/api/v1/government/certidoes", "alerta_dias": 30, "tipo": "certidao"},
    {"nome": "eSocial pendentes", "endpoint": "/api/v1/government/esocial/eventos", "alerta_dias": 0, "tipo": "esocial"},
    {"nome": "DCTFWeb", "endpoint": "/api/v1/government/dctfweb/status", "alerta_dias": 5, "tipo": "fiscal"},
]
        ...
        for cert in CERT_PATH.glob("*.p12"):
            try:
                result = subprocess.run(
                    ["openssl", "pkcs12", "-in", str(cert), "-nokeys", "-passin", "pass:", "-legacy"],
                    capture_output=True, text=True, timeout=10,
                )
                date_match = re.search(r"notAfter=(.+)", result.stdout + result.stderr)
                if date_match:
                    expiry = datetime.strptime(date_match.group(1).strip(), "%b %d %H:%M:%S %Y %Z").date()
                ...
                alertas.append({"certificado": cert.name, "erro": str(e), "status": "ERRO"})
        return alertas
    ...
    def auditar(self) -> dict:
        ...
        for obr in OBRIGACOES:
            ...
            if "error" in data:
                continue
            if obr["tipo"] == "esocial":
                pendentes = [e for e in data.get("items", []) if e.get("status") in ["pendente", "erro"]]
                if pendentes:
                    alertas.append({
                ...
                vencimento = data.get("vencimento") or data.get("validade") or data.get("expiry")
                if vencimento:
                    try:
        ...
        resultado = {
            ...
            "bugs": [
                {"tipo": a["tipo"], "descricao": a["descricao"],
                 "acao_jordan": a.get("acao_jordan", False), "autocorrigivel": False}
                for a in alertas
            ],
        }
        print(f"  OK: {ok}/{total}, Alertas: {len(alertas)}, Score: {resultado['score']}/10")
        return resultado
