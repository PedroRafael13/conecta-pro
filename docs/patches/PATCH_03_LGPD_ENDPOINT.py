...
from core.auth.dependencies import get_current_user, require_authentication
from core.database import get_db
from core.audit.audit_log import AuditLog, AuditAction
...
class LGPDDataManager:
    ...
    FISCAL_TABLES = {
        'nfe', 'nfse', 'invoices', 'notas_fiscais',
        'sped_files', 'fiscal_obligations'
    }

    # Campos sensíveis que devem ser anonimizados
    SENSITIVE_FIELDS = {
        'cpf', 'cnpj', 'rg', 'email', 'telefone', 'celular',
        'endereco', 'address', 'nome', 'name', 'sobrenome', 'lastname'
    }

    def __init__(self, db: AsyncSession, user_id: UUID, tenant_id: Optional[UUID] = None):
        self.db = db
    ...
    async def _export_profile(self) -> Dict:
        ...
        return {
            "user_record": dict(user._mapping) if user else None
        }
...
async def export_user_data(
    ...
    try:
        ...
        return JSONResponse(
            content=data,
            headers={
                "Content-Disposition": f"attachment; filename=lgpd_export_{current_user.id}.json"
            }
        )
...
async def anonymize_user_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erro ao anonimizar dados"
        )
    ...
    **Atenção:** Esta operação é irreversível!
    ...
    if not confirm:
        raise HTTPException(
            ...
            detail="Confirmação necessária. Adicione ?confirm=true"
        )
    ...
    try:
        ...
        # 1. Exporta dados antes de anonimizar
        export_data = await manager.export_user_data()
        ...
        background_tasks.add_task(
            ...
            details={
                "anonymization_result": result.to_dict(),
                "export_stored": True
            }
        )
