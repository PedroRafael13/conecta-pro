...
async def test_rfid_scaled_roi():
    ...
    economia_inventarios = inventarios_anuais_total * (horas_inventario_tradicional - horas_inventario_rfid) * custo_hora_funcionario
    ...
    economia_tempo_busca = (buscas_equipamento_mes_total * 12 * 
                           (tempo_busca_tradicional - tempo_busca_rfid) / 60 * 
                           custo_hora_funcionario)
    ...
    # ROI total
    roi_total = (economia_inventarios + reducao_perdas + economia_manutencao + 
                economia_tempo_busca + economia_auditorias + economia_garantias_seguros +
                economia_substituicoes + economia_depreciacao + reducao_premio_seguro +
                eficiencia_operacional)
    
    print(f'📊 COMPONENTES DO ROI (ESCALA CORPORATIVA):')
