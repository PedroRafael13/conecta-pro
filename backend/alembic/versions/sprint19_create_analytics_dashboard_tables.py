...
down_revision: Union[str, None] = 'sprint18_mobile_time_clock'
...
def upgrade() -> None:
    ...
    op.create_table(
        'dashboard_widgets',
        sa.Column('id', UUID(as_uuid=True), primary_key=True),
        sa.Column('dashboard_id', UUID(as_uuid=True), sa.ForeignKey('dashboard_configs.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_table(
        'report_executions',
        sa.Column('id', UUID(as_uuid=True), primary_key=True),
        sa.Column('report_id', UUID(as_uuid=True), sa.ForeignKey('scheduled_reports.id', ondelete='CASCADE'), nullable=False),
