...
def upgrade() -> None:
    ...
    # Criar Enums (safe)
    # ============================================================
    create_enum_safe("sentiment_type", ["very_positive", "positive", "neutral",
                                         "negative", "very_negative", "mixed"])

    create_enum_safe("emotion_type", ["joy", "satisfaction", "gratitude", "trust",
                                       "anticipation", "surprise", "neutral", "concern",
                                       "frustration", "disappointment", "anger", "fear",
                                       "sadness", "disgust", "urgency"])

    create_enum_safe("source_type", ["ticket", "email", "chat", "review", "survey",
                                      "social_media", "call_transcript", "feedback_form",
                                      "complaint", "suggestion", "comment", "nps_response",
                                      "whatsapp", "other"])
    ...
    create_enum_safe("trend_category", ["overall", "by_source", "by_customer", "by_segment",
                                         "by_product", "by_service", "by_topic", "by_aspect"])

    create_enum_safe("insight_type", ["sentiment_drop", "sentiment_spike", "sentiment_anomaly",
                                       "emerging_topic", "trending_topic", "recurring_issue",
                                       "churn_risk", "customer_champion", "customer_recovery",
                                       "service_issue", "product_issue", "process_bottleneck",
                                       "upsell_opportunity", "improvement_suggestion", "feature_request",
                                       "benchmark_deviation", "competitor_mention",
                                       "urgent_attention", "compliance_risk",
                                       "success_story", "team_recognition"])
    ...
    create_enum_safe("rule_category", ["sentiment", "emotion", "keyword", "aspect", "urgency",
                                        "churn", "escalation", "notification", "classification", "custom"])

    create_enum_safe("rule_action", ["alert", "escalate", "notify_email", "notify_sms", "notify_slack",
                                      "create_ticket", "assign_agent", "tag", "priority_boost",
                                      "auto_respond", "trigger_workflow", "log"])
    ...
    op.create_table(
        ...
        # Fonte
        sa.Column('source_type', ENUM('ticket', 'email', 'chat', 'review', 'survey',
            'social_media', 'call_transcript', 'feedback_form', 'complaint',
            'suggestion', 'comment', 'nps_response', 'whatsapp', 'other',
        ...
        sa.Column('sentiment_type', ENUM('very_positive', 'positive', 'neutral',
            'negative', 'very_negative', 'mixed',
            name='sentiment_type', create_type=False), server_default='neutral'),
        ...
        # Emocoes detectadas
        sa.Column('primary_emotion', ENUM('joy', 'satisfaction', 'gratitude', 'trust',
            'anticipation', 'surprise', 'neutral', 'concern', 'frustration',
            'disappointment', 'anger', 'fear', 'sadness', 'disgust', 'urgency',
            name='emotion_type', create_type=False)),
        sa.Column('secondary_emotion', ENUM('joy', 'satisfaction', 'gratitude', 'trust',
            'anticipation', 'surprise', 'neutral', 'concern', 'frustration',
            'disappointment', 'anger', 'fear', 'sadness', 'disgust', 'urgency',
            name='emotion_type', create_type=False)),
        ...
        sa.Column('status', ENUM('pending', 'processing', 'completed', 'failed', 'requires_review',
            name='analysis_status', create_type=False), server_default='pending'),
        ...
        sa.Column('corrected_sentiment', ENUM('very_positive', 'positive', 'neutral',
            'negative', 'very_negative', 'mixed',
    ...
    op.create_table(
        ...
        # Categoria
        sa.Column('category', ENUM('sentiment', 'emotion', 'keyword', 'aspect', 'urgency',
            'churn', 'escalation', 'notification', 'classification', 'custom',
            name='rule_category', create_type=False), nullable=False),
        ...
        # Acoes
        sa.Column('primary_action', ENUM('alert', 'escalate', 'notify_email', 'notify_sms',
            'notify_slack', 'create_ticket', 'assign_agent', 'tag', 'priority_boost',
            'auto_respond', 'trigger_workflow', 'log',
            name='rule_action', create_type=False), server_default='alert'),
    ...
    op.create_table(
        ...
        # Periodo
        sa.Column('period_type', ENUM('hourly', 'daily', 'weekly', 'monthly', 'quarterly', 'yearly',
            name='trend_period', create_type=False), nullable=False),
        sa.Column('period_start', sa.DateTime, nullable=False),
        sa.Column('period_end', sa.DateTime, nullable=False),
        sa.Column('period_label', sa.String(50)),

        # Categoria
        sa.Column('category', ENUM('overall', 'by_source', 'by_customer', 'by_segment',
            'by_product', 'by_service', 'by_topic', 'by_aspect',
            name='trend_category', create_type=False), server_default='overall'),
        ...
        # Tendencia
        sa.Column('trend_direction', ENUM('improving', 'stable', 'declining', 'volatile',
            name='trend_direction', create_type=False), server_default='stable'),
    ...
    op.create_table(
        ...
        # Tipo e prioridade
        sa.Column('insight_type', ENUM('sentiment_drop', 'sentiment_spike', 'sentiment_anomaly',
            'emerging_topic', 'trending_topic', 'recurring_issue', 'churn_risk',
            'customer_champion', 'customer_recovery', 'service_issue', 'product_issue',
            'process_bottleneck', 'upsell_opportunity', 'improvement_suggestion',
            'feature_request', 'benchmark_deviation', 'competitor_mention',
            'urgent_attention', 'compliance_risk', 'success_story', 'team_recognition',
            name='insight_type', create_type=False), nullable=False),
        sa.Column('priority', ENUM('critical', 'high', 'medium', 'low', 'info',
            name='insight_priority', create_type=False), server_default='medium'),
        sa.Column('status', ENUM('new', 'acknowledged', 'in_progress', 'implemented', 'dismissed', 'expired',
            name='insight_status', create_type=False), server_default='new'),
