...
def upgrade() -> None:
    ...
    report_priority_enum = postgresql.ENUM(
        "low", "normal", "high", "critical", name="report_priority_enum", create_type=False
    )
    ...
    template_status_enum = postgresql.ENUM(
        "draft", "active", "inactive", "deprecated", "archived", name="template_status_enum", create_type=False
    )
    ...
    execution_trigger_enum = postgresql.ENUM(
        "manual", "scheduled", "api", "webhook", "event", "retry", name="execution_trigger_enum", create_type=False
    )
    ...
    widget_size_enum = postgresql.ENUM(
        "small", "medium", "large", "full", "custom", name="widget_size_enum", create_type=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "priority",
            postgresql.ENUM("low", "normal", "high", "critical", name="report_priority_enum", create_type=False),
            nullable=False,
            default="normal",
        ),
        sa.Column("template_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_report_templates.id"), nullable=True),
        ...
        sa.Column(
            "execution_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_report_executions.id"), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column("description", sa.Text, nullable=True),
        sa.Column(
            "template_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_report_templates.id"), nullable=False
        ),
        sa.Column(
            "status",
            postgresql.ENUM(
    ...
    op.create_foreign_key(
        "fk_execution_schedule", "ai_report_executions", "ai_report_schedules", ["schedule_id"], ["id"]
    )
    ...
    op.create_table(
        ...
        sa.Column("template_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_report_templates.id"), nullable=True),
        sa.Column(
            "parent_section_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_report_sections.id"), nullable=True
        ),
        sa.Column(
            "section_type",
            postgresql.ENUM(
    ...
    op.create_table(
        ...
        sa.Column(
            "size",
            postgresql.ENUM("small", "medium", "large", "full", "custom", name="widget_size_enum", create_type=False),
            default="medium",
        ),
        sa.Column("width", sa.String(50), nullable=True),
        sa.Column("height", sa.String(50), nullable=True),
