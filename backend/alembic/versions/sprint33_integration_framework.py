...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            'connector_type',
            sa.Enum(
                'bling', 'solides', 'dominio', 'omie', 'nibo', 'totvs', 'sap', 'custom',
                name='connector_type'
            ),
            nullable=False,
            default='custom'
        ...
        sa.Column(
            'auth_type',
            sa.Enum(
                'api_key', 'oauth2', 'oauth2_client_credentials', 'basic', 'mtls', 'certificate', 'custom',
                name='auth_type'
            ),
            nullable=False,
            default='api_key'
    ...
    op.create_table(
        ...
        sa.Column(
            'status',
            sa.Enum('pending', 'running', 'completed', 'failed', 'cancelled', 'partial', 'timeout', name='sync_run_status'),
            nullable=False,
            default='pending'
        ),
        sa.Column('status_message', sa.Text, nullable=True),
    ...
    op.create_unique_constraint(
        'uq_sync_state_tenant_account_entity',
        'sync_states',
        ['tenant_id', 'account_id', 'entity_type']
    )
