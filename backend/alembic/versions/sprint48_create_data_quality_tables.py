...
def upgrade() -> None:
    ...
    rule_type_enum = postgresql.ENUM(
        'REQUIRED', 'UNIQUE', 'FORMAT', 'RANGE', 'LENGTH', 'PATTERN',
        'CUSTOM', 'REFERENCE', 'CONSISTENCY', 'COMPLETENESS',
        'CPF_VALID', 'CNPJ_VALID', 'EMAIL_VALID', 'PHONE_VALID',
        'CEP_VALID', 'DATE_VALID', 'NUMERIC_RANGE', 'STRING_LENGTH',
        'ENUM_VALUE', 'REGEX_MATCH', 'NOT_NULL', 'NOT_EMPTY',
        'POSITIVE_NUMBER', 'NON_NEGATIVE', 'PAST_DATE', 'FUTURE_DATE',
        'DATE_RANGE', 'VALUE_LIST', 'FOREIGN_KEY', 'CROSS_FIELD',
        'CONDITIONAL', 'AGGREGATE', 'STATISTICAL',
        name='ruletype_enum',
        create_type=False
    )
    ...
    rule_severity_enum = postgresql.ENUM(
        ...
        name='ruleseverity_enum',
        create_type=False
    )

    # Rule Status Enum
    rule_status_enum = postgresql.ENUM(
        'ACTIVE', 'INACTIVE', 'DRAFT', 'DEPRECATED', 'TESTING',
        name='rulestatus_enum',
        create_type=False
    )
    ...
    rule_category_enum = postgresql.ENUM(
        ...
        'UNIQUENESS', 'TIMELINESS', 'STANDARDIZATION',
    ...
    check_scope_enum = postgresql.ENUM(
        'FULL', 'INCREMENTAL', 'SAMPLE', 'SINGLE_RECORD', 'BATCH',
        name='checkscope_enum',
        create_type=False
    )

    # Issue Type Enum
    issue_type_enum = postgresql.ENUM(
        'MISSING_VALUE', 'INVALID_VALUE', 'DUPLICATE_RECORD', 'INCONSISTENT_DATA',
        'FORMAT_ERROR', 'RANGE_VIOLATION', 'REFERENCE_ERROR', 'PATTERN_MISMATCH',
        'LENGTH_VIOLATION', 'TYPE_MISMATCH', 'ENCODING_ERROR', 'COMPLETENESS_ERROR',
        'ACCURACY_ERROR', 'TIMELINESS_ERROR', 'STANDARDIZATION_ERROR',
        'OUTLIER_DETECTED', 'ANOMALY_DETECTED', 'CROSS_FIELD_ERROR',
        'BUSINESS_RULE_VIOLATION', 'CUSTOM_ERROR',
        name='issuetype_enum',
        create_type=False
    )
    ...
    issue_status_enum = postgresql.ENUM(
        ...
        'IGNORED', 'FALSE_POSITIVE', 'WONT_FIX',
    ...
    issue_resolution_enum = postgresql.ENUM(
        'MANUAL_FIX', 'AUTO_FIX', 'DATA_CORRECTION', 'RECORD_DELETION',
        'MERGE_RECORDS', 'IGNORE', 'FALSE_POSITIVE', 'DEFERRED',
    ...
    duplicate_type_enum = postgresql.ENUM(
        'EXACT', 'FUZZY', 'PARTIAL', 'PHONETIC', 'SEMANTIC',
        name='duplicatetype_enum',
        create_type=False
    )

    # Duplicate Status Enum
    duplicate_status_enum = postgresql.ENUM(
        'DETECTED', 'REVIEWING', 'CONFIRMED', 'MERGED',
        'REJECTED', 'DEFERRED', 'AUTO_MERGED',
    ...
    merge_strategy_enum = postgresql.ENUM(
        'KEEP_FIRST', 'KEEP_LAST', 'KEEP_MOST_COMPLETE',
        'MERGE_FIELDS', 'MANUAL',
    ...
    data_type_enum = postgresql.ENUM(
        ...
        'EMAIL', 'CPF', 'CNPJ', 'PHONE', 'CEP', 'URL', 'UUID',
        'JSON', 'ARRAY', 'CURRENCY', 'PERCENTAGE', 'UNKNOWN',
        name='datatype_enum',
        create_type=False
    )
    ...
    op.create_table(
        ...
        sa.Column('rule_type', postgresql.ENUM(
            'REQUIRED', 'UNIQUE', 'FORMAT', 'RANGE', 'LENGTH', 'PATTERN',
            'CUSTOM', 'REFERENCE', 'CONSISTENCY', 'COMPLETENESS',
            'CPF_VALID', 'CNPJ_VALID', 'EMAIL_VALID', 'PHONE_VALID',
            'CEP_VALID', 'DATE_VALID', 'NUMERIC_RANGE', 'STRING_LENGTH',
            ...
            'DATE_RANGE', 'VALUE_LIST', 'FOREIGN_KEY', 'CROSS_FIELD',
            'CONDITIONAL', 'AGGREGATE', 'STATISTICAL',
            name='ruletype_enum', create_type=False
        ), nullable=False),
        sa.Column('severity', postgresql.ENUM(
            'CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO',
            name='ruleseverity_enum', create_type=False
        ), nullable=False, server_default='MEDIUM'),
        sa.Column('category', postgresql.ENUM(
            'COMPLETENESS', 'ACCURACY', 'CONSISTENCY', 'VALIDITY',
            'UNIQUENESS', 'TIMELINESS', 'STANDARDIZATION',
            name='rulecategory_enum', create_type=False
        ), nullable=False),
        sa.Column('status', postgresql.ENUM(
            'ACTIVE', 'INACTIVE', 'DRAFT', 'DEPRECATED', 'TESTING',
            name='rulestatus_enum', create_type=False
    ...
    op.create_table(
        ...
        sa.Column('scope', postgresql.ENUM(
            'FULL', 'INCREMENTAL', 'SAMPLE', 'SINGLE_RECORD', 'BATCH',
            name='checkscope_enum', create_type=False
        ), nullable=False, server_default='FULL'),
        sa.Column('status', postgresql.ENUM(
            'PENDING', 'RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED', 'PARTIAL',
            name='checkstatus_enum', create_type=False
    ...
    op.create_table(
        ...
        sa.Column('issue_type', postgresql.ENUM(
            ...
            'OUTLIER_DETECTED', 'ANOMALY_DETECTED', 'CROSS_FIELD_ERROR',
            'BUSINESS_RULE_VIOLATION', 'CUSTOM_ERROR',
            name='issuetype_enum', create_type=False
        ), nullable=False),
        sa.Column('severity', postgresql.ENUM(
            'CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO',
            name='ruleseverity_enum', create_type=False
        ...
        sa.Column('status', postgresql.ENUM(
            'OPEN', 'ACKNOWLEDGED', 'IN_PROGRESS', 'FIXED',
            'IGNORED', 'FALSE_POSITIVE', 'WONT_FIX',
            name='issuestatus_enum', create_type=False
        ), nullable=False, server_default='OPEN'),
        sa.Column('resolution', postgresql.ENUM(
            'MANUAL_FIX', 'AUTO_FIX', 'DATA_CORRECTION', 'RECORD_DELETION',
            'MERGE_RECORDS', 'IGNORE', 'FALSE_POSITIVE', 'DEFERRED',
            name='issueresolution_enum', create_type=False
        )),
        sa.Column('entity_type', sa.String(100), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', postgresql.ENUM(
            'DETECTED', 'REVIEWING', 'CONFIRMED', 'MERGED',
            'REJECTED', 'DEFERRED', 'AUTO_MERGED',
            name='duplicatestatus_enum', create_type=False
        ), nullable=False, server_default='DETECTED'),
        sa.Column('duplicate_type', postgresql.ENUM(
            'EXACT', 'FUZZY', 'PARTIAL', 'PHONETIC', 'SEMANTIC',
            name='duplicatetype_enum', create_type=False
        ), nullable=False, server_default='FUZZY'),
        sa.Column('entity_type', sa.String(100), nullable=False),
        ...
        sa.Column('merge_strategy', postgresql.ENUM(
            'KEEP_FIRST', 'KEEP_LAST', 'KEEP_MOST_COMPLETE',
            'MERGE_FIELDS', 'MANUAL',
            name='mergestrategy_enum', create_type=False
        )),
    ...
    op.create_table(
        ...
        sa.Column('status', postgresql.ENUM(
            'PENDING', 'RUNNING', 'COMPLETED', 'FAILED',
            name='profilestatus_enum', create_type=False
        ), nullable=False, server_default='PENDING'),
        sa.Column('detected_type', postgresql.ENUM(
            'STRING', 'INTEGER', 'FLOAT', 'BOOLEAN', 'DATE', 'DATETIME',
            'EMAIL', 'CPF', 'CNPJ', 'PHONE', 'CEP', 'URL', 'UUID',
            'JSON', 'ARRAY', 'CURRENCY', 'PERCENTAGE', 'UNKNOWN',
            name='datatype_enum', create_type=False
        )),
        sa.Column('total_records', sa.Integer, server_default='0'),
