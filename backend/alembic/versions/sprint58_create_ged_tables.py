...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "document_id",
            UUID(as_uuid=False),
            sa.ForeignKey("ged_documents.id", ondelete="CASCADE"),
            nullable=False,
            index=True,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "document_id", UUID(as_uuid=False), sa.ForeignKey("ged_documents.id", ondelete="CASCADE"), primary_key=True
        ),
        sa.Column(
            "tag_id", UUID(as_uuid=False), sa.ForeignKey("ged_document_tags.id", ondelete="CASCADE"), primary_key=True
        ),
        sa.Column("created_at", sa.DateTime(), default=sa.func.now()),
