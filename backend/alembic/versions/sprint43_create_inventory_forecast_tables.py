...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "forecast_type",
            ENUM("demand", "reorder", "seasonal", "trend", name="forecast_type", create_type=False),
            server_default="demand",
            nullable=False,
        ),
        sa.Column(
            "status",
            ENUM("pending", "processing", "completed", "failed", "expired", name="forecast_status", create_type=False),
    ...
    # Indices
    op.create_index("ix_forecasts_product_status", "inventory_forecasts", ["product_id", "status"])
    op.create_index("ix_forecasts_dates", "inventory_forecasts", ["start_date", "end_date"])
    ...
    op.create_index("ix_forecast_results_forecast_date", "inventory_forecast_results", ["forecast_id", "date"])
    ...
    op.create_table(
        ...
        sa.Column(
            "seasonality_type",
            ENUM(
                "none", "weekly", "monthly", "quarterly", "yearly", "custom", name="seasonality_type", create_type=False
            ),
            server_default="none",
            nullable=False,
        ),
        ...
        sa.Column(
            "trend_direction",
            ENUM("stable", "increasing", "decreasing", "volatile", name="trend_direction", create_type=False),
    ...
    # Indices
    op.create_index("ix_patterns_product_active", "inventory_demand_patterns", ["product_id", "is_active"])
    op.create_index("ix_patterns_predictability", "inventory_demand_patterns", ["predictability_score"])
