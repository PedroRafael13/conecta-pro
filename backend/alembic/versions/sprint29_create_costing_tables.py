...
down_revision: Union[str, None] = 'sprint28_fiscal'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('alocacao_referencia_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('cost_allocations.id'), nullable=True),
        # Metadata
        sa.Column('metadata', postgresql.JSONB, nullable=True),
        sa.Column('observacoes', sa.Text, nullable=True),
        # Auditoria
    ...
    op.create_table(
        ...
        sa.Column('nome', sa.String(200), nullable=False),
        sa.Column('descricao', sa.Text, nullable=True),
        sa.Column('tipo', sa.String(50), nullable=False),  # ABC_COSTING, PROFITABILITY, VARIANCE, BREAK_EVEN, TREND, FORECAST
        sa.Column('status', sa.String(20), nullable=False, default='PENDING'),
        # Período
        sa.Column('periodo_inicio', sa.Date, nullable=True),
        sa.Column('periodo_fim', sa.Date, nullable=True),
