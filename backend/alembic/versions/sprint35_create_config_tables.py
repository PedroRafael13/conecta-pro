...
down_revision: Union[str, None] = 'sprint34_reports'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', postgresql.ENUM('active', 'inactive', 'suspended',
                  'blocked', 'trial', 'cancelled', name='tenant_status',
                  create_type=False), nullable=False, server_default='trial'),
        sa.Column('plano', postgresql.ENUM('free', 'basic', 'professional',
                  'enterprise', 'custom', name='tenant_plan',
                  create_type=False), nullable=False, server_default='free'),
        sa.Column('tipo', postgresql.ENUM('company', 'individual', 'government',
                  'nonprofit', 'educational', name='tenant_type',
                  create_type=False), nullable=False, server_default='company'),
        ...
        sa.Column('features_habilitadas', postgresql.JSONB, nullable=False,
                  server_default='[]'),
        sa.Column('modulos_habilitados', postgresql.JSONB, nullable=False,
                  server_default='[]'),
    ...
    op.create_table(
        ...
        sa.Column('categoria', postgresql.ENUM('general', 'security', 'notifications',
                  'integrations', 'appearance', 'billing', 'features', 'limits',
                  'custom', name='setting_category', create_type=False),
                  nullable=False, server_default='general'),
        sa.Column('tipo', postgresql.ENUM('string', 'integer', 'float', 'boolean',
                  'json', 'list', 'password', 'email', 'url', 'date',
                  'datetime', 'color', 'file', 'image', name='setting_type',
                  create_type=False), nullable=False, server_default='string'),
    ...
    op.create_table(
        ...
        sa.Column('tipo', postgresql.ENUM('string', 'integer', 'float', 'boolean',
                  ...
                  'datetime', 'color', 'file', 'image', name='setting_type',
                  create_type=False), nullable=False, server_default='string'),
        sa.Column('escopo', postgresql.ENUM('global', 'default', 'system',
                  'security', name='config_scope', create_type=False),
                  nullable=False, server_default='global'),
        sa.Column('prioridade', postgresql.ENUM('low', 'normal', 'high',
                  'critical', name='config_priority', create_type=False),
    ...
    op.create_table(
        ...
        sa.Column('nome', sa.String(200), nullable=False),
        sa.Column('descricao', sa.Text, nullable=True),
        sa.Column('status', postgresql.ENUM('active', 'inactive', 'scheduled',
                  'expired', 'testing', name='flag_status', create_type=False),
                  nullable=False, server_default='inactive'),
        sa.Column('tipo', postgresql.ENUM('release', 'experiment', 'operational',
                  'permission', 'kill_switch', name='flag_type', create_type=False),
                  nullable=False, server_default='release'),
        sa.Column('estrategia', postgresql.ENUM('all', 'none', 'percentage',
                  'gradual', 'tenant_list', 'user_list', 'attribute_based',
                  name='rollout_strategy', create_type=False),
        ...
        sa.Column('tenants_habilitados', postgresql.JSONB, nullable=False,
                  server_default='[]'),
        sa.Column('tenants_desabilitados', postgresql.JSONB, nullable=False,
                  server_default='[]'),
        sa.Column('usuarios_habilitados', postgresql.JSONB, nullable=False,
                  server_default='[]'),
        sa.Column('usuarios_desabilitados', postgresql.JSONB, nullable=False,
                  server_default='[]'),
    ...
    op.create_table(
        ...
        sa.Column('nome', sa.String(200), nullable=False),
        sa.Column('descricao', sa.Text, nullable=True),
        sa.Column('canal', postgresql.ENUM('email', 'sms', 'push', 'whatsapp',
                  'slack', 'webhook', 'in_app', 'telegram',
                  name='notification_channel', create_type=False),
                  nullable=False, server_default='email'),
        sa.Column('tipo', postgresql.ENUM('transactional', 'marketing', 'system',
                  'alert', 'reminder', 'welcome', 'password_reset', 'verification',
                  name='notification_type', create_type=False),
                  nullable=False, server_default='transactional'),
        sa.Column('status', postgresql.ENUM('draft', 'active', 'inactive',
                  'archived', 'testing', name='template_status',
        ...
        sa.Column('variaveis_obrigatorias', postgresql.JSONB, nullable=False,
                  server_default='[]'),
        ...
        sa.Column('respeitar_horario_silencioso', sa.Boolean, nullable=False,
                  server_default='true'),
        ...
        sa.Column('created_by', sa.String(100), nullable=True),
        sa.Column('updated_by', sa.String(100), nullable=True),
        sa.UniqueConstraint('tenant_id', 'codigo', 'canal',
                          name='uq_template_tenant_code_channel'),
    )
    ...
    op.create_index('ix_notification_templates_tenant_id',
                   'notification_templates', ['tenant_id'])
    op.create_index('ix_notification_templates_codigo',
                   'notification_templates', ['codigo'])
    op.create_index('ix_notification_templates_canal',
                   'notification_templates', ['canal'])
    op.create_index('ix_notification_templates_tipo',
                   'notification_templates', ['tipo'])
    op.create_index('ix_notification_templates_status',
