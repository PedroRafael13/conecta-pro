...
def upgrade() -> None:
    ...
    create_enum_safe(
        ...
        ["pending", "uploading", "uploaded", "processing", "transcribed", "analyzed", "failed", "archived"],
    )

    create_enum_safe(
        "voice_recording_source",
        [
            "phone_call",
            "voicemail",
            "meeting",
    ...
    create_enum_safe("audio_format", ["wav", "mp3", "ogg", "flac", "m4a", "webm", "aac", "wma", "amr", "other"])

    create_enum_safe(
        "transcription_status", ["pending", "processing", "completed", "partial", "failed", "reviewed", "corrected"]
    )

    create_enum_safe(
        "transcription_provider",
        ["google_speech", "aws_transcribe", "azure_speech", "whisper", "deepgram", "assemblyai", "vosk", "local"],
    )
    ...
    create_enum_safe("call_analysis_status", ["pending", "analyzing", "completed", "partial", "failed", "reviewed"])
    ...
    create_enum_safe("call_sentiment", ["very_positive", "positive", "neutral", "negative", "very_negative", "mixed"])
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.ForeignKey("ai_voice_recordings.id"),
            nullable=False,
            index=True,
        ),
        sa.Column("status", sa.String(30), nullable=False, server_default="pending", index=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "transcription_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("ai_transcriptions.id"),
            nullable=False,
            index=True,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "recording_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_voice_recordings.id"), nullable=True
        ),
        sa.Column(
            "transcription_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_transcriptions.id"), nullable=True
        ),
        sa.Column("raw_text", sa.Text, nullable=False),
        sa.Column("normalized_text", sa.Text, nullable=True),
        sa.Column(
            "command_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("ai_command_definitions.id"), nullable=True
        ),
