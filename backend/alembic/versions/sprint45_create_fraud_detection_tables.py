...
down_revision: str | None = "sprint44_contract_analysis"
...
def upgrade() -> None:
    ...
    alert_severity_enum = postgresql.ENUM("low", "medium", "high", "critical", name="alertseverity", create_type=False)
    ...
    pattern_status_enum = postgresql.ENUM(
        "draft", "active", "inactive", "testing", "deprecated", name="patternstatus", create_type=False
    )
    ...
    risk_level_enum = postgresql.ENUM(
        "minimal", "low", "medium", "high", "critical", "blocked", name="risklevel", create_type=False
    )
    ...
    op.create_index("ix_risk_profiles_entity", "risk_profiles", ["entity_type", "entity_id"], unique=True)
