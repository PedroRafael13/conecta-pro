...
def upgrade() -> None:
    ...
    op.create_unique_constraint(
        'uq_solides_sync_state_condominio_entity',
        'solides_sync_state',
        ['condominio_id', 'entity_type']
    )
    ...
    op.create_table(
        ...
        sa.Column(
            'status',
            sa.Enum('pending', 'resolved_solides', 'resolved_conecta', 'resolved_manual', 'ignored', name='solides_conflict_status'),
            nullable=False,
    ...
    op.create_index('ix_solides_conflict_pending', 'solides_sync_conflict', ['condominio_id', 'status'], postgresql_where=sa.text("status = 'pending'"))
    ...
    op.create_table(
        ...
        sa.Column(
            'last_sync_source',
            sa.Enum('solides', 'conecta', 'manual', name='solides_sync_source'),
            nullable=True
        ),
        # Auditoria
        sa.Column('ativo', sa.Boolean, nullable=False, server_default='true'),
    ...
    op.create_unique_constraint(
        'uq_solides_mapping_solides',
        'solides_entity_mapping',
        ['condominio_id', 'entity_type', 'solides_id']
    )
    op.create_unique_constraint(
        'uq_solides_mapping_conecta',
        'solides_entity_mapping',
        ['condominio_id', 'entity_type', 'conecta_id']
    )
    ...
    op.create_index('ix_solides_webhook_retry', 'solides_webhook_log', ['status', 'next_retry_at'], postgresql_where=sa.text("status = 'failed'"))
