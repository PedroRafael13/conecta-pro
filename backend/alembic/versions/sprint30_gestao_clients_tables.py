...
def upgrade() -> None:
    ...
    client_segment_enum = postgresql.ENUM(
        "small", "medium", "large", "enterprise", "vip", "strategic", name="client_segment_enum", create_type=False
    )

    document_type_enum = postgresql.ENUM(
        "cpf", "cnpj", "passport", "rg", "other", name="document_type_enum", create_type=False
    )
    ...
    sync_status_enum = postgresql.ENUM(
        "pending", "syncing", "synced", "error", "disabled", name="sync_status_enum", create_type=False
    )

    sync_direction_enum = postgresql.ENUM(
        "push", "pull", "bidirectional", name="sync_direction_enum", create_type=False
    )

    # Create all enums
    op.execute(
        "CREATE TYPE client_type_enum AS ENUM ('pf', 'pj', 'condominium', 'holding', 'franchise', 'government', 'other')"
    )
    op.execute(
        "CREATE TYPE client_status_enum AS ENUM ('prospect', 'active', 'suspended', 'blocked', 'cancelled', 'defaulter', 'churned')"
    )
    ...
    op.execute(
        "CREATE TYPE condominium_type_enum AS ENUM ('residential', 'commercial', 'mixed', 'industrial', 'horizontal', 'gated_community', 'shopping')"
    )
    op.execute(
        "CREATE TYPE condominium_status_enum AS ENUM ('prospect', 'implantation', 'active', 'suspended', 'cancelled')"
    )
    op.execute(
        "CREATE TYPE administration_type_enum AS ENUM ('self_managed', 'administrator', 'property_manager', 'hybrid')"
    )
    op.execute(
        "CREATE TYPE unit_type_enum AS ENUM ('apartment', 'house', 'store', 'office', 'warehouse', 'parking', 'storage', 'penthouse', 'duplex', 'triplex', 'garden', 'rooftop', 'common_area', 'commercial')"
    )
    op.execute(
        "CREATE TYPE unit_status_enum AS ENUM ('available', 'occupied', 'vacant', 'renovation', 'blocked', 'reserved', 'defaulter')"
    )
    op.execute(
        "CREATE TYPE contract_service_type_enum AS ENUM ('portaria_remota', 'controle_acesso', 'cftv', 'alarme', 'cerca_eletrica', 'monitoramento_24h', 'app_morador', 'assembleia_virtual', 'manutencao', 'limpeza', 'jardinagem', 'administracao', 'consultoria', 'integracao', 'suporte')"
    )
    op.execute(
        "CREATE TYPE service_status_enum AS ENUM ('pending', 'implantation', 'active', 'suspended', 'cancelled', 'finished')"
    )
    op.execute(
        "CREATE TYPE integration_type_enum AS ENUM ('guardian', 'plus', 'erp_external', 'banking', 'nfe', 'whatsapp', 'email', 'sms', 'webhook')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "condominium_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("condominiums.id", ondelete="SET NULL"),
            nullable=True,
        ),
