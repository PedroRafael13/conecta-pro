...
down_revision: Union[str, None] = 'sprint52_voice'
...
def upgrade() -> None:
    ...
    knowledge_base_status_enum = postgresql.ENUM(
        'draft', 'active', 'inactive', 'archived', 'maintenance',
        name='knowledge_base_status_enum',
        create_type=False
    )
    knowledge_base_status_enum.create(op.get_bind(), checkfirst=True)

    knowledge_base_type_enum = postgresql.ENUM(
        'general', 'technical', 'support', 'faq', 'procedures',
        'policies', 'training', 'product', 'internal', 'external',
    ...
    knowledge_base_visibility_enum = postgresql.ENUM(
        ...
        name='knowledge_base_visibility_enum',
        create_type=False
    )
    ...
    article_type_enum = postgresql.ENUM(
        'how_to', 'tutorial', 'guide', 'reference', 'troubleshooting',
        'announcement', 'policy', 'procedure', 'best_practice', 'glossary',
    ...
    qa_interaction_type_enum = postgresql.ENUM(
        'question', 'clarification', 'follow_up', 'feedback', 'escalation',
    ...
    qa_response_type_enum = postgresql.ENUM(
        'direct_answer', 'article_match', 'faq_match', 'generated',
        'no_answer', 'escalated', 'clarification_needed',
    ...
    op.create_table(
        ...
        sa.Column('knowledge_base_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_knowledge_bases.id'), nullable=False),
        sa.Column('created_at', sa.DateTime, server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime, server_default=sa.func.now(), onupdate=sa.func.now()),
        sa.Column('ativo', sa.Boolean, server_default='true'),
    )
    ...
    op.create_table(
        ...
        sa.Column('parent_article_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_kb_articles.id'), nullable=True),
        sa.Column('view_count', sa.Integer, server_default='0'),
    ...
    op.create_table(
        ...
        sa.Column('knowledge_base_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_knowledge_bases.id'), nullable=False),
        sa.Column('category_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_kb_categories.id'), nullable=True),
        sa.Column('related_article_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_kb_articles.id'), nullable=True),
        sa.Column('author_id', postgresql.UUID(as_uuid=True), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('knowledge_base_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_knowledge_bases.id'), nullable=True),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('condominio_id', postgresql.UUID(as_uuid=True), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('parent_interaction_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_qa_interactions.id'), nullable=True),
        sa.Column('created_at', sa.DateTime, server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime, server_default=sa.func.now(), onupdate=sa.func.now()),
    )
    ...
    op.create_table(
        ...
        sa.Column('target_article_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('ai_kb_articles.id'), nullable=True),
        sa.Column('created_at', sa.DateTime, server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime, server_default=sa.func.now(), onupdate=sa.func.now()),
        sa.Column('ativo', sa.Boolean, server_default='true'),
    )
