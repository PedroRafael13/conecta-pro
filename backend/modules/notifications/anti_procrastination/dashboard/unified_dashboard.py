...
from ..models import (
    PendingTask, PendingTaskData, TaskFilter, SystemSummary, 
    DepartmentSummary, Department, TaskPriority, TaskStatus,
    TaskCategory, EscalationLevel
)
...
class UnifiedDashboard:
    ...
    async def get_department_dashboard(
        ...
        tasks_query = self.db.query(PendingTask).filter(
            and_(
        ...
        urgent_tasks = sorted(
            [self._task_to_data(task) for task in tasks],
    ...
    def _count_overdue_tasks(self) -> int:
        ...
        return self.db.query(PendingTask).filter(
            and_(
                PendingTask.due_date < date.today(),
                PendingTask.status == TaskStatus.PENDING
            )
        ).count()
    ...
    def _calculate_avg_resolution_time(self) -> float:
        ...
        completed_tasks = self.db.query(PendingTask).filter(
    ...
    async def _identify_bottlenecks(self) -> List[Dict[str, Any]]:
        ...
        # Por departamento
        for dept in Department:
            old_tasks = self.db.query(PendingTask).filter(
                and_(
                    PendingTask.department == dept,
                    PendingTask.status == TaskStatus.PENDING,
        ...
        for category in TaskCategory:
            escalated_tasks = self.db.query(PendingTask).filter(
                and_(
                    PendingTask.category == category,
                    PendingTask.escalation_level > EscalationLevel.LEVEL_1
                )
            ).count()
    ...
    async def _get_critical_alerts(self) -> List[Dict[str, Any]]:
        ...
        critical_tasks = self.db.query(PendingTask).filter(
            and_(
    ...
    async def _generate_executive_recommendations(self) -> List[str]:
        ...
        if summary.critical_count > 10:
            recommendations.append(
                f"🚨 Atenção: {summary.critical_count} tarefas críticas pendentes. "
                "Considere realocação de recursos."
            )
        
        if summary.overdue_count > 20:
            recommendations.append(
                f"⏰ {summary.overdue_count} tarefas atrasadas. "
                "Revisar processos e prazos."
            )
