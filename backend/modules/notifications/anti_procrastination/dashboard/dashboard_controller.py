...
from ..models import TaskFilter, Department, TaskCategory, TaskPriority, TaskStatus, EscalationLevel
...
async def get_executive_dashboard(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Erro ao carregar dashboard executivo: {str(e)}"
        )
...
async def get_system_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao carregar resumo do sistema: {str(e)}"
        )
...
async def search_tasks(
    # Filtros de busca
    department: Optional[Department] = Query(None, description="Filtrar por departamento"),
    category: Optional[TaskCategory] = Query(None, description="Filtrar por categoria"),
    ...
    # Filtros de tempo
    days_pending_min: Optional[int] = Query(None, ge=0, description="Mínimo de dias pendente"),
    days_pending_max: Optional[int] = Query(None, ge=0, description="Máximo de dias pendente"),
    
    # Filtros de escalation
    escalation_level: Optional[EscalationLevel] = Query(None, description="Nível de escalation"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro na busca de tarefas: {str(e)}"
        )
...
async def get_realtime_dashboard(
    ...
    try:
        ...
        if departments:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"Departamento inválido: {str(e)}"
                )
        ...
        realtime_data = {
            ...
            "worst_department": max(departments_data, key=lambda d: d.critical_tasks + d.overdue_tasks).department.value if departments_data else None
        }
...
async def get_productivity_metrics(
    ...
    department: Optional[Department] = Query(None, description="Departamento específico"),
    current_user: User = Depends(get_current_user),
    dashboard: UnifiedDashboard = Depends(get_dashboard_service)
) -> Dict[str, Any]:
    """
    ...
    if not _has_executive_permission(user):
        ...
        if user_dept:
            try:
                ...
                pass  # Departamento inválido, mantém filtro original
