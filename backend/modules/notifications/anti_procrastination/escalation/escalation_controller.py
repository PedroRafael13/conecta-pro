...
from .escalation_engine import EscalationEngine, EscalationRule, TaskCategory, TaskPriority, Department, EscalationAction
...
class EscalationRuleRequest(BaseModel):
    ...
    level_2_actions: List[EscalationAction] = Field(default=[EscalationAction.NOTIFY_USER, EscalationAction.NOTIFY_SUPERVISOR])
...
class ManualEscalationRequest(BaseModel):
    ...
    force_level: Optional[int] = Field(None, ge=1, le=4)
    ...
    try:
        ...
        if not _has_escalation_permission(current_user):
            raise HTTPException(
                ...
                detail="Acesso negado. Requer permissão de escalation."
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao processar escalation: {str(e)}"
        )
...
async def manual_escalation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro na escalation manual: {str(e)}"
        )
...
async def get_escalation_history(
    ...
    task_id: Optional[UUID] = Query(None, description="Filtrar por tarefa específica"),
    current_user: User = Depends(get_current_user),
    escalation_engine: EscalationEngine = Depends(get_escalation_service)
) -> List[Dict[str, Any]]:
    """
...
async def get_escalation_rules(
    ...
    try:
        ...
        if not _has_admin_permission(current_user):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Acesso negado. Requer permissão administrativa."
            )
...
async def update_escalation_rule(
    ...
    try:
        ...
        if success:
            return JSONResponse(content={
                "message": f"✅ Regra {rule_id} atualizada com sucesso.",
                "rule_id": str(rule_id)
            })
        else:
            raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao atualizar regra: {str(e)}"
        )
...
async def start_escalation_engine(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao iniciar engine: {str(e)}"
        )
...
async def stop_escalation_engine(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao parar engine: {str(e)}"
        )
