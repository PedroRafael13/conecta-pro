...
from ..models import Department, EscalationLevel, PendingTask, TaskCategory, TaskPriority, TaskStatus
...
class EscalationRule:
    ...
    level_2_actions: list[EscalationAction] = field(
        default_factory=lambda: [EscalationAction.NOTIFY_USER, EscalationAction.NOTIFY_SUPERVISOR]
    )
    level_3_actions: list[EscalationAction] = field(
        default_factory=lambda: [EscalationAction.NOTIFY_MANAGER, EscalationAction.SEND_EMAIL]
    )
    level_4_actions: list[EscalationAction] = field(
        default_factory=lambda: [EscalationAction.NOTIFY_DIRECTOR, EscalationAction.BLOCK_USER]
    )
...
class EscalationLog(Base):
    ...
    __table_args__ = {"comment": "Log de escalations automáticas executadas"}
...
class EscalationEngine:
    ...
    async def get_escalation_summary(self) -> dict[str, Any]:
        ...
        for level in EscalationLevel:
            count = (
                ...
                .filter(PendingTask.escalation_level == level, PendingTask.status == TaskStatus.PENDING)
                .count()
            )
        ...
        today_escalations = (
            self.db.query(EscalationLog)
            .filter(EscalationLog.executed_at >= datetime.combine(today, datetime.min.time()))
            .count()
        )
    ...
    async def get_escalation_history(self, days: int = 30, task_id: UUID | None = None) -> list[dict[str, Any]]:
        """
        ...
        query = self.db.query(EscalationLog).filter(EscalationLog.executed_at >= start_date)
    ...
    async def _process_escalations(self):
        ...
        pending_tasks = self.db.query(PendingTask).filter(PendingTask.status == TaskStatus.PENDING).all()
    ...
    def _check_escalation_needed(self, task: PendingTask) -> tuple[bool, EscalationLevel | None]:
        """
    ...
    def _find_applicable_rule(self, task: PendingTask) -> EscalationRule | None:
    ...
    async def _execute_action(
        self, action: EscalationAction, task: PendingTask, level: EscalationLevel
    ) -> dict[str, Any]:
        """
    ...
    async def _notify_manager(self, task: PendingTask, level: EscalationLevel) -> dict[str, Any]:
        ...
        return {"success": True, "details": "Gerente notificado sobre tarefa crítica", "notified_users": ["manager_id"]}
    ...
    async def _notify_director(self, task: PendingTask, level: EscalationLevel) -> dict[str, Any]:
        ...
        return {
            "success": True,
            "details": "Diretor notificado sobre tarefa em nível crítico",
    ...
    async def _send_escalation_email(self, task: PendingTask, level: EscalationLevel) -> dict[str, Any]:
        ...
        return {"success": True, "details": "Email de escalation enviado", "notified_users": []}
    ...
    async def _block_user_access(self, task: PendingTask, level: EscalationLevel) -> dict[str, Any]:
        ...
        return {
            "success": True,
            "details": "Acesso bloqueado até resolução da tarefa",
            "notified_users": [str(task.assigned_to)] if task.assigned_to else [],
        }
    ...
    def _get_default_rule(self) -> EscalationRule:
        """Retorna regra padrão genérica."""
        return EscalationRule(name="Default Rule", level_1_days=3, level_2_days=7, level_3_days=14, level_4_days=21)
