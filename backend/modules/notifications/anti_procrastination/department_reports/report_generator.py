...
from ..models import Department, PendingTask, TaskPriority, TaskStatus
...
class DepartmentReportGenerator:
    ...
    async def generate_executive_summary(self) -> dict[str, Any]:
        ...
        problem_departments = sorted(all_departments, key=lambda d: d.critical_tasks + d.overdue_tasks, reverse=True)
    ...
    async def _get_department_metrics(self, department: Department) -> DepartmentMetrics:
        ...
        tasks = (
            ...
            .filter(PendingTask.department == department, PendingTask.status == TaskStatus.PENDING)
            .all()
        )
