...
async def check_daily_requirement(
    current_user: User = Depends(get_current_user), checklist_service: ChecklistManager = Depends(get_checklist_service)
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        requirement = await checklist_service.check_daily_requirement(current_user.id, user_dept)

        return JSONResponse(content=requirement)

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao verificar requirement: {str(e)}"
        )
...
async def generate_daily_checklist(
    ...
    try:
        ...
        checklist_id = await checklist_service.generate_daily_checklist(current_user.id, user_name, user_dept)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao gerar checklist: {str(e)}"
        )
...
async def get_checklist(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao buscar checklist: {str(e)}"
        )
...
async def start_checklist(
    ...
    try:
        ...
        if success:
            return JSONResponse(
                content={"success": True, "message": "✅ Checklist iniciado. Revise cada item com atenção."}
            )
        else:
            raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao iniciar checklist: {str(e)}"
        )
...
async def update_checklist_item(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao atualizar item: {str(e)}"
        )
...
async def complete_checklist(
    ...
    try:
        success = await checklist_service.complete_checklist(
            checklist_id, feedback.feedback, feedback.satisfaction_score
        )
        ...
        if success:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Não foi possível concluir o checklist."
            )

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao concluir checklist: {str(e)}"
        )
...
async def get_compliance_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao carregar métricas: {str(e)}"
        )
...
async def enforce_daily_checklist(
    ...
    bypass_routes = ["/api/v1/daily-checklist", "/api/v1/auth", "/api/v1/health", "/docs", "/redoc"]
