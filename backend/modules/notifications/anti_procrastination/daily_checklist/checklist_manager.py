...
from ..models import Department, PendingTask, TaskPriority
...
class ChecklistItem:
    ...
    days_pending: int
    urgency_score: float
    action_taken: TaskAction | None = None
    comment: str = ""
...
class DailyChecklist(Base):
    ...
    __table_args__ = {"comment": "Checklists diários obrigatórios para revisão de pendências"}
...
class ChecklistManager:
    ...
    async def check_daily_requirement(self, user_id: UUID, user_department: str) -> dict[str, Any]:
        ...
        existing_checklist = (
            self.db.query(DailyChecklist)
            .filter(DailyChecklist.user_id == user_id, DailyChecklist.checklist_date == today)
            .first()
        )
        ...
        if not user_dept:
            return {"required": False, "blocked": False, "message": "Departamento não configurado para checklist."}
        ...
        if not pending_tasks:
            return {"required": False, "blocked": False, "message": "🎉 Sem tarefas pendentes! Acesso liberado."}
    ...
    async def generate_daily_checklist(self, user_id: UUID, user_name: str, user_department: str) -> UUID:
        """
    ...
    async def get_checklist(self, checklist_id: UUID) -> dict[str, Any]:
        ...
        """
        checklist = self.db.query(DailyChecklist).filter(DailyChecklist.id == checklist_id).first()
        ...
        return {
            ...
            "progress_percentage": round((checklist.completed_items / checklist.total_items) * 100, 1)
    ...
    async def complete_checklist(
        self, checklist_id: UUID, user_feedback: str = "", satisfaction_score: int = 5
    ) -> bool:
        """
    ...
    async def get_checklist_metrics(self, days: int = 30) -> dict[str, Any]:
        ...
        checklists = self.db.query(DailyChecklist).filter(DailyChecklist.checklist_date >= start_date).all()
        ...
        avg_time = (
            sum(c.time_spent_minutes for c in completed_checklists) / len(completed_checklists)
        ...
        avg_satisfaction = (
            sum(c.satisfaction_score for c in rated_checklists) / len(rated_checklists) if rated_checklists else 0
        )
    ...
    async def _get_user_pending_tasks(self, user_id: UUID, department: Department) -> list:
        ...
        user_tasks = (
            self.db.query(PendingTask).filter(PendingTask.assigned_to == user_id, PendingTask.status == "pending").all()
        )
        ...
        dept_tasks = (
            ...
            .filter(
                PendingTask.department == department, PendingTask.assigned_to.is_(None), PendingTask.status == "pending"
            )
            .all()
        )
    ...
    async def _calculate_daily_trend(self, start_date: date) -> list[dict[str, Any]]:
        ...
        while current_date <= date.today():
            day_checklists = self.db.query(DailyChecklist).filter(DailyChecklist.checklist_date == current_date).all()
