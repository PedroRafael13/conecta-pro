...
class IntelligentTaskPredictor:
    ...
    async def predict_critical_tasks(self, tenant_id: str, horizon: PredictionHorizon = PredictionHorizon.SHORT_TERM) -> List[TaskPrediction]:
        """
        ...
        try:
            ...
            task_predictions = await self._convert_ai_result_to_predictions(
                prediction_result, tenant_id, horizon
            )
            
            # Filtrar por confiança
            reliable_predictions = [
                pred for pred in task_predictions 
    ...
    async def predict_workload_distribution(self, tenant_id: str, days_ahead: int = 14) -> List[WorkloadPrediction]:
        ...
        try:
            ...
            workload_predictions = await self._convert_to_workload_predictions(
                prediction_result, tenant_id, days_ahead
            )
    ...
    async def analyze_behavioral_patterns(self, tenant_id: str) -> List[PatternInsight]:
        ...
        """
        try:
            logger.info(f"🎯 Analisando padrões comportamentais...")
            ...
            pattern_insights = await self._convert_to_pattern_insights(
    ...
    async def predict_escalation_needs(self, task_data: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Analisar resultado
            escalation_probability = float(prediction_result.prediction) if isinstance(prediction_result.prediction, (int, float)) else 0.5
    ...
    async def _convert_ai_result_to_predictions(self, prediction_result, tenant_id: str, horizon: PredictionHorizon) -> List[TaskPrediction]:
        ...
        for i in range(3):  # Simular 3 predições
            prediction = TaskPrediction(
                ...
                contributing_factors=[
                    f"Padrão sazonal identificado",
                    f"Sobrecarga prevista no departamento",
                    f"Dependência de tarefa externa"
                ],
                prevention_actions=[
    ...
    async def _convert_to_workload_predictions(self, prediction_result, tenant_id: str, days_ahead: int) -> List[WorkloadPrediction]:
