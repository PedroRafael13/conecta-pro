...
class ScaleOptimizer:
    ...
    def optimize(
        ...
        preferences: Optional[List[EmployeePreference]] = None,
    ) -> OptimizationResult:
        """
        ...
        """
        logger.info(
            f"Iniciando otimizacao: {len(slots)} slots, "
            f"{len(employees)} funcionarios"
        )
        ...
        emp_last_date: Dict[UUID, Optional[date]] = {e.employee_id: None for e in employees}
        
        # Ordena slots por prioridade (noturno e fim de semana primeiro)
        sorted_slots = sorted(
            slots,
            key=lambda s: (s.is_holiday, s.is_weekend, s.is_night_shift),
            reverse=True
        )
        ...
        coverage = self._calculate_coverage(
            [s for s in assigned_slots if s.employee_id],
            assigned_slots
        )
        ...
        logger.info(
            f"Otimizacao concluida: cobertura={coverage:.1f}%, "
            f"HE={overtime:.1f}h, score={quality:.1f}"
        )
    ...
    def _find_best_employee(
        ...
        emp_last_date: Dict[UUID, Optional[date]],
        constraints: OptimizationConstraints,
        ...
        for emp_id, emp in emp_data.items():
            ...
            if not self._check_constraints(
                emp_id, slot, emp_consecutive, emp_last_date, constraints, preference
            ):
                continue
            
            # Calcula score do candidato
            score = self._calculate_employee_score(
                emp_id, slot, emp, emp_hours, preference
            )
