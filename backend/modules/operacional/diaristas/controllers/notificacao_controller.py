...
class EnviarNotificacaoRequest(BaseModel):
    ...
    mensagem_custom: Optional[str] = Field(None, max_length=1000)
    titulo_custom: Optional[str] = Field(None, max_length=100)
...
async def listar_notificacoes(
    diarist_id: Optional[UUID] = Query(None, description="Filtrar por diarista"),
    tipo: Optional[TipoNotificacao] = Query(None, description="Filtrar por tipo"),
    status_filter: Optional[StatusNotificacao] = Query(None, alias="status", description="Filtrar por status"),
...
async def get_template(tipo: TipoNotificacao):
    ...
    if not template:
        raise HTTPException(
            ...
            detail=f"Template '{tipo.value}' não encontrado"
        )
...
@router.get(
    ...
    description="Lista canais de notificação disponíveis"
)
