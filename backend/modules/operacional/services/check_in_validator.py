...
class ValidationResult:
    ...
    is_valid: bool
    overall_score: float
    geo_validation: GeolocationValidation | None = None
    face_validation: FaceValidationResult | None = None
    time_validation: dict[str, Any] | None = None
...
class CheckInValidator:
    ...
    async def validate_check_in(
        ...
        """
        logger.info(f"Validando check-in: employee={data.employee_id}, shift={data.shift_id}")
        ...
        logger.info(f"Validacao concluida: score={overall_score:.1f}, valido={is_valid}, anomalias={len(anomalies)}")
