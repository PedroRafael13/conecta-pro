...
class RealTimeAnalyticsService:
    ...
    async def _detect_statistical_outlier(self, metric: MetricPoint) -> Optional[Anomaly]:
    ...
    async def _detect_trend_change(self, metric: MetricPoint) -> Optional[Anomaly]:
    ...
    async def _detect_pattern_break(self, metric: MetricPoint) -> Optional[Anomaly]:
    ...
    async def _detect_spike(self, metric: MetricPoint) -> Optional[Anomaly]:
    ...
    async def get_realtime_metrics(self) -> RealTimeMetrics:
        ...
        for metric_name, buffer in self.metric_buffers.items():
            if buffer:
                last_point = buffer[-1]
                current_metrics.append(MetricPoint(
                    name=metric_name,
    ...
    async def add_custom_threshold(self, metric_name: str, 
