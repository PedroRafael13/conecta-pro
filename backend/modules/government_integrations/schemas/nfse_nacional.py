...
class AmbienteNacionalEnum(str, Enum):
    """Ambientes do Padrao Nacional."""
    PRODUCAO = "producao"
    HOMOLOGACAO = "homologacao"


class TipoTributacaoEnum(str, Enum):
...
class RegimeEspecialEnum(str, Enum):
...
class MotivoCancelamentoEnum(str, Enum):
...
class PrestadorNacionalRequest(BaseModel):
    ...
    cnpj: str = Field(
        ...
        min_length=14,
        max_length=18,
        description="CNPJ do prestador (14 digitos)"
    )
    inscricao_municipal: str = Field(
        ...,
        min_length=1,
        max_length=15,
        description="Inscricao municipal do prestador"
    )
    ...
    razao_social: str = Field(
        ...
        min_length=2,
        max_length=150,
        description="Razao social do prestador"
    )
    nome_fantasia: Optional[str] = Field(
        None,
        max_length=60,
        description="Nome fantasia do prestador"
    )
    regime_especial: RegimeEspecialEnum = Field(
        default=RegimeEspecialEnum.ME_EPP_SIMPLES,
        description="Regime especial de tributacao"
    )
    optante_simples: bool = Field(
        default=True,
        description="Se e optante do Simples Nacional"
    )
...
class TomadorNacionalRequest(BaseModel):
    ...
    cpf_cnpj: str = Field(
        ...
        min_length=11,
        max_length=18,
        description="CPF (11 digitos) ou CNPJ (14 digitos) do tomador"
    )
    razao_social: str = Field(
        ...,
        min_length=2,
        max_length=150,
        description="Nome ou razao social do tomador"
    )
    ...
    inscricao_municipal: Optional[str] = Field(
        None,
        max_length=15,
        description="Inscricao municipal do tomador (se houver)"
    )
...
class ServicoNacionalRequest(BaseModel):
    ...
    codigo_tributacao_nacional: str = Field(
        default="1.1701.10.00",
        description="Codigo NBS (Nomenclatura Brasileira de Servicos) ou LC 116"
    )
    descricao: str = Field(
        ...,
        min_length=10,
        max_length=2000,
        description="Descricao detalhada do servico prestado"
    )
    ...
    valor_deducao: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        description="Valor de deducoes permitidas"
    )
    valor_desconto_incondicionado: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        description="Valor de desconto incondicionado"
    )
    valor_desconto_condicionado: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        description="Valor de desconto condicionado"
    )
    codigo_cnae: Optional[str] = Field(
        None,
        max_length=7,
        description="Codigo CNAE da atividade"
    )
    aliquota_iss: Decimal = Field(
        default=Decimal("0.05"),
        ge=0,
        le=1,
        description="Aliquota do ISS (0.05 = 5%)"
    )
    iss_retido: bool = Field(
        default=False,
        description="Se o ISS sera retido pelo tomador"
    )


class EmitirDPSRequest(BaseModel):
    """Request para emissao de DPS (Declaracao de Prestacao de Servicos)."""
    prestador: Optional[PrestadorNacionalRequest] = Field(
        None,
        description="Dados do prestador (opcional, usa config padrao se nao informado)"
    )
    ...
    competencia: Optional[str] = Field(
        None,
        pattern=r"^\d{4}-\d{2}$",
        description="Competencia no formato YYYY-MM (default: mes atual)"
    )
    tipo_tributacao: TipoTributacaoEnum = Field(
        default=TipoTributacaoEnum.TRIBUTACAO_MUNICIPIO,
        description="Tipo de tributacao"
    )
...
class ConsultarNFSeNacionalRequest(BaseModel):
    ...
    numero_nfse: str = Field(
        ...,
        description="Numero nacional da NFS-e"
    )
...
class CancelarNFSeNacionalRequest(BaseModel):
    ...
    numero_nfse: str = Field(
        ...,
        description="Numero nacional da NFS-e a cancelar"
    )
    motivo_cancelamento: MotivoCancelamentoEnum = Field(
        default=MotivoCancelamentoEnum.ERRO_EMISSAO,
        description="Motivo do cancelamento"
    )
    justificativa: Optional[str] = Field(
        None,
        max_length=255,
        description="Justificativa detalhada do cancelamento"
    )
...
class SubstituirNFSeNacionalRequest(BaseModel):
    ...
    numero_nfse_substituida: str = Field(
        ...,
        description="Numero nacional da NFS-e a ser substituida"
    )
    tomador: TomadorNacionalRequest
    servico: ServicoNacionalRequest
...
class DPSResponse(BaseModel):
    ...
    id_dps: Optional[str] = Field(None, description="ID da DPS")
    numero_nfse: Optional[str] = Field(None, description="Numero nacional da NFS-e")
    numero_dps: Optional[str] = Field(None, description="Numero da DPS")
    chave_acesso: Optional[str] = Field(None, description="Chave de acesso da NFS-e")
    ...
    data_emissao: Optional[datetime] = Field(None, description="Data/hora da emissao")
    payload_json: Optional[Dict[str, Any]] = Field(None, description="Payload JSON enviado")


class EmitirDPSResponse(DPSResponse):
    """Response para emissao de DPS."""
    valor_servico: Optional[Decimal] = Field(None, description="Valor do servico")
    valor_iss: Optional[Decimal] = Field(None, description="Valor do ISS")
    valor_liquido: Optional[Decimal] = Field(None, description="Valor liquido")
    tomador_cpf_cnpj: Optional[str] = Field(None, description="CPF/CNPJ do tomador")
    previsao_migracao: Optional[str] = Field(None, description="Previsao de migracao")
