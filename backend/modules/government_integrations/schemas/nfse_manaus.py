...
class NaturezaOperacaoEnum(StrEnum):
...
class CodigoCancelamentoEnum(StrEnum):
...
class TomadorRequest(BaseModel):
    ...
    razao_social: str = Field(..., min_length=2, max_length=150, description="Nome ou razão social do tomador")
    endereco: str = Field(..., min_length=1, max_length=125, description="Logradouro")
    numero: str = Field(default="S/N", max_length=10, description="Número do endereço")
    complemento: str | None = Field(None, max_length=60, description="Complemento do endereço")
    bairro: str = Field(..., min_length=1, max_length=60, description="Bairro")
    cidade: str = Field(default="Manaus", max_length=60, description="Cidade")
    ...
    inscricao_municipal: str | None = Field(
        None, max_length=15, description="Inscrição municipal do tomador (se houver)"
    )
...
class ServicoRequest(BaseModel):
    ...
    codigo_servico: str = Field(
        default="11.02", description="Código do serviço (Lista LC 116). Ex: 11.02 para vigilância"
    )
    discriminacao: str = Field(
        ..., min_length=10, max_length=2000, description="Descrição detalhada do serviço prestado"
    )
    valor_servicos: Decimal = Field(..., gt=0, description="Valor total dos serviços")
    valor_deducoes: Decimal = Field(default=Decimal("0"), ge=0, description="Valor de deduções permitidas")
    aliquota_iss: Decimal = Field(default=Decimal("0.05"), ge=0, le=1, description="Alíquota do ISS (0.05 = 5%)")
    iss_retido: bool = Field(default=False, description="Se o ISS será retido pelo tomador")
    codigo_cnae: str | None = Field(None, max_length=7, description="Código CNAE da atividade")
...
class EmitirNFSeRequest(BaseModel):
    ...
    competencia: str | None = Field(
        None, pattern=r"^\d{4}-\d{2}$", description="Competência no formato YYYY-MM (default: mês atual)"
    )
    natureza_operacao: NaturezaOperacaoEnum = Field(
        default=NaturezaOperacaoEnum.TRIBUTACAO_MUNICIPIO, description="Natureza da operação fiscal"
    )
    optante_simples: bool = Field(default=True, description="Se é optante do Simples Nacional")
...
class ConsultarNFSeRpsRequest(BaseModel):
    ...
    serie: str = Field(default="RPS", description="Série do RPS")
    tipo: str = Field(default="1", description="Tipo do RPS (1=RPS)")


class ConsultarNFSeNumeroRequest(BaseModel):
    """Request para consulta de NFS-e por número."""

    numero_nfse: str = Field(..., description="Número da NFS-e")
...
class CancelarNFSeRequest(BaseModel):
    ...
    codigo_cancelamento: CodigoCancelamentoEnum = Field(
        default=CodigoCancelamentoEnum.ERRO_EMISSAO, description="Código do motivo do cancelamento"
    )
    motivo: str | None = Field(None, max_length=255, description="Descrição do motivo do cancelamento")

    class Config:
        json_schema_extra = {
            "example": {"numero_nfse": "123456", "codigo_cancelamento": "1", "motivo": "Erro no valor do serviço"}
        }
...
class SubstituirNFSeRequest(BaseModel):
    ...
    numero_nfse_substituida: str = Field(..., description="Número da NFS-e a ser substituída")
    tomador: TomadorRequest
    servico: ServicoRequest
...
class NFSeResponse(BaseModel):
    ...
    numero_nfse: str | None = Field(None, description="Número da NFS-e")
    numero_rps: str | None = Field(None, description="Número do RPS")
    numero_lote: str | None = Field(None, description="Número do lote")
    codigo_verificacao: str | None = Field(None, description="Código de verificação")
    protocolo: str | None = Field(None, description="Protocolo de recebimento")
    status: NFSeStatusEnum = Field(..., description="Status da operação")
    mensagem: str | None = Field(None, description="Mensagem de retorno")
    data_emissao: datetime | None = Field(None, description="Data/hora da emissão")
    xml_envio: str | None = Field(None, description="XML enviado")


class EmitirNFSeResponse(NFSeResponse):
    """Response para emissão de NFS-e."""

    valor_servicos: Decimal | None = Field(None, description="Valor dos serviços")
...
class LoteNFSeResponse(BaseModel):
    ...
    numero_lote: str
    situacao: str
    quantidade_rps: int | None = None
    notas_processadas: list[NFSeResponse] | None = None
