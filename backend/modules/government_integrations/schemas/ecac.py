...
class SituacaoFiscalEnum(str, Enum):
...
class TipoPendenciaEnum(str, Enum):
...
class SituacaoDebitoEnum(str, Enum):
    """Situacao do debito."""
    ABERTO = "aberto"
    SUSPENSO = "suspenso"
    PARCELADO = "parcelado"
...
class SituacaoProcessoEnum(str, Enum):
...
class ConsultaSituacaoFiscalRequest(BaseModel):
    ...
    cpf_cnpj: Optional[str] = Field(
        None,
        description="CPF ou CNPJ para consulta (opcional, usa o configurado)"
    )
...
class ConsultaDebitosRequest(BaseModel):
    ...
    situacao: Optional[SituacaoDebitoEnum] = Field(
        None,
...
class ConsultaDeclaracoesRequest(BaseModel):
    ...
    exercicio_inicio: int = Field(
        ...,
        ge=2000,
        le=2100,
        description="Exercicio inicial"
    )
    exercicio_fim: Optional[int] = Field(
        None,
        ge=2000,
        le=2100,
...
class EmitirCertidaoRequest(BaseModel):
    ...
    finalidade: Optional[str] = Field(
        None,
        max_length=200,
        description="Finalidade da certidao"
    )
    cpf_cnpj: Optional[str] = Field(
        None,
...
class ValidarCertidaoRequest(BaseModel):
    ...
    codigo_controle: str = Field(
        ...,
        min_length=1,
        max_length=50,
        description="Codigo de controle da certidao"
    )
...
class SimularParcelamentoRequest(BaseModel):
    ...
    debitos: List[str] = Field(
        ...,
        min_length=1,
        description="Lista de codigos de debitos a parcelar"
    )
    quantidade_parcelas: int = Field(
        ...,
        ge=2,
        le=60,
        description="Quantidade de parcelas (2 a 60)"
    )
...
class ConsultaProcessosRequest(BaseModel):
    ...
    situacao: Optional[SituacaoProcessoEnum] = Field(
        None,
...
class SituacaoFiscalResponse(BaseModel):
    ...
    tipo_certidao_disponivel: Optional[TipoCertidaoEnum] = None
...
class ValidacaoCertidaoResponse(BaseModel):
    ...
    valida: bool
    data_validacao: str
    tipo_certidao: Optional[TipoCertidaoEnum] = None
...
class ParcelamentoResponse(BaseModel):
    ...
    valor_parcela: str
    situacao: str
    proxima_parcela: Optional[str] = None
    data_vencimento_proxima: Optional[str] = None
