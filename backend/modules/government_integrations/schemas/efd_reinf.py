...
class ClassificacaoTributariaEnum(str, Enum):
...
class GerarR1000Request(BaseModel):
    ...
    classificacao_tributaria: ClassificacaoTributariaEnum = Field(
        default=ClassificacaoTributariaEnum.EMPRESA_SIMPLES,
        description="Classificação tributária"
    )
    inicio_validade: str = Field(
        ...,
        pattern=r"^\d{4}-\d{2}$",
        description="Início da validade (YYYY-MM)"
    )
    fim_validade: Optional[str] = Field(
        None,
        pattern=r"^\d{4}-\d{2}$",
        description="Fim da validade (YYYY-MM)"
    )
    ...
    ind_coop: str = Field(
        default="0",
        pattern=r"^[0-3]$",
        description="Indicador de cooperativa (0=Não)"
    )
    ind_constr: str = Field(
        default="0",
        pattern=r"^[0-1]$",
        description="Indicador de construtora (0=Não)"
    )
    ind_desoneracao: str = Field(
        default="0",
        pattern=r"^[0-1]$",
        description="Indicador de desoneração (0=Não)"
    )
    ...
    retificacao: bool = Field(
        default=False,
        description="Se é retificação de evento anterior"
    )
...
class RetencaoServicoRequest(BaseModel):
    ...
    cnpj_prestador: str = Field(
        ...
        min_length=14,
        max_length=18,
        description="CNPJ do prestador do serviço"
    )
    ...
    valor_base_retencao: Optional[Decimal] = Field(
        None,
        ge=0,
        description="Base de cálculo da retenção (default: valor_bruto)"
    )
    valor_retencao: Decimal = Field(
        ...,
        ge=0,
        description="Valor da retenção (11% ou 3.5%)"
    )
    valor_retencao_adicional: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        description="Valor adicional de retenção"
    )
    serie_nf: str = Field(
        default="1",
        max_length=5,
    ...
    ind_cprb: str = Field(
        default="0",
        pattern=r"^[0-1]$",
        description="Indicador CPRB (0=Não)"
    )
...
class GerarR2010Request(BaseModel):
    ...
    retencoes: List[RetencaoServicoRequest] = Field(
        ...,
        min_length=1,
        description="Lista de retenções de serviços tomados"
    )
    retificacao: bool = Field(
        default=False,
        description="Se é retificação"
    )
...
class PagamentoPFRequest(BaseModel):
    ...
    natureza_rendimento: str = Field(
        ...,
        description="Código da natureza do rendimento"
    )
    valor_bruto: Decimal = Field(
        ...,
        gt=0,
    ...
    valor_irrf: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        description="Valor do IRRF retido"
    )
    valor_inss: Decimal = Field(
        default=Decimal("0"),
        ge=0,
...
class GerarR4010Request(BaseModel):
    ...
    pagamentos: List[PagamentoPFRequest] = Field(
        ...,
        min_length=1,
        description="Lista de pagamentos a pessoas físicas"
    )
    retificacao: bool = Field(
        default=False,
        description="Se é retificação"
...
class PagamentoPJRequest(BaseModel):
    ...
    razao_social: str = Field(
        ...
        min_length=2,
        max_length=150,
        description="Razão social do beneficiário"
    )
    natureza_rendimento: str = Field(
        ...,
        description="Código da natureza do rendimento"
    ...
    valor_csll: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        description="Valor da CSLL retida"
    )
    valor_cofins: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        description="Valor da COFINS retida"
    )
    valor_pis: Decimal = Field(
        default=Decimal("0"),
        ge=0,
...
class GerarR4020Request(BaseModel):
    ...
    pagamentos: List[PagamentoPJRequest] = Field(
        ...,
        min_length=1,
        description="Lista de pagamentos a pessoas jurídicas"
    )
    retificacao: bool = Field(
        default=False,
        description="Se é retificação"
...
class EventoResponse(BaseModel):
    ...
    periodo_apuracao: Optional[str] = Field(None, description="Período de apuração")
