...
class TipoPagamentoNFCe(StrEnum):
...
class ConsumidorNFCe(BaseModel):
    ...
    cpf: str | None = Field(None, min_length=11, max_length=14, description="CPF do consumidor")
    nome: str | None = Field(None, max_length=60, description="Nome do consumidor")
    email: str | None = Field(None, max_length=60, description="Email para envio do XML")
...
class ItemNFCe(BaseModel):
    ...
    ean: str | None = Field(None, max_length=14, description="Codigo EAN/GTIN")
    ...
    cest: str | None = Field(None, min_length=7, max_length=7, description="Codigo CEST")
...
class PagamentoNFCe(BaseModel):
    ...
    bandeira: str | None = Field(None, description="Bandeira do cartao")
    autorizacao: str | None = Field(None, description="Numero da autorizacao")
...
class NFCeEmissaoRequest(BaseModel):
    ...
    # Consumidor (opcional)
    consumidor: ConsumidorNFCe | None = Field(None, description="Dados do consumidor")

    # Itens
    itens: list[ItemNFCe] = Field(..., min_length=1, max_length=500, description="Lista de itens (max 500 para NFC-e)")

    # Pagamentos
    pagamentos: list[PagamentoNFCe] = Field(..., min_length=1, description="Formas de pagamento")
...
class NFCeConsultaRequest(BaseModel):
    ...
    chave_acesso: str = Field(..., min_length=44, max_length=44, description="Chave de acesso da NFC-e (44 digitos)")


class NFCeCancelamentoRequest(BaseModel):
    """Request para cancelamento de NFC-e."""

    chave_acesso: str = Field(..., min_length=44, max_length=44, description="Chave de acesso da NFC-e")
    justificativa: str = Field(
        ..., min_length=15, max_length=255, description="Justificativa do cancelamento (min 15 caracteres)"
    )
...
class NFCeInutilizacaoRequest(BaseModel):
    ...
    justificativa: str = Field(..., min_length=15, max_length=255, description="Justificativa da inutilizacao")
...
class NFCeContingenciaRequest(BaseModel):
    ...
    motivo_contingencia: str = Field(..., min_length=15, max_length=255, description="Motivo da contingencia")
...
class NFCeResponse(BaseModel):
    ...
    chave_acesso: str | None = Field(None, description="Chave de acesso")
    protocolo: str | None = Field(None, description="Protocolo de autorizacao")
    numero: int | None = Field(None, description="Numero da NFC-e")
    serie: int | None = Field(None, description="Serie da NFC-e")

    # QR Code
    qrcode_url: str | None = Field(None, description="URL do QR Code")
    url_consulta: str | None = Field(None, description="URL de consulta pela chave")

    # XML
    xml: str | None = Field(None, description="XML autorizado")

    # Dados adicionais
    data_autorizacao: str | None = Field(None, description="Data/hora autorizacao")
    valor_total: Decimal | None = Field(None, description="Valor total da NFC-e")
...
class NFCeStatusServicoResponse(BaseModel):
    ...
    tempo_medio: int | None = Field(None, description="Tempo medio de resposta (ms)")
...
class NFCeDANFERequest(BaseModel):
    ...
    chave_acesso: str = Field(..., min_length=44, max_length=44, description="Chave de acesso da NFC-e")
    formato: str = Field(
        default="pdf", pattern=r"^(pdf|html|escpos)$", description="Formato de saida (pdf, html, escpos)"
    )
