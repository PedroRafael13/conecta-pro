...
class ConfiguracaoExtracao:
    ...
    webhook_conclusao: str | None = None
...
class OrquestradorExtracao:
    ...
    def __init__(
        self,
        credential_provider: ProvedorCredenciais | None = None,
        queue_manager: GerenciadorFilas | None = None,
    ...
    async def validar_prerequisitos(self, tenant_id: UUID, servicos: list[TipoServico]) -> dict[str, Any]:
        """
        ...
        for servico in servicos:
            ...
            if tipo_cred:
                try:
                    validacao = await self.credentials.validar_credencial(tenant_id, tipo_cred)
                    resultado["certificados"][servico.value] = validacao

                    if not validacao.get("valida", False):
                        resultado["valido"] = False
                        resultado["erros"].append(f"Credencial inválida para {servico.value}: {validacao.get('erro')}")
        ...
        if servicos_sefaz:
            try:
                verificacao = await self.verificador.verificar_endpoint("SP", "nfe", usar_principal=True)
    ...
    async def iniciar_extracao(
        self, tenant_id: UUID, config: ConfiguracaoExtracao, usuario_id: UUID | None = None
    ) -> ResultadoExtracao:
        """
        ...
        logger.info(
            f"Extração iniciada: {resultado.id} - Tenant: {tenant_id} - Serviços: {[s.value for s in config.servicos]}"
        )
    ...
    async def _extrair_paralelo(self, tenant_id: UUID, config: ConfiguracaoExtracao, resultado: ResultadoExtracao):
        """Executa extração de serviços em paralelo."""
        semaphore = asyncio.Semaphore(config.max_workers)

        async def extrair_com_limite(servico: TipoServico):
            async with semaphore:
                return await self._extrair_servico(tenant_id, servico, config, resultado)

        tarefas = [extrair_com_limite(servico) for servico in config.servicos]

        await asyncio.gather(*tarefas, return_exceptions=True)

    async def _extrair_sequencial(self, tenant_id: UUID, config: ConfiguracaoExtracao, resultado: ResultadoExtracao):
    ...
    async def _extrair_servico(
        self, tenant_id: UUID, servico: TipoServico, config: ConfiguracaoExtracao, resultado: ResultadoExtracao
    ) -> ResultadoServico:
        ...
        try:
            ...
            if extrator is None:
                ...
                resultado_servico = await self._extrair_generico(tenant_id, servico, config)
            else:
    ...
    async def _extrair_generico(
        self, tenant_id: UUID, servico: TipoServico, config: ConfiguracaoExtracao
    ) -> ResultadoServico:
        ...
        if servico == TipoServico.SEFAZ_NFE:
            ...
            resultado = await extrator.extrair(
                tenant_id, config.data_inicio, config.data_fim, config.cnpjs, config.ufs, config.modo_incremental
            )
        ...
        # Trabalhista
            ...
            resultado = await extrator.extrair(
                tenant_id, config.data_inicio, config.data_fim, config.cnpjs, config.modo_incremental
            )
    ...
    async def _registrar_conclusao(self, resultado: ResultadoExtracao, usuario_id: UUID | None):
        ...
        await self.events.publicar(
            EventoSincronizacao(
                tipo_evento=TipoEventoSistema.SYNC_CONCLUIDA
                if resultado.status == StatusExtracao.CONCLUIDA
    ...
    def _mapear_tipo_credencial(self, servico: TipoServico) -> TipoCredencial | None:
    ...
    async def obter_status_extracao(self, extracao_id: UUID) -> dict | None:
...
# Instância singleton
_orchestrator_instance: OrquestradorExtracao | None = None
