...
async def upload_certificate(
    ...
    alias: Optional[str] = Form(None, description="Alias/nome identificador do certificado"),
    ...
    if ext not in ['pfx', 'p12']:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Formato invalido. Use .pfx ou .p12"
        )
    ...
    try:
        ...
        if not cert_manager.load():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Falha ao carregar certificado. Verifique a senha."
            )
        ...
        if not is_valid:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Certificado invalido: {validation_msg}"
            )
        ...
        logger.info(
            "Certificado carregado: %s (CPF/CNPJ: %s)",
...
async def list_certificates(
    ...
    for alias, cert_manager in store.list_all():
        ...
        certificates.append(CertificateInfoResponse(
...
async def get_certificate(
    ...
    if not cert_manager:
        raise HTTPException(
            ...
            detail=f"Certificado '{certificate_id}' nao encontrado"
        )
...
async def delete_certificate(
    ...
    return CertificateDeleteResponse(
        success=True,
        message=f"Certificado '{certificate_id}' removido com sucesso"
    )
...
async def get_public_key(
    ...
    try:
        ...
        return {
            "certificate_id": certificate_id,
            "public_certificate": cert_base64,
            "format": "X509 DER Base64"
        }

    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao extrair chave publica: {str(e)}"
        )
...
async def test_signature(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erro ao testar assinatura: {str(e)}"
        )
