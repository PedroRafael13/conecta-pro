...
class IntelligentSchedulerService:
    ...
    async def suggest_optimal_time(self, 
                                  event_requirements: Dict[str, Any],
                                  preferred_date_range: Tuple[datetime, datetime],
                                  duration_minutes: int) -> SchedulingSuggestion:
        """
    ...
    async def _generate_candidate_slots(self, 
                                       date_range: Tuple[datetime, datetime],
                                       duration_minutes: int,
                                       required_resources: List[str]) -> List[Tuple[datetime, datetime]]:
    ...
    async def _check_resources_availability(self, 
                                          start_time: datetime,
                                          end_time: datetime,
        ...
        for resource_id in resource_ids:
            ...
            if (start_time.time() < available_start or 
                end_time.time() > available_end):
                return False
            
            # Verifica conflitos com eventos existentes
            for event in self.events.values():
                if (resource_id in event.resource_ids and 
                    not (end_time <= event.start_time or start_time >= event.end_time)):
                    return False
        
        return True
    
    async def _calculate_slot_score(self, 
                                   start_time: datetime,
                                   end_time: datetime,
                                   required_resources: List[str],
                                   required_skills: List[str],
                                   priority: EventPriority) -> float:
...
if forbidden_hours in conditions:                    forbidden_period = conditions[forbidden_hours]                    # Converter tuplas (hora, minuto) para float                    forbidden_start = forbidden_period[0][0] + forbidden_period[0][1] / 60                    forbidden_end = forbidden_period[1][0] + forbidden_period[1][1] / 60                    event_start_hour = start_time.hour + start_time.minute / 60                    event_end_hour = end_time.hour + end_time.minute / 60                                        # Verifica overlap com horário proibido                    if not (event_end_hour <= forbidden_start or event_start_hour >= forbidden_end):
    ...
    async def _calculate_pattern_bonus(self, 
                                      start_time: datetime,
                                      end_time: datetime,
                                      required_resources: List[str]) -> float:
    ...
    async def _allocate_resources(self, 
                                 ...
                                 end_time: datetime,
                                 required_resources: List[str],
                                 required_skills: List[str]) -> Dict[str, str]:
        ...
        if not allocation and required_skills:
            for skill in required_skills:
                suitable_resources = [
                    ...
                    if skill in r.skills and 
                    await self._check_resources_availability(start_time, end_time, [r.id])
                ]
    ...
    async def _estimate_conflicts(self, 
                                 start_time: datetime,
                                 end_time: datetime,
                                 required_resources: List[str]) -> int:
    ...
    async def _generate_reasoning(self, 
                                 best_slot: Tuple[datetime, datetime, float],
                                 resource_allocation: Dict[str, str],
                                 conflicts: int) -> str:
    ...
    async def auto_schedule_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        preferred_start = datetime.fromisoformat(event_data.get("preferred_start")) if event_data.get("preferred_start") else datetime.now()
        preferred_end = datetime.fromisoformat(event_data.get("preferred_end")) if event_data.get("preferred_end") else preferred_start + timedelta(days=7)
        ...
        if suggestion.confidence_score >= 0.7:
            ...
            return {
                ...
                "alternative_times": [
                    ...
                    for start, end in suggestion.alternative_times
                ]
            }
    ...
    async def _balance_workload(self, events: List[CalendarEvent]) -> Dict[str, int]:
        ...
        return {
            ...
            "resources_rebalanced": improvements
        }
    ...
    async def _calculate_utilization_improvement(self, events: List[CalendarEvent], date_range: Tuple[datetime, datetime]) -> float:
        """Calcula melhoria na utilização de recursos."""
