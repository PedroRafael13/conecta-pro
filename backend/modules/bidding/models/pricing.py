...
class BiddingPricing(Base):
    ...
    assessment_id = Column(UUID(as_uuid=True), ForeignKey("bidding_assessments.id"), nullable=True, index=True)
    ...
    # Cenarios
    cenario = Column(String(50), nullable=True)
    cenarios_completos = Column(JSONB, nullable=True)
