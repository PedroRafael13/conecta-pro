...
async def get_expiring(dias: int = Query(default=30, ge=1, le=90), db: Session = Depends(get_db)):
...
async def get_by_tipo(tipo: str, db: Session = Depends(get_db)):
...
async def update_document(document_id: UUID, data: CompanyDocumentUpdate, db: Session = Depends(get_db)):
    """Atualiza documento."""
    service = DocumentService(db)
