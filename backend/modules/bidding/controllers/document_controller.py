...
async def get_expiring(
    current_user: CurrentActiveUser, dias: int = Query(default=30, ge=1, le=90), db: Session = Depends(get_db)
):
...
@router.get("/status")
async def get_status_geral(current_user: CurrentActiveUser, db: Session = Depends(get_db)):
...
async def verificar_habilitacao(current_user: CurrentActiveUser, db: Session = Depends(get_db)):
...
@router.get("/tipos")
async def get_tipos_documento(current_user: CurrentActiveUser, db: Session = Depends(get_db)):
...
async def get_by_tipo(tipo: str, current_user: CurrentActiveUser, db: Session = Depends(get_db)):
    """Busca documento mais recente por tipo."""
    service = DocumentService(db)
    doc = await service.get_by_tipo(tipo)
    if not doc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Documento do tipo {tipo} nao encontrado")
    return doc


@router.get("/{document_id}", response_model=CompanyDocumentResponse)
async def get_document(document_id: UUID, current_user: CurrentActiveUser, db: Session = Depends(get_db)):
...
async def create_document(data: CompanyDocumentCreate, current_user: CurrentActiveUser, db: Session = Depends(get_db)):
    """Cria novo documento."""
    service = DocumentService(db)
...
async def update_document(
    document_id: UUID, data: CompanyDocumentUpdate, current_user: CurrentActiveUser, db: Session = Depends(get_db)
):
    """Atualiza documento."""
    service = DocumentService(db)
...
async def delete_document(document_id: UUID, current_user: CurrentActiveUser, db: Session = Depends(get_db)):
    """Remove documento."""
    service = DocumentService(db)
