...
async def list_vigentes(db: Session = Depends(get_db)):
...
async def list_vencendo(dias: int = Query(default=90, ge=1, le=365), db: Session = Depends(get_db)):
...
async def update_contract(contract_id: UUID, data: PublicContractUpdate, db: Session = Depends(get_db)):
    """Atualiza contrato."""
    service = ContractService(db)
...
@router.post("/{contract_id}/aditivo", response_model=PublicContractResponse)
async def add_aditivo(contract_id: UUID, data: ContractAddendumCreate, db: Session = Depends(get_db)):
...
async def calcular_reajuste(contract_id: UUID, data: ContractReadjustRequest, db: Session = Depends(get_db)):
...
async def aplicar_reajuste(
    contract_id: UUID, percentual: float, data_aplicacao: date | None = None, db: Session = Depends(get_db)
):
...
async def list_medicoes(contract_id: UUID, db: Session = Depends(get_db)):
    """Lista medicoes de um contrato."""
    service = ContractService(db)
    return await service.get_measurements(contract_id)


@router.post("/{contract_id}/medicoes")
