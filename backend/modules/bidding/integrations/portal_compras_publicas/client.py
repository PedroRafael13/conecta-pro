"""
Cliente para Portal de Compras Publicas
========================================
Portal de licitacoes eletronicas para orgaos publicos.
https://www.portaldecompraspublicas.com.br

O Portal de Compras Publicas e amplamente utilizado por municipios
e orgaos publicos para pregoes eletronicos e dispensas.
Como nao dispoe de API publica documentada, utiliza scraping HTTP.
"""
...
class PortalComprasPublicasClient:
    """
    Cliente para o Portal de Compras Publicas.

    Plataforma de licitacoes eletronicas utilizada por
    orgaos publicos municipais e estaduais em todo o Brasil.
    ...
    # Modalidades disponiveis no portal
    MODALIDADES = {
        "todos": "",
        "pregao_eletronico": "1",
        "dispensa": "2",
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if response.status_code == 429:
                    ...
                    logger.warning(
                        "Portal Compras Publicas rate limit, aguardando %.1fs (tentativa %d/%d)",
                        wait,
                        attempt + 1,
                        self.MAX_RETRIES,
                    )
                ...
                last_exc = e
                wait = self.BACKOFF_BASE * (2**attempt)
                logger.warning("Portal Compras Publicas erro de conexao, retry em %.1fs: %s", wait, e)
                await asyncio.sleep(wait)
        raise last_exc or httpx.ConnectError("Max retries exceeded for Portal de Compras Publicas")
    ...
    def _to_opportunity_dict(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "portal": "portal_compras_publicas",
    ...
    def _resolve_modalidade(self, modalidade: str | None) -> str:
        """Resolve nome de modalidade para codigo do portal."""
        if not modalidade:
            return self.MODALIDADES["todos"]
        return self.MODALIDADES.get(modalidade.lower(), modalidade)

    # ---- Metodos publicos ----

    async def health_check(self) -> dict[str, Any]:
        """Verifica disponibilidade do Portal de Compras Publicas."""
        try:
    ...
    async def buscar_editais(
        ...
        """
        Busca editais no Portal de Compras Publicas.

        Args:
            modalidade: Modalidade de licitacao (pregao_eletronico, dispensa, etc.)
            uf: UF para filtrar
            data_inicial: Data a partir da qual buscar
            data_final: Data limite para busca
            pagina: Pagina de resultados

        Returns:
            Lista de dicts compativeis com BiddingOpportunity
        """
        mod_code = self._resolve_modalidade(modalidade)
        ...
        if data_final:
            form_data["dt_fim"] = data_final.strftime("%d/%m/%Y")

        try:
            response = await self._request_with_retry(
                "POST",
                f"{self.BASE_URL}/processos/consultar",
                data=form_data,
            )
            if response.status_code == 200:
                return self._parse_html_resultados(response.text, uf)
        except Exception as e:
            logger.info("POST consulta Portal Compras Publicas falhou: %s", e)

        # Fallback: GET na pagina de pesquisa
        try:
            params: dict[str, str] = {"uf": uf}
            if mod_code:
                params["modalidade"] = mod_code
            if data_inicial:
            ...
            response = await self._request_with_retry(
                "GET",
                f"{self.BASE_URL}/processos/pesquisar",
                params=params,
            )
            if response.status_code == 200:
                return self._parse_html_resultados(response.text, uf)
        except Exception as e:
            logger.error("Erro scraping Portal Compras Publicas: %s", e)

        return []

    def _parse_html_resultados(self, html: str, uf: str = "") -> list[dict[str, Any]]:
        """Extrai licitacoes do HTML de resultados do Portal de Compras Publicas."""
        results: list[dict[str, Any]] = []

        # Padrao 1: tabela de resultados
        row_pattern = re.compile(
            r'<tr[^>]*class="[^"]*(?:linha|result|item|processo)[^"]*"[^>]*>.*?</tr>',
            re.DOTALL | re.IGNORECASE,
        )
        td_pattern = re.compile(r"<td[^>]*>(.*?)</td>", re.DOTALL | re.IGNORECASE)
        link_pattern = re.compile(
            r'href=["\']([^"\']*(?:detalhar|edital|processo|licitacao)[^"\']*)["\']',
            re.IGNORECASE,
        )
        id_pattern = re.compile(r"(?:idProcesso|idLicitacao|id)=(\d+)", re.IGNORECASE)
        ...
        if not results:
            card_pattern = re.compile(
                r"(?:Licita[çc][aã]o|Preg[aã]o|Edital|Processo)\s*(?:N[º°.]?\s*)?([\d/.]+).*?"
    ...
    async def get_edital(self, edital_id: str) -> dict[str, Any] | None:
        ...
        try:
            response = await self._request_with_retry(
                "POST",
                f"{self.BASE_URL}/processos/detalhar",
                data={"idProcesso": edital_id},
            )
            if response.status_code == 200:
                return self._parse_detalhe_edital(response.text, edital_id)
        except Exception as e:
            logger.info("POST detalhe Portal Compras Publicas falhou: %s", e)
        ...
        try:
            response = await self._request_with_retry(
                "GET",
                f"{self.BASE_URL}/processos/detalhar",
                params={"idProcesso": edital_id},
            )
            if response.status_code == 200:
                return self._parse_detalhe_edital(response.text, edital_id)
        except Exception as e:
            logger.error("Erro ao buscar edital Portal Compras Publicas %s: %s", edital_id, e)
    ...
    def _parse_detalhe_edital(self, html: str, edital_id: str) -> dict[str, Any] | None:
        ...
        field_patterns = {
            ...
            "orgao": r"(?:Entidade|[OÓ]rg[aã]o|Comprador)\s*:?\s*</(?:th|td|label|span|strong|b)>\s*<(?:td|span|div|p)[^>]*>(.*?)</",
            ...
            "orgao_cnpj": r"(?:CNPJ)\s*:?\s*</(?:th|td|label|span|strong|b)>\s*<(?:td|span|div|p)[^>]*>(.*?)</",
        }
        ...
        publicacao_match = re.search(
            r"(?:Data|Publicacao).*?(?:Publicacao|Divulgacao).*?:\s*(\d{2}/\d{2}/\d{4}(?:\s+\d{2}:\d{2})?)",
            html,
            re.IGNORECASE,
        )
        if publicacao_match:
            raw["data_publicacao"] = self._parse_date(publicacao_match.group(1))
    ...
    async def buscar_por_objeto(self, termo: str, uf: str = "AM", pagina: int = 1) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Busca por objeto Portal Compras Publicas falhou: %s", e)
        ...
        try:
            ...
            logger.error("Erro fallback busca por objeto Portal Compras Publicas: %s", e)
    ...
    async def buscar_oportunidades(self, **filters) -> list[dict[str, Any]]:
        ...
        Filtros aceitos:
            uf, modalidade, data_inicial, data_final, termo, pagina
        """
    ...
    async def get_detalhes(self, licitacao_id: str) -> dict[str, Any] | None:
        """
        Busca detalhes de uma licitacao especifica.

        Alias para get_edital, mantendo interface padronizada.

        Args:
            licitacao_id: Identificador da licitacao no portal
        """
        return await self.get_edital(licitacao_id)
