"""
Cliente para BLL Compras
========================
Portal de licitacoes eletronicas BLL Compras.
https://bllcompras.com

O portal BLL Compras e amplamente utilizado por municipios
...
class BLLClient:
    """
    Cliente para o portal BLL Compras.
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if response.status_code == 429:
                    ...
                    logger.warning(
                        "BLL Compras rate limit, aguardando %.1fs (tentativa %d/%d)",
                        wait,
                        attempt + 1,
                        self.MAX_RETRIES,
                    )
                ...
                last_exc = e
                wait = self.BACKOFF_BASE * (2**attempt)
                logger.warning("BLL Compras erro de conexao, retry em %.1fs: %s", wait, e)
                await asyncio.sleep(wait)
        raise last_exc or httpx.ConnectError("Max retries exceeded for BLL Compras")
    ...
    async def health_check(self) -> dict[str, Any]:
        """Verifica disponibilidade do portal BLL Compras."""
        try:
    ...
    async def buscar_editais(
        ...
        """
        Busca editais no portal BLL Compras.
        ...
        try:
            ...
            logger.info("POST consulta BLL Compras falhou: %s", e)
        ...
        try:
            ...
            logger.error("Erro scraping BLL Compras: %s", e)

        return []

    def _parse_html_resultados(self, html: str, uf: str = "") -> list[dict[str, Any]]:
        """Extrai licitacoes do HTML de resultados do BLL Compras."""
    ...
    async def get_edital(self, edital_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.info("POST detalhe BLL Compras falhou: %s", e)
        ...
        try:
            ...
            logger.error("Erro ao buscar edital BLL %s: %s", edital_id, e)
    ...
    async def buscar_por_objeto(self, termo: str, uf: str = "AM", pagina: int = 1) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info("Busca por objeto BLL Compras falhou: %s", e)
        ...
        try:
            ...
            logger.error("Erro fallback busca por objeto BLL: %s", e)
