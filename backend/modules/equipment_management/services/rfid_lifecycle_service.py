...
class RFIDLifecycleService:
    ...
    def _initialize_demo_data(self):
        ...
        for i, eq_data in enumerate(demo_equipment, 1):
            ...
            rfid_code = f"RFID_{random.randint(100000, 999999):06d}"
            ...
            rfid_tag = RFIDTag(
                ...
                last_scan_datetime=datetime.now() - timedelta(hours=random.randint(1, 72)),
            ...
            location = EquipmentLocation(
                ...
                coordinates=(random.uniform(-23.0, -22.0), random.uniform(-46.0, -45.0)),
                last_updated=datetime.now()
            )
            
            # Calcular depreciação
            purchase_date = datetime.now() - timedelta(days=random.randint(365, 1095))  # 1-3 anos
            ...
            equipment = Equipment(
                ...
                next_maintenance_date=datetime.now() + timedelta(days=random.randint(30, 120)),
                usage_hours=random.uniform(5000, 15000),
                condition_score=random.uniform(70, 95),
                replacement_cost_current=eq_data['purchase_price'] * random.uniform(1.05, 1.25)  # inflação
            )
    ...
    async def get_equipment_by_rfid(self, rfid_code: str) -> Optional[Equipment]:
    ...
    async def get_equipment_location(self, equipment_id: str) -> Optional[EquipmentLocation]:
    ...
    async def _create_condition_alert(self, equipment: Equipment, condition_score: float):
        ...
        alert = LifecycleAlert(
            ...
            estimated_cost=equipment.replacement_cost_current if alert_type == AlertType.SUBSTITUICAO_RECOMENDADA else equipment.replacement_cost_current * 0.2,
            recommended_action=f"Substituir equipamento" if condition_score < 40 else "Agendar manutenção corretiva"
        )
    ...
    async def calculate_depreciation(self, equipment_id: str) -> bool:
        ...
        if equipment.depreciation.method == "linear":
            annual_depreciation = (equipment.purchase_price - equipment.depreciation.salvage_value) / equipment.depreciation.useful_life_years
            accumulated_depreciation = min(annual_depreciation * age_years, equipment.purchase_price - equipment.depreciation.salvage_value)
        else:
    ...
    async def add_maintenance_record(self, equipment_id: str, maintenance_type: str, 
                                   description: str, cost: float, technician: str, 
                                   parts_used: List[str] = None) -> bool:
    ...
    async def get_active_alerts(self, severity_filter: Optional[str] = None) -> List[LifecycleAlert]:
    ...
    async def generate_inventory_report(self) -> InventoryReport:
        ...
        replacement_recommendations = len([a for a in active_alerts if a.alert_type == AlertType.SUBSTITUICAO_RECOMENDADA])
    ...
    async def get_lifecycle_analytics(self) -> LifecycleAnalytics:
        ...
        inventory_turnover_ratio = random.uniform(0.8, 1.2)
        ...
        avg_utilization = sum(min(100, eq.usage_hours / (8760 * 3)) * 100 for eq in self.equipment.values()) / len(self.equipment)  # 3 anos = 8760*3 horas
    ...
    async def get_scan_history(self, equipment_id: Optional[str] = None, 
                             last_n_records: int = 50) -> List[Dict[str, Any]]:
    ...
    async def find_equipment(self, search_term: str) -> List[Equipment]:
        ...
        for equipment in self.equipment.values():
            if (search_term in equipment.name.lower() or
                search_term in equipment.model.lower() or
                search_term in equipment.manufacturer.lower() or
                search_term in equipment.current_location.room.lower() or
                search_term in equipment.current_location.building.lower() or
                search_term in equipment.serial_number.lower()):
    ...
    async def get_replacement_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for equipment in self.equipment.values():
            ...
            age_factor = min(1.0, (datetime.now() - equipment.purchase_date).days / (equipment.expected_lifespan_years * 365))
            ...
            if replacement_score > 0.6:  # Limiar para recomendação
                ...
                if age_factor > 0.8:
                    recommendation["reasons"].append(f"Idade avançada ({(datetime.now() - equipment.purchase_date).days / 365:.1f} anos)")
