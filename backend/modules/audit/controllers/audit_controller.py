...
@router.post(
    "/logs/{log_id}/review",
    response_model=AuditLogResponse,
    summary="Completa revisão de log"
)
async def complete_log_review(
    ...
    try:
        log = await service.complete_review(
            log_id=log_id,
...
@router.get(
    ...
    summary="Lista regras de compliance"
)
...
async def update_compliance_rule(
    ...
    try:
        rule = await service.update_compliance_rule(
            rule_id=rule_id,
            data=data,
            user_id=current_user.get("id")
        )
...
@router.get(
    "/checks/{check_id}",
    response_model=ComplianceCheckResponse,
    summary="Busca verificação por ID"
)
...
@router.post(
    "/checks/{check_id}/start",
    response_model=ComplianceCheckResponse,
    summary="Inicia verificação"
)
async def start_compliance_check(
    ...
    try:
        check = await service.start_check(
            check_id=check_id,
...
@router.post(
    "/checks/{check_id}/complete/compliant",
    response_model=ComplianceCheckResponse,
    summary="Marca como conforme"
)
async def complete_check_compliant(
    ...
    try:
        check = await service.complete_check_compliant(
            check_id=check_id,
            evidence=evidence,
            notes=notes
        )
...
async def create_data_retention(
    ...
    try:
        policy = await service.create_data_retention(
            data=data,
            user_id=current_user.get("id")
        )
...
@router.get(
    "/retention",
    response_model=DataRetentionList,
    summary="Lista políticas de retenção"
)
async def list_data_retention_policies(
    data_category: Optional[str] = None,
    retention_status: Optional[str] = Query(None, alias="status"),
...
async def get_data_retention(
    ...
    if not policy:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Política não encontrada"
        )
    return DataRetentionResponse.model_validate(policy)
...
async def update_data_retention(
    ...
    try:
        policy = await service.update_data_retention(
            policy_id=policy_id,
            data=data,
            user_id=current_user.get("id")
        )
...
@router.post(
    "/retention/{policy_id}/execute",
    response_model=DataRetentionExecution,
    summary="Executa política"
)
...
@router.post(
    "/access",
    response_model=AccessHistoryResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Registra acesso"
)
...
@router.get(
    "/access",
    response_model=AccessHistoryList,
    summary="Lista histórico de acessos"
)
...
@router.get(
    "/access/stats",
    response_model=AccessHistoryStats,
    summary="Estatísticas de acessos"
)
...
@router.get(
    "/access/user/{user_id}",
    response_model=List[AccessHistoryResponse],
    summary="Histórico de acessos do usuário"
)
...
@router.get(
    "/dashboard",
    response_model=AuditDashboard,
    summary="Dashboard de auditoria"
)
...
@router.get(
    ...
    summary="Visão geral de compliance"
)
...
@router.get(
    ...
    summary="Visão geral de segurança"
)
