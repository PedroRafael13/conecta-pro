...
from modules.guardian.models.campo_tecnico import CampoTecnico
...
async def list_technicians(
    ...
    try:
        ...
        # Aplicar filtros
        if status:
            query = query.where(CampoTecnico.status == status)
        if specialty:
        ...
        if status:
            count_query = count_query.where(CampoTecnico.status == status)
        if specialty:
