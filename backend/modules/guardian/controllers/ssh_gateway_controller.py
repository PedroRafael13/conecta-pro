...
async def create_ssh_connection(request: SSHConnectionRequest):
    ...
    try:
        ...
        logger.info(f"Iniciando conexão SSH {session_id} para {request.username}@{request.host}:{request.port}")
...
@router.get("/sessions")
async def list_ssh_sessions(status: Optional[str] = None, limit: int = 10):
    """
