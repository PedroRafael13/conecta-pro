...
def log_campo_operation(operation: str, technician_id: Optional[str] = None, success: bool = True, **fields):
    """
...
def log_database_operation(operation: str, table: str, duration_ms: float, success: bool = True, **fields):
    ...
    database_logger.info(
        f"DB {operation} on {table} - {duration_ms}ms",
        operation=operation,
        table=table,
...
"""
# Logger específico
logger = get_guardian_logger("campo")
logger.info("Técnico criado", technician_id="123", name="João")

# Logs convenientes
log_campo_operation("create_technician", technician_id="123", name="João Silva")
log_api_operation("/api/v1/campo/technicians", "POST", 201, 89.5, user_id="admin")

# Log de segurança
security_logger.security("unauthorized_access", "high", "Tentativa de acesso negada", ip="192.168.1.100")
"""