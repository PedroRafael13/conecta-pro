...
async def list_templates(
    ...
    templates, total = await service.list_templates(
        tenant_id, category, status, search, page, page_size
    )
...
async def list_schedules(
    ...
    schedules, total = await service.list_schedules(
        tenant_id, template_id, schedule_status, page, page_size
    )
...
@router.post("/exports", response_model=ReportExportResponse)
...
async def list_exports(
    ...
    exports, total = await service.list_exports(
        tenant_id, template_id, schedule_id, export_status,
...
async def record_download(
    ...
    if not success:
        raise HTTPException(
            status_code=400,
            detail="Não foi possível registrar o download"
        )
...
@router.post("/kpis", response_model=ExecutiveKPIResponse)
...
async def list_kpis(
    category: Optional[str] = None,
    kpi_status: Optional[str] = Query(None, alias="status"),
    ...
    kpis, total = await service.list_kpis(
        tenant_id, category, kpi_status, alert_level,
        visible_on_dashboard, page, page_size
    )
...
async def list_benchmarks(
    ...
    benchmark_status: Optional[str] = Query(None, alias="status"),
    visible_on_dashboard: Optional[bool] = None,
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    service: ReportService = Depends(get_service),
    ...
    benchmarks, total = await service.list_benchmarks(
        ...
        benchmark_status, visible_on_dashboard, page, page_size
    )
