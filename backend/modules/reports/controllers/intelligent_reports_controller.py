...
async def generate_intelligent_report(
    ...
    departments: Optional[List[str]] = Query(None, description="Departamentos (opcional)"),
    categories: Optional[List[str]] = Query(None, description="Categorias (opcional)"),
...
@router.get("/recent",
            summary="Relatórios Recentes",
            description="Lista relatórios gerados recentemente")
async def get_recent_reports(
    limit: int = Query(10, description="Limite de relatórios (max 50)")
) -> Dict[str, Any]:
...
async def get_insights_summary(
    days: int = Query(7, description="Número de dias para análise (1-30)")
) -> Dict[str, Any]:
...
@router.post("/export/{report_id}",
             ...
             description="Exporta relatório para diferentes formatos")
...
@router.get("/health",
            summary="Health Check Reports",
            description="Verifica saúde do sistema de relatórios inteligentes")
