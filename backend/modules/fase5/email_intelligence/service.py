...
class EmailIntelligenceService:
    ...
    def _setup_keywords(self) -> None:
        ...
        self.category_keywords = {
            EmailCategory.PROPOSTA_COMERCIAL: [
                "proposta", "orcamento", "cotacao", "preco", "valores",
                "servicos", "contrato", "negociacao"
            ],
            EmailCategory.SOLICITACAO_ORCAMENTO: [
                "orcamento", "cotacao", "quanto custa", "valor",
                "precisa de", "gostaria de contratar"
            ],
            EmailCategory.RECLAMACAO: [
                "reclamacao", "insatisfeito", "problema", "nao funciona",
                "pessimo", "horrivel", "cancelar", "nunca mais"
            ],
            EmailCategory.LICITACAO: [
                "licitacao", "pregao", "edital", "proposta comercial",
                "certame", "habilitacao", "documentacao"
            ],
            EmailCategory.RH: [
                "funcionario", "contratacao", "admissao", "demissao",
                "ferias", "salario", "beneficio", "atestado"
            ],
            EmailCategory.FINANCEIRO: [
                "pagamento", "boleto", "nota fiscal", "fatura",
                "cobranca", "inadimplencia", "parcelamento"
            ],
        }

        self.intent_keywords = {
            EmailIntent.SOLICITAR_PROPOSTA: [
                "gostaria de receber", "solicito proposta", "envie orcamento",
                "preciso de uma proposta"
            ],
            EmailIntent.FAZER_RECLAMACAO: [
                "reclamo", "insatisfeito", "problema grave", "absurdo",
                "inaceitavel"
            ],
            EmailIntent.RENOVAR_CONTRATO: [
                "renovacao", "prorrogacao", "novo contrato", "continuar"
            ],
            EmailIntent.CANCELAR_SERVICO: [
                "cancelamento", "encerrar", "rescindir", "nao quero mais"
            ],
        }

        self.urgency_keywords = [
            "urgente", "emergencia", "imediato", "agora", "hoje",
            "prazo", "deadline", "critico", "importante"
        ]

        self.negative_keywords = [
            "insatisfeito", "problema", "reclamacao", "pessimo",
            "horrivel", "nao funciona", "cancelar", "advogado",
            "procon", "processo"
        ]

        self.positive_keywords = [
            "obrigado", "excelente", "otimo", "satisfeito",
            "parabens", "recomendo", "gostei", "perfeito"
        ]
    ...
    async def analyze_email(self, email: EmailMessage) -> EmailAnalysis:
        ...
        logger.info(
            f"Email analyzed: category={category.value}, "
            f"priority={priority.value}, intent={intent.value}"
        )
    ...
    def _identify_intent(
        self, text: str
    ) -> Tuple[EmailIntent, List[EmailIntent], Decimal]:
    ...
    async def process_incoming_email(
        ...
        # Obter contexto
        context = await self.get_email_context(
