...
class BaseMCPServer(ABC):
    ...
    def __init__(self, config: MCPServerConfig):
        self.config = config
        self.app = FastAPI(title=config.name, description=config.description, version="1.0.0")
        self.tools: dict[str, Callable] = {}
        self.resources: dict[str, MCPResource] = {}
        self.stats = {"requests_handled": 0, "errors": 0, "started_at": None}
        self._setup_routes()
    ...
    def _setup_routes(self) -> None:
        ...
        async def health():
            return {"status": "healthy", "server_type": self.config.server_type.value, "uptime": self._get_uptime()}

        @self.app.get(self.config.metrics_endpoint)
        async def metrics():
            return {"stats": self.stats, "tools_count": len(self.tools), "resources_count": len(self.resources)}

        @self.app.get("/tools")
        async def list_tools():
            return {
                "tools": [
                    MCPTool(name=name, description=f"Tool: {name}", input_schema={"type": "object"}).model_dump()
        ...
        async def list_resources():
            return {"resources": [r.model_dump() for r in self.resources.values()]}

        @self.app.post("/invoke")
        async def invoke_tool(request: MCPRequest, api_key: str = Header(None, alias="X-API-Key")):
