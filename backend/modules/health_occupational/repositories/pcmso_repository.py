...
class PCMSORepository:
    ...
    def get_exam_by_id(self, exam_id: UUID) -> Optional[MedicalExam]:
        """Busca exame por ID."""
    ...
    def get_aso_by_id(self, aso_id: UUID) -> Optional[ASO]:
        """Busca ASO por ID."""
    ...
    def get_aso_by_numero(self, numero: str) -> Optional[ASO]:
    ...
    def get_expiring_asos(self, days: int = 30) -> List[ASO]:
        ...
        return self.db.query(ASO).filter(
            ASO.ativo == True,
            ASO.cancelado == False,
            ASO.data_vencimento <= limit_date,
            ASO.data_vencimento >= date.today(),
        ).order_by(ASO.data_vencimento).all()
    ...
    def get_complementary_exams(self, exam_principal_id: UUID) -> List[ComplementaryExam]:
        ...
        return self.db.query(ComplementaryExam).filter(
            ComplementaryExam.exame_principal_id == exam_principal_id
        ).all()
    ...
    def get_exam_statistics(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        total = self.db.query(MedicalExam).filter(
        ...
        for tipo in ["admissional", "periodico", "demissional", "retorno_trabalho", "mudanca_funcao"]:
            by_type[tipo] = self.db.query(MedicalExam).filter(
                MedicalExam.data_agendamento >= start_date,
                MedicalExam.data_agendamento <= end_date,
        ...
        for status in ["agendado", "confirmado", "realizado", "cancelado"]:
            by_status[status] = self.db.query(MedicalExam).filter(
                MedicalExam.data_agendamento >= start_date,
                MedicalExam.data_agendamento <= end_date,
