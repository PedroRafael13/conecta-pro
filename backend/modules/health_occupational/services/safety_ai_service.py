...
class SafetyAIService:
    ...
    def __init__(self):
        ...
        self.accident_prediction_model: GradientBoostingRegressor | None = None
    ...
    async def assess_employee_risk(
        ...
        try:
            ...
            features = self._extract_safety_features(work_conditions, health_data, historical_incidents or [])
            ...
            # Gerar recomendações
            recommendations = self._generate_safety_recommendations(risk_level, predicted_accidents, features)
    ...
    def _extract_safety_features(
        self, work_conditions: dict[str, Any], health_data: dict[str, Any], historical_incidents: list[dict[str, Any]]
    ) -> dict[str, float]:
        ...
        try:
            ...
            features["days_since_last_incident"] = float(
                (
                    datetime.now()
                    - datetime.fromisoformat(historical_incidents[0].get("date", datetime.now().isoformat()))
                ).days
            ...
            features["risk_multiplier"] = (
                features["work_height"] * 0.1 + features["chemical_exposure"] * 0.2 + features["machinery_risk"] * 0.15
            )

            features["fatigue_risk"] = min(100, features["fatigue_level"] * (features["work_hours_per_day"] / 8))
    ...
    async def predict_accidents(
        self, workplace_data: dict[str, Any], employee_data: list[dict[str, Any]], prediction_horizon_days: int = 30
    ...
    def _predict_specific_accident(
        ...
        horizon_days: int,
    ) -> AccidentPrediction | None:
        """Prediz acidente específico."""
        try:
            # Calcular probabilidade baseada no tipo de acidente
            probability = self._calculate_accident_probability(accident_type, workplace_data, employee_data)
    ...
    def _calculate_accident_probability(
        self, accident_type: AccidentType, workplace_data: dict[str, Any], employee_data: list[dict[str, Any]]
    ) -> float:
    ...
    def _generate_safety_recommendations(
        self, risk_level: RiskLevel, predicted_accidents: list[AccidentType], features: dict[str, float]
    ) -> list[str]:
        ...
        for accident_type in predicted_accidents:
            if accident_type == AccidentType.QUEDA:
                recommendations.extend(
                    ["🪜 Instalar proteções contra quedas", "👷 Treinamento específico para trabalho em altura"]
                )
            elif accident_type == AccidentType.CORTE:
                recommendations.extend(
                    ["🧤 EPIs de proteção para mãos reforçados", "⚙️ Manutenção preventiva em equipamentos cortantes"]
                )
            elif accident_type == AccidentType.LESAO_REPETITIVA:
                recommendations.extend(["💪 Programa de ginástica laboral", "⏰ Pausas regulares para descanso"])
    ...
    def _identify_urgent_actions(self, risk_level: RiskLevel, features: dict[str, float]) -> list[str]:
    ...
    def _identify_contributing_factors(self, accident_type: AccidentType, workplace_data: dict[str, Any]) -> list[str]:
    ...
    def _generate_prevention_measures(self, accident_type: AccidentType, factors: list[str]) -> list[str]:
    ...
    async def generate_safety_analytics(self, workplace_id: str, period_days: int = 30) -> dict[str, Any]:
        ...
        try:
            ...
            analytics = {
                ...
                "risk_distribution": {"critico": 2, "alto": 5, "medio": 15, "baixo": 25, "muito_baixo": 53},
                "predicted_accidents": {"total_predictions": 8, "high_probability": 3, "prevented_actions": 6},
    ...
    def _calculate_assessment_confidence(self, features: dict[str, float]) -> float:
        ...
        required_features = ["work_height", "age", "experience_years", "health_score", "incidents_last_year"]
    ...
    async def train_models(self, training_data: list[dict[str, Any]]) -> bool:
        ...
        try:
            ...
            x_features, y_target = self._prepare_training_data(training_data)

            if len(x_features) < 10:  # Dados insuficientes
    ...
    def _prepare_training_data(self, training_data: list[dict[str, Any]]) -> tuple[np.ndarray, np.ndarray]:
        ...
        for record in training_data:
            features = self._extract_safety_features(
                record.get("work_conditions", {}), record.get("health_data", {}), record.get("historical_incidents", [])
            )

            x_features.append(list(features.values()))
            # Label: 1 se houve acidente, 0 caso contrário
            y_target.append(1 if record.get("had_accident", False) else 0)
...
async def quick_safety_check(employee_id: str, work_conditions: dict[str, Any]) -> str:
