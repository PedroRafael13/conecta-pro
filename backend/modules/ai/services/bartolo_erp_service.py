...
class BartoloERPService:
    ...
    def _build_intent_patterns(self) -> dict[IntentType, list[str]]:
        ...
        return {
            ...
            IntentType.REPORT: [r"\b(relatório|report)\b", r"\b(sumário|resumo|dashboard)\b"],
            IntentType.ALERT: [r"\b(alerta|aviso|problema|crítico)\b", r"\b(emergência|urgente)\b"],
            IntentType.GREETING: [r"\b(olá|oi|bom dia|boa tarde|boa noite)\b", r"\b(tchau|até|obrigado)\b"],
            IntentType.HELP: [r"\b(ajuda|help|socorro|como usar)\b", r"\b(não entendo|não sei)\b"],
            IntentType.MODULE_INFO: [r"\b(módulo|modulo|sistema)\b", r"\b(funcionalidade|recurso|feature)\b"],
        }
    ...
    def _build_entity_patterns(self) -> dict[EntityType, list[str]]:
        ...
        return {
            ...
            EntityType.PROCESS: [r"\b(processo|workflow|automação)\b", r"\b(aprovação|validação|compliance)\b"],
            EntityType.METRIC: [
    ...
    async def process_message(self, message: str, user_context: dict[str, Any] | None = None) -> ConversationTurn:
        """
    ...
    async def _process_nlu(self, text: str) -> NLUResult:
        ...
        return NLUResult(
            text=text, intent=intent, entities=entities, context=self.context.copy(), timestamp=datetime.now()
        )
    ...
    def _detect_intent(self, text: str) -> Intent:
        ...
        return Intent(type=best_intent, confidence=best_confidence, details={"patterns_matched": best_confidence})
        ...
        for entity_type, patterns in self.entity_patterns.items():
            for pattern in patterns:
                for match in re.finditer(pattern, text, re.IGNORECASE):
                    entities.append(
                        Entity(
                            type=entity_type, value=match.group(), confidence=0.8, start=match.start(), end=match.end()
                        )
                    )
