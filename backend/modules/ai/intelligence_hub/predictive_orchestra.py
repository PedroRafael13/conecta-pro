...
class PredictiveOrchestra:
    ...
    async def _detect_conflicts(self, request: PredictionRequest) -> List[str]:
        ...
        for active_id, active_request in self.active_predictions.items():
            if (active_request.module_name == request.module_name and 
                active_request.data_type == request.data_type and
                active_request.tenant_id == request.tenant_id):
    ...
    async def cleanup_old_predictions(self, days_old: int = 7):
        ...
        old_predictions = [
            pred_id for pred_id, result in self.completed_predictions.items()
