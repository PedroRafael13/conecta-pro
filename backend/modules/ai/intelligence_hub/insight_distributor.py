...
class InsightDistributor:
    ...
    async def subscribe_module(
        self, module_name: str, insight_types: list[InsightType], min_priority: InsightPriority = InsightPriority.NORMAL
    ):
    ...
    async def set_user_preferences(self, user_id: str, preferences: dict[str, Any]):
        ...
        self.user_preferences[user_id] = {**preferences, "updated_at": datetime.now().isoformat()}
    ...
    async def _create_delivery_plan(self, insight: Insight, modules: list[str], users: list[str]) -> dict[str, Any]:
