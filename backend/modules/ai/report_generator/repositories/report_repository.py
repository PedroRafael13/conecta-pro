...
class ReportRepository:
    ...
    def get_template(self, template_id: UUID) -> Optional[AIReportTemplate]:
        """Busca template por ID."""
        return self.db.query(AIReportTemplate).filter(
            AIReportTemplate.id == template_id,
            AIReportTemplate.is_active == True
        ).first()

    def get_template_by_code(self, code: str) -> Optional[AIReportTemplate]:
        """Busca template por código."""
        return self.db.query(AIReportTemplate).filter(
            AIReportTemplate.code == code,
            AIReportTemplate.is_active == True
        ).first()
    ...
    def list_templates(
        ...
        query = self.db.query(AIReportTemplate).filter(AIReportTemplate.is_active == True)

        if organization_id:
            query = query.filter(
                or_(
                    AIReportTemplate.organization_id == organization_id,
                    AIReportTemplate.is_public == True,
                    AIReportTemplate.is_system == True
                )
            )
        if category:
    ...
    def get_most_used_templates(self, limit: int = 10) -> List[AIReportTemplate]:
        ...
        return self.db.query(AIReportTemplate).filter(
            AIReportTemplate.is_active == True,
            AIReportTemplate.status == TemplateStatusEnum.ACTIVE
        ).order_by(desc(AIReportTemplate.usage_count)).limit(limit).all()
    ...
    def get_schedule(self, schedule_id: UUID) -> Optional[AIReportSchedule]:
        """Busca agendamento por ID."""
        return self.db.query(AIReportSchedule).filter(
            AIReportSchedule.id == schedule_id,
            AIReportSchedule.is_active == True
        ).first()
    ...
    def list_schedules(
        ...
        query = self.db.query(AIReportSchedule).filter(AIReportSchedule.is_active == True)
    ...
    def get_due_schedules(self) -> List[AIReportSchedule]:
        ...
        ).order_by(asc(AIReportSchedule.next_run_at)).all()

    def get_failed_schedules(self) -> List[AIReportSchedule]:
        """Busca agendamentos com falha."""
        return self.db.query(AIReportSchedule).filter(
            AIReportSchedule.is_active == True,
            AIReportSchedule.status == ScheduleStatusEnum.FAILED
        ).all()
    ...
    def create_execution(self, execution: ReportExecution) -> ReportExecution:
        ...
        if execution.schedule_id:
            count = self.db.query(ReportExecution).filter(
                ReportExecution.schedule_id == execution.schedule_id
            ).count()
    ...
    def get_execution(self, execution_id: UUID) -> Optional[ReportExecution]:
    ...
    def get_running_executions(self) -> List[ReportExecution]:
        ...
        return self.db.query(ReportExecution).filter(
            ReportExecution.status.in_(running_statuses)
        ).all()
    ...
    def get_section(self, section_id: UUID) -> Optional[ReportSection]:
        """Busca seção por ID."""
        return self.db.query(ReportSection).filter(
            ReportSection.id == section_id,
            ReportSection.is_active == True
        ).first()
    ...
    def get_widget(self, widget_id: UUID) -> Optional[ReportWidget]:
        """Busca widget por ID."""
        return self.db.query(ReportWidget).filter(
            ReportWidget.id == widget_id,
            ReportWidget.is_active == True
        ).first()
    ...
    def list_widgets_by_template(self, template_id: UUID) -> List[ReportWidget]:
        ...
        return self.db.query(ReportWidget).filter(
            ...
            ReportWidget.is_active == True
        ).order_by(asc(ReportWidget.order)).all()
    ...
    def list_reusable_widgets(
        ...
        query = self.db.query(ReportWidget).filter(
            ReportWidget.is_active == True,
            ReportWidget.is_reusable == True
        )
        if organization_id:
            query = query.filter(
                or_(
                    ReportWidget.organization_id == organization_id,
                    ReportWidget.is_system == True
                )
            )
        if widget_type:
    ...
    def get_report_stats(
        ...
        # Total de insights
        total_insights = self.db.query(
            func.sum(func.jsonb_array_length(Report.insights))
        ).filter(
            Report.is_active == True,
            Report.created_at >= start_date
        )
    ...
    def get_schedule_stats(self, organization_id: Optional[UUID] = None) -> Dict[str, Any]:
        ...
        # Total de execuções
        total_runs = self.db.query(func.sum(AIReportSchedule.total_runs)).filter(
            AIReportSchedule.is_active == True
        )
        if organization_id:
            total_runs = total_runs.filter(AIReportSchedule.organization_id == organization_id)

        successful_runs = self.db.query(func.sum(AIReportSchedule.successful_runs)).filter(
            AIReportSchedule.is_active == True
        )
        if organization_id:
        ...
        return {
            ...
            "overall_success_rate": (
                (successful_runs.scalar() or 0) / (total_runs.scalar() or 1) * 100
            ),
        }
    ...
    def get_execution_metrics(
        ...
        query = self.db.query(ReportExecution).filter(
            ReportExecution.created_at >= start_date
        )
        if organization_id:
        ...
        avg_metrics = self.db.query(
            ...
            ReportExecution.status == ExecutionStatusEnum.COMPLETED,
            ReportExecution.created_at >= start_date
        )
        if organization_id:
    ...
    def get_dashboard_data(self, organization_id: Optional[UUID] = None) -> Dict[str, Any]:
