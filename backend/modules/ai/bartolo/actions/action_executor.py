...
from .action_schemas import ActionRequest, ActionPreview, ActionConfirmation, ActionResult
...
class ActionExecutor:
    ...
    async def get_user_info(self, user_id: str) -> Optional[tuple[str, str]]:
        """
        ...
        try:
            ...
            if user_uuid:
                ...
                result = await self.db.execute(
                    text("SELECT id, role FROM users WHERE CAST(id AS TEXT) LIKE :user_id AND is_active = true LIMIT 1"),
                    {"user_id": f"%{user_id}%"}
                )
    ...
    async def execute_action(self, confirmation: ActionConfirmation) -> ActionResult:
        ...
        if datetime.utcnow() - created_at > timedelta(minutes=self.ACTION_TIMEOUT_MINUTES):
            ...
            raise ValueError(f"Ação {confirmation.action_id} expirou (timeout de {self.ACTION_TIMEOUT_MINUTES} minutos)")
        ...
        try:
            ...
            logger.info(
                f"Ação {confirmation.action_id} concluída: "
    ...
    async def _cleanup_expired_actions(self):
        ...
        expired = [
            action_id
            for action_id, (_, created_at) in self._pending_actions.items()
    ...
    async def _audit_action_complete(
        self,
        request: ActionRequest,
        result: ActionResult,
        correlation_id: str
    ...
    async def _audit_action_cancelled(
        self,
        request: ActionRequest,
        confirmation: ActionConfirmation
    ):
