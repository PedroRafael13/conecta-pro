...
class RelatorioAgent:
    ...
    INTENT_PATTERNS = [
        ...
        # ==================================================================
        (
            r"(?:gerar|gere|criar|crie|produzir|produza|montar|monte)\s+(?:um?\s+)?(?:novo\s+)?relat[oó]rio",
            RelatorioIntent.GERAR_RELATORIO,
        ),
        (r"(?:preciso|quero|precis[oa])\s+(?:de\s+)?(?:um\s+)?relat[oó]rio", RelatorioIntent.GERAR_RELATORIO),
        (
            r"(?:pode|consegue|da\s+para)\s+(?:gerar|criar|montar)\s+(?:um?\s+)?relat[oó]rio",
            RelatorioIntent.GERAR_RELATORIO,
        ),
        ...
        (
            r"relat[oó]rio\s+(?:de\s+)?(?:horas?\s+extras?|custos?|banco\s+(?:de\s+)?horas|substituic|disciplinar|ocorr[eê]ncia|diarista|ronda|posto|escala|geral)",
            RelatorioIntent.GERAR_RELATORIO,
        ),
        # ==================================================================
        # EXPORTAR_RELATORIO
        ...
        # ==================================================================
        (
            r"(?:ver|veja|mostrar|mostre|abrir|abra|exibir|exiba)\s+(?:o?\s+)?relat[oó]rio\s+\w+",
            RelatorioIntent.VER_RELATORIO,
        ),
        ...
        # ==================================================================
        (
            r"(?:resumo|overview|panorama|dashboard)\s+(?:operacional|geral|mensal|semanal)",
            RelatorioIntent.RELATORIO_RAPIDO,
        ),
        ...
        # ==================================================================
        (
            r"(?:quais?\s+)?(?:tipos?\s+(?:de\s+)?)?relat[oó]rios?\s+(?:dispon[ií]veis?|existem|tem)",
            RelatorioIntent.TIPOS_RELATORIO,
        ),
        (
            r"(?:tipos?\s+(?:de\s+)?)?relat[oó]rios?\s+(?:dispon[ií]veis?|poss[ií]veis?)",
            RelatorioIntent.TIPOS_RELATORIO,
        ),
        (
            r"(?:o\s+que|quais?)\s+relat[oó]rios?\s+(?:posso|consigo|da\s+para)\s+(?:gerar|criar)",
            RelatorioIntent.TIPOS_RELATORIO,
        ),
        ...
        # ==================================================================
        (
            r"(?:listar|liste|ver|veja|mostrar|mostre)\s+(?:os\s+)?relat[oó]rios?\s+(?:gerados?|recentes?|anteriores?)",
            RelatorioIntent.LISTAR_RELATORIOS,
        ),
        ...
        # ==================================================================
        (
            r"(?:estat[ií]sticas?|stats?|m[eé]tricas?|indicadores?)\s+(?:de\s+|dos?\s+)?relat[oó]rios?",
            RelatorioIntent.ESTATISTICAS,
        ),
    ...
    def _detect_intent(self, message: str) -> RelatorioIntent | None:
        ...
        relatorios_mock = [
            {
                "id": "RPT-001",
                "tipo": "Horas Extras",
                "periodo": f"01-{hoje.strftime('%d')} {hoje.strftime('%b/%Y')}",
                "status": "concluido",
                "formato": "PDF",
                "gerado_em": (hoje - timedelta(days=1)).strftime("%d/%m/%Y"),
            },
            {
                "id": "RPT-002",
                "tipo": "Custos Operacionais",
                "periodo": "Jan/2026",
                "status": "concluido",
                "formato": "XLSX",
                "gerado_em": (hoje - timedelta(days=3)).strftime("%d/%m/%Y"),
            },
            {
                "id": "RPT-003",
                "tipo": "Ocorrencias",
                ...
                "status": "concluido",
                "formato": "PDF",
                "gerado_em": (hoje - timedelta(days=5)).strftime("%d/%m/%Y"),
            },
            {
                "id": "RPT-004",
                "tipo": "Rondas",
                "periodo": f"Semana {hoje.isocalendar()[1]}/2026",
                "status": "concluido",
                "formato": "PDF",
                "gerado_em": (hoje - timedelta(days=2)).strftime("%d/%m/%Y"),
            },
            {
                "id": "RPT-005",
                ...
                "status": "concluido",
                "formato": "PDF",
                "gerado_em": (hoje - timedelta(days=15)).strftime("%d/%m/%Y"),
            },
        ]
...
3. Coordenador Ops: 5 relatorios"""
    ...
    def _detect_report_type(self, message: str) -> str | None:
    ...
    def _extract_period(self, message: str) -> dict[str, str] | None:
    ...
    def _extract_format(self, message: str) -> str | None:
    ...
    def _extract_report_id(self, message: str) -> str | None:
