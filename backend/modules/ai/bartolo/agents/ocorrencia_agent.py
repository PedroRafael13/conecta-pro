...
class OcorrenciaIntent(StrEnum):
...
class OcorrenciaAgent:
    ...
    INTENT_PATTERNS = [
        ...
        # ==================================================================
        (
            r"(?:resolver|resolva|fechar|feche|encerrar|encerre|concluir|conclua)\s+(?:a\s+)?ocorrencia",
            OcorrenciaIntent.RESOLVER_OCORRENCIA,
        ),
        ...
        # ==================================================================
        (
            r"(?:criar|crie|cria|registrar|registre|abrir|abra|nova|novo)\s+(?:uma?\s+)?ocorrencia",
            OcorrenciaIntent.CRIAR_OCORRENCIA,
        ),
        (
            r"(?:registrar|registre)\s+(?:uma?\s+)?(?:infracao|nao\s+conformidade|irregularidade)",
            OcorrenciaIntent.CRIAR_OCORRENCIA,
        ),
        (
            r"(?:abrir|abra)\s+(?:um\s+)?(?:registro|chamado)\s+(?:de\s+)?(?:ocorrencia|infracao)",
            OcorrenciaIntent.CRIAR_OCORRENCIA,
        ),
        (
            r"(?:reportar|reporte)\s+(?:uma?\s+)?(?:ocorrencia|infracao|irregularidade)",
            OcorrenciaIntent.CRIAR_OCORRENCIA,
        ),
        (
            r"(?:funcionario|colaborador|vigilante|porteiro)\s+(?:cometeu|fez|esta|estava)\s+",
            OcorrenciaIntent.CRIAR_OCORRENCIA,
        ),
        # ==================================================================
        ...
        # ==================================================================
        (
            r"(?:detalhe|detalhes|info|informacoes?)\s+(?:da\s+)?ocorrencia\s+OCO-\d{4}-\d+",
            OcorrenciaIntent.OCORRENCIA_DETALHES,
        ),
        (
            r"(?:ver|veja|mostrar|mostre|exibir|exiba)\s+(?:a\s+)?ocorrencia\s+OCO-\d{4}-\d+",
            OcorrenciaIntent.OCORRENCIA_DETALHES,
        ),
        ...
        # ==================================================================
        (
            r"ocorrencias?\s+(?:do|da|de)\s+(?:funcionario|colaborador|vigilante|porteiro|agente)",
            OcorrenciaIntent.OCORRENCIAS_FUNCIONARIO,
        ),
        (
            r"(?:historico|historico)\s+(?:de\s+)?(?:ocorrencias?|infracoes?)\s+(?:do|da|de)",
            OcorrenciaIntent.OCORRENCIAS_FUNCIONARIO,
        ),
        (
            r"(?:infracoes?|irregularidades?)\s+(?:do|da|de)\s+(?:funcionario|colaborador)",
            OcorrenciaIntent.OCORRENCIAS_FUNCIONARIO,
        ),
        ...
        # ==================================================================
        (
            r"(?:estatisticas?|stats?|numeros?|indicadores?|dashboard)\s+(?:de\s+)?ocorrencias?",
            OcorrenciaIntent.ESTATISTICAS_OCORRENCIAS,
        ),
        (
            r"(?:resumo|panorama|visao\s+geral)\s+(?:de\s+|das\s+)?ocorrencias?",
            OcorrenciaIntent.ESTATISTICAS_OCORRENCIAS,
        ),
        ...
        # ==================================================================
        (
            r"(?:ver|veja|mostrar|mostre|exibir|exiba|listar|liste)\s+(?:as\s+)?ocorrencias?(?:\s+abertas?)?",
            OcorrenciaIntent.VER_OCORRENCIAS,
        ),
        ...
        (
            r"(?:infracoes?|irregularidades?|nao\s+conformidades?)\s+(?:abertas?|pendentes?)",
            OcorrenciaIntent.VER_OCORRENCIAS,
        ),
    ...
    def _detect_intent(self, message: str) -> OcorrenciaIntent | None:
        ...
        if self.data_connector and self.db and code:
            try:
                ...
                if occurrence:
                    ...
                    resolved = (
                        occurrence.resolved_at.strftime("%d/%m/%Y %H:%M") if occurrence.resolved_at else "Pendente"
                    )
