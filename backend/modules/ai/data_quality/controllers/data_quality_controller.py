...
async def create_rule(rule_data: DataQualityRuleCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Regra com código '{rule_data.code}' já existe"
        )
...
async def list_rules(
    ...
    repo = DataQualityRepository(db)
    rules, _ = repo.list_rules(entity_type=entity_type, category=category, status=status, skip=skip, limit=limit)
    return rules
...
async def list_checks(
    ...
    """Lista verificações."""
    repo = DataQualityRepository(db)
    checks, _ = repo.list_checks(entity_type=entity_type, status=status, skip=skip, limit=limit)
    return checks
...
async def list_issues(
    ...
    issues, _ = repo.list_issues(
        entity_type=entity_type, entity_id=entity_id, status=status, check_id=check_id, skip=skip, limit=limit
    )
    return issues
...
async def get_issue(issue_id: UUID, db: Session = Depends(get_db)):
    ...
    if not issue:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Issue não encontrado")
    return issue
...
async def list_duplicates(
    ...
    duplicates, _ = repo.list_duplicates(
        entity_type=entity_type, status=status, min_score=min_score, skip=skip, limit=limit
    )
    return duplicates
...
async def get_duplicate(duplicate_id: UUID, db: Session = Depends(get_db)):
    ...
    if not duplicate:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Duplicata não encontrada")
    return duplicate
...
async def merge_duplicates(duplicate_id: UUID, merge_request: DuplicateMergeRequest, db: Session = Depends(get_db)):
    ...
    duplicate.merge(
        master_id=merge_request.master_record_id, merged_id=uuid_lib.uuid4(), strategy=merge_request.strategy
    )
...
async def validate_data(request: ValidationRequest, db: Session = Depends(get_db)):
    ...
    is_valid, issues, fixed_data = validator.validate_record(
        data=request.data, rules=rules, entity_type=request.entity_type
    )
...
async def cleanse_data(request: CleansingRequest):
    ...
    cleaned_data, changes = cleaner.clean_record(
        data=request.data, default_operations=request.operations if request.operations else None
    )

    return CleansingResponse(
        original_data=request.data, cleaned_data=cleaned_data, changes=changes, changes_count=len(changes)
    )
