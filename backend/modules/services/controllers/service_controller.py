...
from modules.services.models import (
    ...
    OrderStatus, OrderPriority, ReportType
)
from modules.services.schemas import (
    ServiceCatalogCreate, ServiceCatalogUpdate,
    ServiceCatalogResponse, ServiceCatalogListResponse, ServiceCatalogStats,
    ServiceOrderCreate, ServiceOrderUpdate,
    ServiceOrderResponse, ServiceOrderListResponse,
    ServiceOrderStats,
    ServiceExecutionCreate, ServiceExecutionResponse,
...
@router.get(
    "/catalog",
    response_model=List[ServiceCatalogListResponse],
...
@router.get(
    "/catalog/stats",
    response_model=ServiceCatalogStats,
...
@router.post(
    "/catalog/{service_id}/discontinue",
    response_model=ServiceCatalogResponse,
...
@router.get(
    "/orders",
    response_model=List[ServiceOrderListResponse],
...
async def get_at_risk_orders(
    threshold_hours: int = Query(4, ge=1, le=24),
    svc: ServiceManagementService = Depends(get_management_service)
) -> list:
...
async def schedule_order(
    order_id: UUID,
    scheduled_date: date,
    time_start: Optional[str] = Query(None, pattern=r"^\d{2}:\d{2}$"),
    time_end: Optional[str] = Query(None, pattern=r"^\d{2}:\d{2}$"),
    technician_id: Optional[UUID] = None,
    technician_name: Optional[str] = None,
    svc: ServiceManagementService = Depends(get_management_service)
) -> ServiceOrderResponse:
    """Agenda uma ordem de serviço."""
    order = svc.schedule_order(
        order_id, scheduled_date, time_start, time_end,
...
@router.get(
    "/executions",
    response_model=List[ServiceExecutionResponse],
...
async def list_executions(
    ...
    executions = repo.list_service_executions(
        order_id=order_id,
        technician_id=technician_id,
        skip=skip,
        limit=limit
...
@router.post(
    "/executions/{execution_id}/start-travel",
    response_model=ServiceExecutionResponse,
...
@router.post(
    "/executions/{execution_id}/arrive",
    response_model=ServiceExecutionResponse,
...
async def finish_execution(
    ...
    """Finaliza a execução."""
    execution = svc.finish_execution(
        execution_id, work_description, findings, recommendations
    )
    if not execution:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def add_material(
    ...
    execution = svc.add_material_to_execution(
        execution_id, material_name, quantity, unit_price
    )
    if not execution:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_checklist(
    ...
    execution = svc.update_checklist_item(
        execution_id, item_index, completed, notes
    )
    if not execution:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def add_signature(
    ...
    execution = svc.add_signature(
        execution_id, signer_name, signature_data, signer_role
    )
    if not execution:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@router.get(
    "/reports",
    response_model=List[ServiceReportResponse],
...
async def review_report(
    ...
    """Revisa um relatório."""
    user_id = UUID(current_user.get("sub")) if current_user.get("sub") else None
    report = svc.review_report(
        report_id, user_id, reviewer_name, review_notes
    )
    if not report:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@router.post(
    "/sla-configs/{sla_id}/set-default",
    response_model=SLAConfigResponse,
...
@router.get(
    "/sla-configs/{sla_id}/compliance-report",
...
@router.get(
    "/analytics/service/{service_id}",
    response_model=ServiceAnalysis,
...
@router.get(
    "/analytics/sla/{sla_id}",
    response_model=SLAAnalysis,
