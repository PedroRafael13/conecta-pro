...
class ServiceOrder:
    ...
    completion_rating: Optional[float]
...
class MobileFieldService:
    ...
    async def optimize_technician_route(self, technician_id: str) -> Optional[RouteOptimization]:
        """
        ...
        assigned_orders = [
            order for order in self.service_orders.values()
            if (order.assigned_technician_id == technician_id and 
                order.status in [ServiceStatus.ASSIGNED, ServiceStatus.IN_ROUTE])
        ]
        ...
        while remaining_orders:
            ...
            nearest_order = min(
                remaining_orders,
                key=lambda order: self._calculate_distance(current_pos, order.location.gps_location)
            )
    ...
    async def update_service_status(self, order_id: str, new_status: ServiceStatus, 
                                  notes: Optional[str] = None, photos: Optional[List[str]] = None) -> bool:
        """
    ...
    async def get_technician_dashboard(self, technician_id: str) -> Dict[str, Any]:
        ...
        assigned_orders = [
            order for order in self.service_orders.values()
            if order.assigned_technician_id == technician_id
        ]
        ...
        todays_orders = [
            ...
            if order.scheduled_start.date() == today
        ]
    ...
    async def get_field_service_analytics(self) -> Dict[str, Any]:
        ...
        total_distance_saved = sum(
            opt.optimization_savings.get("distance_saved_km", 0)
            for opt in self.route_optimizations.values()
        )
        
        total_fuel_saved = sum(
            opt.optimization_savings.get("fuel_saved_cost", 0)
            for opt in self.route_optimizations.values()
        )
