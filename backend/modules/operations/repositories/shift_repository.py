...
from modules.operations.models.shift import Shift, ShiftStatus
from modules.operations.schemas.shift import ShiftCreate, ShiftFilter, ShiftUpdate
...
class ShiftRepository:
    ...
    async def create(self, data: ShiftCreate) -> Shift:
        ...
        shift = Shift(
            ...
            is_sunday=data.shift_date.weekday() == 6,
        ...
        if filters.needs_substitution:
            query = query.where(
                and_(
                    ...
                    Shift.substitution_id.is_(None),
                )
            )
    ...
    async def check_out(
        ...
        if shift.actual_start_time:
            start = datetime.combine(shift.shift_date, shift.actual_start_time)
            end = datetime.combine(shift.shift_date, actual_end_time)
