...
class Post(Base):
    ...
    # Requisitos e Equipamentos (JSON)
    ...
    allows_overtime: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    ...
    def is_filled(self) -> bool:
        ...
        active_allocations = [a for a in self.allocations if a.is_active]
        return len(active_allocations) >= self.headcount

    @property
    def vacancy_count(self) -> int:
        """Retorna quantidade de vagas disponíveis."""
        active_allocations = [a for a in self.allocations if a.is_active]
        return max(0, self.headcount - len(active_allocations))
