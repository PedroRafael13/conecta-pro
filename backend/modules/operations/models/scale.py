...
class Scale(Base):
    ...
    def __repr__(self) -> str:
        return f"<Scale {self.code} - {self.month}/{self.year}>"
    ...
    @property
    def filled_shifts_count(self) -> int:
        """Conta turnos preenchidos (com funcionário alocado)."""
        return len([s for s in self.shifts if s.employee_id is not None])
    ...
    def fill_rate(self) -> float:
        ...
        if self.total_shifts == 0:
            return 0.0
        return (self.filled_shifts_count / self.total_shifts) * 100
