...
class Shift(Base):
    ...
    # Horários reais (preenchido via ponto)
    ...
    actual_break_minutes: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    ...
    # Custos
    base_cost: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    overtime_cost: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    ...
    # Substituição
    original_employee_id: Mapped[Optional[str]] = mapped_column(
        UUID(as_uuid=False),
        nullable=True,
    )
    substitution_id: Mapped[Optional[str]] = mapped_column(
        UUID(as_uuid=False),
        nullable=True,
    )
    ...
    def needs_substitution(self) -> bool:
        """Verifica se precisa de substituto."""
        return self.status == ShiftStatus.MISSED.value and not self.substitution_id
