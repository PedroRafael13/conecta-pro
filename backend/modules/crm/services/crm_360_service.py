...
class CustomerInteraction:
    ...
    satisfaction_score: Optional[float] = None  # 0 to 10
...
class CustomerJourney:
    ...
    next_predicted_stage: Optional[JourneyStage]
...
class CRM360Service:
    ...
    def _initialize_demo_data(self):
        ...
        touchpoints_data = [
            {"name": "Website Landing Page", "category": TouchpointCategory.MARKETING, "channel": "website", "stage": JourneyStage.AWARENESS, "digital": True},
            {"name": "WhatsApp First Contact", "category": TouchpointCategory.SALES, "channel": "whatsapp", "stage": JourneyStage.CONSIDERATION, "digital": True},
            {"name": "Proposta Comercial", "category": TouchpointCategory.SALES, "channel": "email", "stage": JourneyStage.DECISION, "digital": True},
            {"name": "Onboarding Call", "category": TouchpointCategory.SUPPORT, "channel": "phone", "stage": JourneyStage.ONBOARDING, "digital": False},
            {"name": "Portal de Cliente", "category": TouchpointCategory.PRODUCT, "channel": "portal", "stage": JourneyStage.ACTIVE_USE, "digital": True},
            {"name": "App Mobile", "category": TouchpointCategory.PRODUCT, "channel": "mobile_app", "stage": JourneyStage.ACTIVE_USE, "digital": True},
            {"name": "Suporte Técnico", "category": TouchpointCategory.TECHNICAL, "channel": "phone", "stage": JourneyStage.ACTIVE_USE, "digital": False},
            {"name": "Renovação de Contrato", "category": TouchpointCategory.SALES, "channel": "face_to_face", "stage": JourneyStage.RENEWAL, "digital": False},
            {"name": "Programa de Indicação", "category": TouchpointCategory.MARKETING, "channel": "email", "stage": JourneyStage.ADVOCACY, "digital": True}
        ]
        ...
        for i, tp_data in enumerate(touchpoints_data, 1):
            touchpoint = CustomerTouchpoint(
                ...
                importance_score=random.uniform(6.0, 9.5),
                satisfaction_avg=random.uniform(7.0, 9.0),
                conversion_rate=random.uniform(0.15, 0.85),
        ...
        demo_customers = [
            ...
            {"name": "Residencial Parque das Flores", "segment": CustomerSegment.PREMIUM, "stage": CustomerStage.CUSTOMER},
        ...
        for i, cust_data in enumerate(demo_customers, 1):
            ...
            journey_start = datetime.now() - timedelta(days=random.randint(30, 730))
            ...
            journey = CustomerJourney(
                ...
                touchpoints_visited=[tp.id for tp in list(self.touchpoints.values())[:random.randint(3, 7)]],
                interactions_count=random.randint(15, 80),
                satisfaction_avg=random.uniform(7.2, 9.1),
                time_in_stage_days=(datetime.now() - journey_start).days,
                next_predicted_stage=self._predict_next_journey_stage(current_stage),
                stage_progression_probability=random.uniform(0.6, 0.9),
                churn_risk_score=random.uniform(0.1, 0.4),
                lifetime_value_predicted=random.uniform(50000, 300000),
                journey_health_score=random.uniform(75, 95)
            )
            ...
            customer = Customer360(
                ...
                basic_info={
                    ...
                    "location": f"São Paulo - Zona {random.choice(['Sul', 'Norte', 'Oeste', 'Central'])}"
                },
            ...
            await self._generate_customer_interactions(customer_id, random.randint(8, 15))
    ...
    def _predict_next_journey_stage(self, current_stage: JourneyStage) -> Optional[JourneyStage]:
    ...
    def _generate_behavioral_insights(self, segment: CustomerSegment) -> Dict[str, Any]:
        ...
        base_insights = {
            "preferred_contact_time": f"{random.randint(9, 17)}:00-{random.randint(18, 20)}:00",
            "response_time_preference": "immediate" if segment in [CustomerSegment.VIP, CustomerSegment.PREMIUM] else "within_24h",
            "communication_style": "formal" if segment == CustomerSegment.VIP else "friendly",
            "decision_making_speed": "fast" if segment in [CustomerSegment.VIP, CustomerSegment.PREMIUM] else "moderate",
            "price_sensitivity": "low" if segment == CustomerSegment.VIP else "high" if segment == CustomerSegment.BRONZE else "medium",
            "digital_adoption": random.uniform(0.6, 0.95),
            "feature_usage_patterns": {
                "financial_module": random.uniform(0.7, 0.95),
                "maintenance_module": random.uniform(0.5, 0.9),
                "communication_module": random.uniform(0.8, 0.98),
                "reporting_module": random.uniform(0.3, 0.8)
            }
        }
    ...
    def _generate_customer_preferences(self) -> Dict[str, Any]:
        ...
        return {
            "preferred_channels": random.sample(["email", "whatsapp", "phone", "portal"], k=random.randint(2, 4)),
            "notification_frequency": random.choice(["immediate", "daily", "weekly"]),
            "report_format": random.choice(["pdf", "excel", "dashboard"]),
            "language": "pt-br",
    ...
    def _generate_satisfaction_metrics(self) -> Dict[str, float]:
        ...
        return {
            "overall_satisfaction": random.uniform(7.0, 9.5),
            "product_satisfaction": random.uniform(7.5, 9.2),
            "support_satisfaction": random.uniform(6.8, 9.1),
            "onboarding_satisfaction": random.uniform(7.2, 8.9),
            "nps_score": random.uniform(7.0, 9.0)
        }
    ...
    def _generate_financial_metrics(self, segment: CustomerSegment) -> Dict[str, float]:
        ...
        return {
            "monthly_revenue": random.uniform(*values["monthly_revenue"]),
            "lifetime_value": random.uniform(*values["lifetime_value"]),
            "acquisition_cost": random.uniform(500, 2000),
            "payment_score": random.uniform(0.8, 1.0),
            "credit_limit": random.uniform(10000, 50000),
            "outstanding_balance": random.uniform(0, 5000)
        }
    
    def _generate_engagement_metrics(self) -> Dict[str, float]:
        """Gerar métricas de engajamento."""
        return {
            "login_frequency_weekly": random.uniform(3, 15),
            "feature_adoption_rate": random.uniform(0.4, 0.9),
            "support_ticket_frequency": random.uniform(0.2, 2.0),
            "portal_usage_hours_monthly": random.uniform(5, 25),
            "mobile_app_usage_rate": random.uniform(0.3, 0.8),
            "email_open_rate": random.uniform(0.4, 0.85),
            "social_engagement_score": random.uniform(0.1, 0.7)
        }
    
    def _generate_risk_indicators(self) -> Dict[str, float]:
        """Gerar indicadores de risco."""
        return {
            "churn_probability": random.uniform(0.05, 0.35),
            "payment_risk": random.uniform(0.1, 0.4),
            "support_escalation_risk": random.uniform(0.1, 0.3),
            "contract_renewal_risk": random.uniform(0.2, 0.5),
            "satisfaction_decline_risk": random.uniform(0.1, 0.4)
        }
    ...
    def _generate_next_best_actions(self, stage: CustomerStage) -> List[Dict[str, Any]]:
        ...
        actions_map = {
            CustomerStage.LEAD: [
                {"action": "send_welcome_email", "priority": "high", "description": "Enviar email de boas-vindas personalizado"},
                {"action": "schedule_demo", "priority": "medium", "description": "Agendar demonstração do produto"}
            ],
            CustomerStage.PROSPECT: [
                {"action": "send_proposal", "priority": "high", "description": "Enviar proposta comercial personalizada"},
                {"action": "schedule_call", "priority": "medium", "description": "Agendar call de acompanhamento"}
            ],
            CustomerStage.CUSTOMER: [
                {"action": "check_satisfaction", "priority": "medium", "description": "Verificar nível de satisfação atual"},
            ...
            CustomerStage.ADVOCATE: [
                {"action": "testimonial_request", "priority": "high", "description": "Solicitar depoimento/testemunhal"},
    ...
    async def _generate_customer_interactions(self, customer_id: str, count: int):
        ...
        interaction_types = [
            {"type": InteractionType.EMAIL, "direction": InteractionDirection.OUTBOUND, "category": TouchpointCategory.MARKETING},
            {"type": InteractionType.PHONE, "direction": InteractionDirection.INBOUND, "category": TouchpointCategory.SUPPORT},
            {"type": InteractionType.WHATSAPP, "direction": InteractionDirection.INBOUND, "category": TouchpointCategory.SALES},
            {"type": InteractionType.PORTAL, "direction": InteractionDirection.INTERNAL, "category": TouchpointCategory.PRODUCT},
            {"type": InteractionType.MOBILE_APP, "direction": InteractionDirection.INTERNAL, "category": TouchpointCategory.PRODUCT}
        ]
        
        for i in range(count):
            interaction_data = random.choice(interaction_types)
            ...
            interaction = CustomerInteraction(
                ...
                satisfaction_score=random.uniform(6.0, 9.5) if random.random() > 0.3 else None,
                outcome=random.choice(["resolved", "pending", "escalated", "completed"]),
    ...
    async def _generate_segment_profiles(self):
        ...
        for segment, data in segments_data.items():
            ...
            profile = CustomerSegmentProfile(
                ...
                avg_satisfaction=sum(c.satisfaction_metrics["overall_satisfaction"] for c in segment_customers) / len(segment_customers) if segment_customers else 8.0,
                churn_rate=data["churn_rate"],
                growth_rate=random.uniform(0.05, 0.25),
                preferred_channels=["email", "whatsapp", "portal"] if segment in [CustomerSegment.VIP, CustomerSegment.PREMIUM] else ["whatsapp", "phone"],
                behavior_patterns={
                    "avg_sessions_monthly": random.uniform(10, 30),
                    "avg_support_tickets_monthly": random.uniform(0.5, 2.0),
    ...
    def _generate_prediction_text(self, insight_type: str, customer: Customer360) -> str:
        ...
        predictions = {
            ...
            "upsell_opportunity": f"Oportunidade de upsell identificada para {customer.basic_info['name']} - valor estimado R$ {random.uniform(2000, 8000):,.2f}",
            ...
            "engagement_increase": f"Potencial para aumentar engajamento de {customer.basic_info['name']} em {random.uniform(20, 40):.0f}%",
    ...
    async def get_customer_360(self, customer_id: str) -> Optional[Customer360]:
        """Obter visão 360° de um cliente."""
        return self.customers.get(customer_id)
    
    async def get_all_customers(self, segment: Optional[CustomerSegment] = None, 
                              stage: Optional[CustomerStage] = None) -> List[Customer360]:
    ...
    async def segment_customers(self) -> Dict[CustomerSegment, List[str]]:
        ...
        for customer_id, customer in self.customers.items():
            ...
            segment_score = (monthly_revenue / 1000 * 0.5 + 
                           satisfaction * 0.3 + 
                           engagement * 10 * 0.2)
    ...
    async def get_predictive_insights(self, customer_id: Optional[str] = None) -> List[PredictiveInsight]:
    ...
    async def generate_customer_health_score(self, customer_id: str) -> float:
        ...
        # Peso dos fatores
        health_score = (satisfaction_factor * 0.3 + 
    ...
    async def get_crm_analytics(self) -> CRMAnalytics:
        ...
        active_customers = len([c for c in self.customers.values() 
                               if c.stage not in [CustomerStage.CHURNED]])
        
        # Calcular métricas do mês atual
        current_month = datetime.now().replace(day=1)
        new_customers_month = len([c for c in self.customers.values() 
                                  if c.created_at >= current_month])
        
        churned_customers_month = len([c for c in self.customers.values() 
                                     if c.stage == CustomerStage.CHURNED and 
                                     c.last_updated >= current_month])
        
        # Métricas de satisfação e valor
        avg_satisfaction = sum(c.satisfaction_metrics["overall_satisfaction"] 
                             for c in self.customers.values()) / total_customers if total_customers > 0 else 0
        
        avg_ltv = sum(c.financial_metrics["lifetime_value"] 
                     for c in self.customers.values()) / total_customers if total_customers > 0 else 0
        ...
        return CRMAnalytics(
            ...
            customer_acquisition_cost=random.uniform(500, 1500),
            ...
            response_time_avg_hours=random.uniform(2.5, 8.0),
            conversion_rates={
                "lead_to_prospect": random.uniform(0.25, 0.45),
                "prospect_to_customer": random.uniform(0.15, 0.35),
                "customer_to_loyal": random.uniform(0.20, 0.40)
            }
        )
    ...
    async def search_customers(self, query: str, filters: Dict[str, Any] = None) -> List[Customer360]:
        ...
        for customer in self.customers.values():
            # Busca textual
            if (query_lower in customer.basic_info.get("name", "").lower() or
                query_lower in customer.basic_info.get("location", "").lower() or
                query_lower in customer.id.lower()):
                ...
                if filters.get("stage") and customer.stage != filters["stage"]:
                    continue
                if filters.get("min_revenue") and customer.financial_metrics["monthly_revenue"] < filters["min_revenue"]:
                    continue
