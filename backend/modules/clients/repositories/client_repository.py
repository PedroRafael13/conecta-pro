...
from modules.clients.schemas.client_schemas import (
    ...
    CondominiumCreate, CondominiumUpdate,
    UnitCreate, UnitUpdate,
    ClientContractCreate, ClientContractUpdate,
    IntegrationSettingsCreate, IntegrationSettingsUpdate,
)
...
class ClientRepository:
    ...
    def create_client(self, data: ClientCreate, created_by: Optional[UUID] = None) -> Client:
        """Create a new client."""
        # Generate code
    ...
    def get_client_with_relations(self, client_id: UUID) -> Optional[Client]:
        """Get client with all relations loaded."""
        return (
            self.db.query(Client)
            .options(
                joinedload(Client.condominiums),
                joinedload(Client.contracts),
                joinedload(Client.integration_settings)
            )
    ...
    def get_client_stats(self) -> dict:
        ...
        inactive = self.db.query(func.count(Client.id)).filter(
            Client.status == ClientStatus.INATIVO
        ).scalar() or 0
        defaulters = self.db.query(func.count(Client.id)).filter(
            Client.is_defaulter.is_(True)
        ).scalar() or 0
        vip = self.db.query(func.count(Client.id)).filter(
            Client.is_vip.is_(True)
        ).scalar() or 0

        by_type = dict(
            self.db.query(Client.type, func.count(Client.id))
    ...
    def _get_next_client_sequence(self) -> int:
        ...
        pattern = f"CLI-{year}-%"
        max_code = (
            self.db.query(func.max(Client.code))
            .filter(Client.code.like(pattern))
            .scalar()
        )
        if max_code:
            try:
    ...
    def get_condominium(self, condominium_id: UUID) -> Optional[Condominium]:
        """Get condominium by ID."""
        return self.db.query(Condominium).filter(Condominium.id == condominium_id).first()

    def get_condominium_with_units(self, condominium_id: UUID) -> Optional[Condominium]:
    ...
    def get_condominium_stats(self, client_id: Optional[UUID] = None) -> dict:
        ...
        by_type = dict(
            query.with_entities(Condominium.type, func.count(Condominium.id))
            .group_by(Condominium.type)
            .all()
        )
        by_status = dict(
            query.with_entities(Condominium.status, func.count(Condominium.id))
            .group_by(Condominium.status)
            .all()
        )
    ...
    def _get_next_condominium_sequence(self, client_code: str) -> int:
        ...
        max_code = (
            self.db.query(func.max(Condominium.code))
            .filter(Condominium.code.like(pattern))
            .scalar()
        )
        if max_code:
            try:
    ...
    def create_unit(self, data: UnitCreate, created_by: Optional[UUID] = None) -> Unit:
        """Create a new unit."""
    ...
    def get_unit_stats(self, condominium_id: UUID) -> dict:
        ...
        by_type = dict(
            query.with_entities(Unit.type, func.count(Unit.id))
            .group_by(Unit.type)
            .all()
        )
        by_status = dict(
            query.with_entities(Unit.status, func.count(Unit.id))
    ...
    def get_client_contract(self, contract_id: UUID) -> Optional[ClientContract]:
    ...
    def get_integration_settings(self, settings_id: UUID) -> Optional[IntegrationSettings]:
        """Get integration settings by ID."""
        return (
            self.db.query(IntegrationSettings)
            .filter(IntegrationSettings.id == settings_id)
            .first()
        )

    def list_integration_settings_by_client(
        self,
        client_id: UUID
        ...
        return (
            self.db.query(IntegrationSettings)
            .filter(IntegrationSettings.client_id == client_id)
            .all()
        )
