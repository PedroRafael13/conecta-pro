...
class PortalSelfServiceService:
    ...
    def _initialize_demo_data(self):
        ...
        for i, article_data in enumerate(knowledge_articles, 1):
            article = KnowledgeBaseArticle(
                ...
                helpful_votes=random.randint(10, 50),
        ...
        for customer_id in demo_customers:
            ...
            # Gerar tickets
            self._generate_support_tickets(customer_id, random.randint(2, 5))
            
            # Gerar conversas de chat
            self._generate_chat_conversations(customer_id, random.randint(1, 3))
            
            # Gerar agendamentos
            self._generate_service_bookings(customer_id, random.randint(1, 4))
            
            # Gerar transações
            self._generate_payment_transactions(customer_id, random.randint(5, 15))
    ...
    def _generate_support_tickets(self, customer_id: str, count: int):
        ...
        for i in range(count):
            ...
            ticket = SupportTicket(
                ...
                assigned_agent=f"AGENT_{random.randint(1, 10):02d}" if status != TicketStatus.OPEN else None,
                resolution="Problema resolvido com sucesso" if status in [TicketStatus.RESOLVED, TicketStatus.CLOSED] else None,
                resolution_time_hours=random.uniform(0.5, 24) if status in [TicketStatus.RESOLVED, TicketStatus.CLOSED] else None,
                customer_rating=random.uniform(7, 10) if status == TicketStatus.CLOSED else None
            )
    ...
    def _generate_chat_conversations(self, customer_id: str, count: int):
        ...
        for i in range(count):
            ...
            for j in range(message_count):
                source = random.choice([ChatSource.CUSTOMER, ChatSource.BOT, ChatSource.AGENT])
                ...
                if source == ChatSource.CUSTOMER:
                    ...
                    content = random.choice(self.bot_responses["help_payment"] + 
                ...
                message = ChatMessage(
                    ...
                    agent_id=f"AGENT_{random.randint(1, 5):02d}" if source == ChatSource.AGENT else None
                )
            ...
            conversation = ChatConversation(
                id=conversation_id,
                customer_id=customer_id,
                subject=random.choice(["Suporte técnico", "Dúvida financeira", "Solicitação de serviço"]),
                status=random.choice(["active", "closed"]),
                created_at=created_at,
                last_message_at=created_at + timedelta(minutes=len(messages)*2),
                messages=messages,
                assigned_agent=f"AGENT_{random.randint(1, 5):02d}" if random.random() > 0.3 else None,
                satisfaction_rating=random.uniform(7, 10) if random.random() > 0.5 else None,
                resolution_time_minutes=random.randint(5, 60) if random.random() > 0.4 else None
            )
    ...
    def _generate_service_bookings(self, customer_id: str, count: int):
        ...
        for i in range(count):
            ...
            booking = ServiceBooking(
                ...
                assigned_technician=f"TECH_{random.randint(1, 8):02d}",
    ...
    def _generate_payment_transactions(self, customer_id: str, count: int):
        ...
        for i in range(count):
            ...
            payment_method = random.choice(["credit_card", "bank_transfer", "pix"])
            ...
            transaction = PaymentTransaction(
                ...
                description=f"Pagamento de fatura mensal",
                payment_method=payment_method,
                status=status,
                created_at=created_date,
                processed_at=created_date + timedelta(minutes=random.randint(1, 30)) if status == PaymentStatus.COMPLETED else None,
                transaction_id=f"TXN_{random.randint(100000, 999999)}" if status == PaymentStatus.COMPLETED else None
            )
    ...
    def _generate_portal_usage(self, customer_id: str, days: int):
        ...
        for i in range(days):
            ...
            activity_multiplier = random.uniform(0.3, 1.5)
            
            usage = PortalUsageAnalytics(
                customer_id=customer_id,
                date=date,
                login_count=max(0, int(random.randint(0, 3) * activity_multiplier)),
                pages_visited=max(0, int(random.randint(5, 20) * activity_multiplier)),
                documents_downloaded=max(0, int(random.randint(0, 5) * activity_multiplier)),
                tickets_created=max(0, int(random.randint(0, 1) * activity_multiplier)),
                chat_sessions=max(0, int(random.randint(0, 2) * activity_multiplier)),
                payments_made=max(0, int(random.randint(0, 1) * activity_multiplier)),
                time_spent_minutes=max(0, int(random.randint(10, 60) * activity_multiplier)),
                features_used=random.sample(["dashboard", "documents", "payments", "chat", "tickets", "bookings"], k=random.randint(1, 4))
            )
    ...
    async def get_customer_dashboard(self, customer_id: str) -> PortalDashboard:
        ...
        quick_stats = {
            ...
            "satisfaction_score": random.uniform(8.5, 9.5)
        }
        ...
        return PortalDashboard(
            ...
            satisfaction_metrics={
                "overall_satisfaction": random.uniform(8.0, 9.5),
                "portal_usability": random.uniform(8.5, 9.2),
                "support_quality": random.uniform(8.2, 9.3)
            },
    ...
    async def create_support_ticket(self, customer_id: str, title: str, description: str, 
                                  category: str, priority: TicketPriority) -> SupportTicket:
    ...
    async def start_chat_conversation(self, customer_id: str, subject: str) -> ChatConversation:
        ...
        welcome_message = ChatMessage(
            ...
            message_type=ChatMessageType.TEXT,
            content=random.choice(self.bot_responses["greeting"]),
    ...
    async def send_chat_message(self, conversation_id: str, source: ChatSource, 
                              content: str, message_type: ChatMessageType = ChatMessageType.TEXT) -> ChatMessage:
    ...
    async def _generate_bot_response(self, customer_message: str) -> Optional[str]:
        ...
        if any(word in message_lower for word in ["pagamento", "pagar", "fatura", "boleto", "pix"]):
            return random.choice(self.bot_responses["help_payment"])
        elif any(word in message_lower for word in ["manutenção", "manutencao", "tecnico", "reparo"]):
            return random.choice(self.bot_responses["help_maintenance"])
        elif any(word in message_lower for word in ["documento", "relatório", "relatorio", "baixar"]):
            return random.choice(self.bot_responses["help_documents"])
        else:
            return random.choice(self.bot_responses["fallback"])
    
    async def process_payment(self, customer_id: str, amount: float, payment_method: str, 
        ...
        transaction = PaymentTransaction(
            ...
            transaction_id=f"TXN_{random.randint(100000, 999999)}" if success else None
        )
    ...
    async def book_service(self, customer_id: str, service_type: ServiceType, title: str, 
                         scheduled_date: datetime, description: str = "") -> ServiceBooking:
    ...
    async def download_document(self, customer_id: str, document_id: str) -> Optional[CustomerDocument]:
    ...
    async def get_portal_analytics(self) -> PortalAnalytics:
        ...
        monthly_active = len([
            customer_id for customer_id, usage_list in self.portal_usage.items()
        ...
        avg_resolution_hours = sum(t.resolution_time_hours for t in resolved_tickets) / len(resolved_tickets) if resolved_tickets else 0
        
        # Downloads de documentos
        total_downloads = sum(
            sum(doc.download_count for doc in doc_list)
            for doc_list in self.customer_documents.values()
        )
        ...
        self_service_tickets = len([t for t in all_tickets if t.assigned_agent is None and t.status == TicketStatus.RESOLVED])
        ...
        kb_effectiveness = sum(article.helpful_votes / max(1, article.helpful_votes + article.unhelpful_votes) 
                             for article in self.knowledge_base.values()) / len(self.knowledge_base)
    ...
    async def get_customer_documents(self, customer_id: str, document_type: DocumentType = None) -> List[CustomerDocument]:
