...
class DocumentService:
    ...
    async def create(self, data: DocumentCreate) -> DocumentResponse:
        ...
        if not folder.is_extension_allowed(data.file_extension):
            raise ValueError(f"Extensão .{data.file_extension} não permitida nesta pasta")

        # Verifica tamanho
        if not folder.is_size_allowed(data.file_size_bytes):
            raise ValueError(f"Arquivo excede o limite de {folder.max_file_size_mb}MB")
        ...
        await self.folder_repository.update_document_stats(data.folder_id, data.file_size_bytes, 1)

        await self.session.commit()
        logger.info("Documento criado: %s - %s", document.id, document.title)
        return DocumentResponse.model_validate(document)

    async def get_by_id(self, document_id: str) -> DocumentResponse | None:
    ...
    async def get_by_code(self, code: str) -> DocumentResponse | None:
    ...
    async def update(self, document_id: str, data: DocumentUpdate) -> DocumentResponse | None:
    ...
    async def delete(self, document_id: str) -> bool:
        ...
        # Atualiza estatísticas da pasta
        await self.folder_repository.update_document_stats(document.folder_id, -document.file_size_bytes, -1)
    ...
    async def get_pending_approval(
        ...
        skip = (page - 1) * page_size
        documents = await self.repository.get_pending_approval(condominium_id, skip, page_size)
        return [DocumentResponse.model_validate(d) for d in documents]
    ...
    async def get_pending_signature(
        ...
        skip = (page - 1) * page_size
        documents = await self.repository.get_pending_signature(condominium_id, skip, page_size)
        return [DocumentResponse.model_validate(d) for d in documents]
    ...
    async def get_expiring_soon(self, days: int = 30, condominium_id: str = None) -> builtins.list[DocumentResponse]:
        """Retorna documentos prestes a expirar."""
        documents = await self.repository.get_expiring_soon(days, condominium_id)
        return [DocumentResponse.model_validate(d) for d in documents]

    async def approve(self, document_id: str, approved_by: str) -> DocumentResponse | None:
        """Aprova documento."""
    ...
    async def reject(self, document_id: str, reason: str) -> DocumentResponse | None:
        """Rejeita documento."""
    ...
    async def publish(self, document_id: str) -> DocumentResponse | None:
        """Publica documento."""
    ...
    async def archive(self, document_id: str, archived_by: str) -> DocumentResponse | None:
        """Arquiva documento."""
    ...
    async def unarchive(self, document_id: str) -> DocumentResponse | None:
    ...
    async def move(self, document_id: str, folder_id: str) -> DocumentResponse | None:
        ...
        await self.folder_repository.update_document_stats(document.folder_id, -document.file_size_bytes, -1)
        await self.folder_repository.update_document_stats(folder_id, document.file_size_bytes, 1)
    ...
    async def view(self, document_id: str) -> DocumentResponse | None:
    ...
    async def download(self, document_id: str) -> DocumentResponse | None:
    ...
    async def search(self, query: str, condominium_id: str = None, limit: int = 20) -> builtins.list[DocumentResponse]:
    ...
    async def submit_for_approval(self, document_id: str) -> DocumentResponse | None:
    ...
    async def create_new_version(
        ...
        await self.session.commit()
        logger.info("Nova versão criada: %s v%s", document_id, version_number)
        return DocumentResponse.model_validate(document)
