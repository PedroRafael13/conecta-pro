...
from modules.integrations.connectors.base.connector import BaseConnector, SyncResult
...
class SyncJob(ABC, Generic[T]):
    ...
    @abstractmethod
    async def get_existing_entity(self, internal_id: UUID) -> Optional[T]:
        """
    ...
    def get_external_updated_at(self, data: Dict[str, Any]) -> Optional[datetime]:
        """
    ...
    async def get_id_mapping(self, external_id: str) -> Optional[IDMap]:
        ...
        try:
            ...
            if existing_mapping:
                ...
                if existing_entity:
                    ...
                    logger.debug(
                        f"[{self.correlation_id}] Atualizado: {external_id} -> {internal_id}"
                    )
                else:
                    ...
                    logger.debug(
                        f"[{self.correlation_id}] Recriado: {external_id} -> {internal_id}"
                    )
            else:
                # Nova entidade
                new_entity = await self.map_to_internal(data)
                ...
                logger.debug(
                    f"[{self.correlation_id}] Criado: {external_id} -> {internal_id}"
                )
            ...
            logger.error(
                f"[{self.correlation_id}] Erro ao processar {external_id}: {e}"
            )
            return False
    ...
    async def run(
        ...
        logger.info(
            f"[{self.correlation_id}] Iniciando sync de {self.ENTITY_TYPE}"
            f" (full={full_sync})"
        )
        ...
        try:
            ...
            async for page_result in self.connector.fetch_all_entities(
                ...
                if not page_result.success:
                    logger.warning(
                        f"[{self.correlation_id}] Página retornou erro: "
                        f"{page_result.errors}"
                    )
            ...
            logger.error(
                f"[{self.correlation_id}] Erro no sync de {self.ENTITY_TYPE}: {e}"
            )
