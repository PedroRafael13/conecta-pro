...
from modules.integrations.connectors.base.rate_limiter import AdaptiveRateLimiter, RateLimiter
...
class IntegrationHTTPClient:
    ...
    def __init__(
        ...
        self.config = config
        self.auth = auth
        self.rate_limiter = rate_limiter or RateLimiter(requests_per_second=10, name=connector_name)
    ...
    async def _ensure_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            ...
            self._client = httpx.AsyncClient(
                base_url=self.config.base_url, timeout=timeout, headers=self.config.headers, follow_redirects=True
            )
    ...
    async def request(
        ...
        metrics = RequestMetrics(request_id=request_id, method=method, url=f"{self.config.base_url}{path}")
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if not skip_rate_limit:
                    ...
                    if wait_time > 0:
                        ...
                        logger.debug(f"[{self.connector_name}] Rate limit wait: {wait_time:.2f}s")
                ...
                response = await client.request(
                    method=method, url=path, params=params, json=json, data=data, headers=headers, timeout=timeout
                )
                ...
                if response.status_code == 429:
                    ...
                    raise RateLimitError(connector=self.connector_name, retry_after=retry_after)
                ...
                if response.status_code == 401:
                    # Não autorizado
                    raise AuthenticationError("Requisição não autorizada", connector=self.connector_name)

                if response.status_code == 404:
                    # Not found - não fazer retry
                    raise APIError("Recurso não encontrado", connector=self.connector_name, status_code=404)
                ...
                last_error = ConnectorConnectionError(
                    f"Erro de conexão: {str(e)}", connector=self.connector_name, original_error=e
                )
                await self.circuit_breaker.record_failure()

            except httpx.ReadTimeout:
                last_error = ConnectorTimeoutError(
                    "Timeout de leitura", connector=self.connector_name, timeout_seconds=int(self.config.timeout_read)
                )
                await self.circuit_breaker.record_failure()
            ...
            if attempt < self.config.max_retries:
                ...
                logger.warning(f"[{self.connector_name}] Tentativa {attempt + 1} falhou. Retry em {delay:.1f}s")
        ...
        raise last_error or ConnectorError("Falha após máximo de tentativas", connector=self.connector_name)

    # Métodos de conveniência
    async def get(self, path: str, params: dict[str, Any] | None = None, **kwargs) -> httpx.Response:
        """GET request."""
        return await self.request("GET", path, params=params, **kwargs)

    async def post(
        self, path: str, json: dict[str, Any] | None = None, data: dict[str, Any] | None = None, **kwargs
    ) -> httpx.Response:
        """POST request."""
    ...
    async def patch(self, path: str, json: dict[str, Any] | None = None, **kwargs) -> httpx.Response:
        """PATCH request."""
