...
@router.get(
    "/dashboard",
    response_model=IntegrationDashboard,
...
async def list_endpoints(
    endpoint_status: Optional[EndpointStatus] = Query(None, alias="status"),
...
@router.post(
    "/endpoints/{endpoint_id}/deprecate",
    response_model=APIEndpointResponse,
...
async def list_api_keys(
    ...
    key_status: Optional[APIKeyStatus] = Query(None, alias="status"),
...
async def revoke_api_key(
    key_id: UUID,
    data: Optional[APIKeyRevokeRequest] = None,
    service: IntegrationService = Depends(get_integration_service),
    current_user: dict = Depends(get_current_user)
) -> APIKeyResponse:
...
async def verify_api_key(
    ...
    valid, key, message = await service.verify_api_key(
        key=api_key,
...
@router.get(
    "/webhooks",
    response_model=WebhookConfigList,
    summary="Lista webhooks"
)
async def list_webhooks(
    client_id: Optional[UUID] = None,
    webhook_status: Optional[WebhookStatus] = Query(None, alias="status"),
    page: int = Query(1, ge=1),
    page_size: int = Query(50, ge=1, le=100),
    service: WebhookService = Depends(get_webhook_service),
    current_user: dict = Depends(get_current_user)
...
async def regenerate_webhook_secret(
    ...
    try:
        new_secret = await service.regenerate_secret(
            webhook_id=webhook_id,
            user_id=current_user.get("id")
        )
...
@router.get(
    "/logs",
    response_model=IntegrationLogList,
