...
class TestIntegrationService:
    ...
    async def test_create_endpoint(self, service):
        ...
        with patch.object(service.repository, "get_endpoint_by_path_method", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = None

            with patch.object(service.repository, "create_endpoint", new_callable=AsyncMock) as mock_create:
                endpoint = APIEndpoint(id=uuid4(), name="Test", path="/test", method=HTTPMethod.GET)
                mock_create.return_value = endpoint

                data = APIEndpointCreate(name="Test", path="/test", method=HTTPMethod.GET)
    ...
    async def test_create_endpoint_duplicate(self, service):
        ...
        with patch.object(service.repository, "get_endpoint_by_path_method", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = APIEndpoint(id=uuid4(), name="Existing", path="/test", method=HTTPMethod.GET)
    ...
    async def test_create_api_key(self, service):
        ...
        with patch.object(service.repository, "create_api_key", new_callable=AsyncMock) as mock_create:
            api_key = APIKey(id=uuid4(), name="Test Key", key_prefix="abc12345", key_hash="hash", key_hint="6789")
    ...
    async def test_verify_api_key_success(self, service):
        ...
        with patch.object(service.repository, "get_api_key_by_hash", new_callable=AsyncMock) as mock_get:
            ...
            with patch.object(service.repository, "update_api_key", new_callable=AsyncMock):
                valid, key, msg = await service.verify_api_key("test_key", client_ip="127.0.0.1")
    ...
    async def test_queue_sync(self, service):
        ...
        with patch.object(service.repository, "create_sync_item", new_callable=AsyncMock) as mock_create:
            item = SyncQueue(id=uuid4(), entity_type=SyncEntityType.CLIENT, external_system=ExternalSystem.OMIE)
            mock_create.return_value = item

            data = SyncQueueCreate(
                entity_type=SyncEntityType.CLIENT, external_system=ExternalSystem.OMIE, payload={"test": True}
            )
    ...
    async def test_complete_sync_success(self, service):
        ...
        with patch.object(service, "get_sync_item", new_callable=AsyncMock) as mock_get:
            ...
            with patch.object(service.repository, "update_sync_item", new_callable=AsyncMock) as mock_update:
...
class TestWebhookService:
    ...
    async def test_create_webhook(self, service):
        ...
        with patch.object(service.repository, "create_webhook", new_callable=AsyncMock) as mock_create:
            ...
            with patch.object(service.repository, "create_log", new_callable=AsyncMock):
                data = WebhookConfigCreate(
                    name="Test Webhook", url="https://example.com/webhook", events=["client.created"]
                )
    ...
    async def test_regenerate_secret(self, service):
        ...
        webhook = WebhookConfig(
            id=uuid4(), name="Test", url="https://example.com", events=["test"], secret_key="old_secret"
        )

        with patch.object(service, "get_webhook", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = webhook

            with (
                patch.object(service.repository, "update_webhook", new_callable=AsyncMock),
                patch.object(service.repository, "create_log", new_callable=AsyncMock),
            ):
    ...
    async def test_trigger_event(self, service):
        ...
        with patch.object(service.repository, "get_webhooks_for_event", new_callable=AsyncMock) as mock_webhooks:
            mock_webhooks.return_value = [webhook]

            with patch.object(service, "_deliver_webhook", new_callable=AsyncMock) as mock_deliver:
                mock_deliver.return_value = {"webhook_id": str(webhook.id), "success": True, "status_code": 200}

                results = await service.trigger_event(event="client.created", payload={"id": "123"})
    ...
    async def test_trigger_event_no_webhooks(self, service):
        ...
        with patch.object(service.repository, "get_webhooks_for_event", new_callable=AsyncMock) as mock_webhooks:
            ...
            results = await service.trigger_event(event="unknown.event", payload={})
    ...
    async def test_check_failing_webhooks(self, service):
        ...
        failing = [
            WebhookConfig(
                id=uuid4(), name=f"Failing {i}", url="https://example.com", events=["test"], consecutive_failures=10
            )
            for i in range(3)
        ]

        with patch.object(service.repository, "get_failing_webhooks", new_callable=AsyncMock) as mock_failing:
...
class TestIntegrationServiceDashboard:
    ...
    async def test_get_dashboard(self, service):
        ...
        keys = [APIKey(id=uuid4(), name="Key", key_prefix="test", key_hash="hash", status=APIKeyStatus.ACTIVE)]
        ...
        with patch.object(service.repository, "list_endpoints", new_callable=AsyncMock) as mock_endpoints:
            mock_endpoints.return_value = (endpoints, 1)

            with patch.object(service.repository, "list_api_keys", new_callable=AsyncMock) as mock_keys:
                mock_keys.return_value = (keys, 1)

                with patch.object(service.repository, "list_webhooks", new_callable=AsyncMock) as mock_webhooks:
                    mock_webhooks.return_value = (webhooks, 1)

                    with patch.object(service, "get_sync_stats", new_callable=AsyncMock) as mock_stats:
                        ...
                        with patch.object(
                            service.repository, "get_recent_errors", new_callable=AsyncMock
                        ) as mock_errors:
