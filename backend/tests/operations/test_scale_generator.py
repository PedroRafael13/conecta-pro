...
def _valid_month_year() -> tuple[int, int]:
    """Retorna mês/ano válido para geração de escala (mês seguinte)."""
    today = date.today()
    # Usar mês seguinte para garantir que todas as datas estejam dentro da janela válida
    if today.month == 12:
...
class TestScaleGenerator:
    ...
    def test_holiday_detection(self, generator, sample_employees, base_config):
        ...
        # Verificar se há turno no dia 1 do mês
        day1_shifts = [s for s in shifts if s.shift_date.day == 1]
        assert len(day1_shifts) > 0
