"""
Testes de integração — Financeiro + Operacional + CRM + Licitações + Segurança
...
def test_fin_contratos_tem_dados():
...
def test_op_posts_retorna_paginado():
...
def test_crm_tem_clientes_reais():
    ...
    assert r.status_code == 200
    body = r.json()
    total = body.get("total", len(body.get("items", body if isinstance(body, list) else [])))
