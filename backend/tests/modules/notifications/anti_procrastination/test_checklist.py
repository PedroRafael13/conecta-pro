...
from modules.notifications.anti_procrastination.daily_checklist.checklist_manager import (
    ChecklistManager, ChecklistStatus, TaskAction, ChecklistItem
)
...
class TestChecklistManager:
    ...
    async def test_generate_daily_checklist(self, checklist_manager):
        ...
        with pytest.patch.object(checklist_manager, "db") as mock_db:
            ...
            result_id = await checklist_manager.generate_daily_checklist(
                user_id, "João Silva", "hr"
            )
    ...
    def test_should_block_access_with_many_old_tasks(self, checklist_manager):
        ...
        # Assert
        assert should_block == True  # Deve bloquear por ter muitas tarefas antigas
