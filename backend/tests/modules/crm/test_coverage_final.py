...
Gaps alvo (após pragma: no cover nos TYPE_CHECKING e pipeline "critical"):
  ...
  - services/crm_360_service.py:770-772, 780-782, 815, 817, 839-843, 849, 922, 930
...
# ===========================================================================
# 5. services/crm_360_service.py — branches restantes
# ===========================================================================


class TestCRM360ServiceRemainingBranches:
    @pytest.fixture
    def service(self):
    ...
    @pytest.mark.asyncio
    async def test_track_interaction_with_satisfaction_score(self, service):
        """Lines 770-772: atualizar satisfaction_metrics quando satisfaction_score fornecido."""
        ...
        before = service.customers[cid].satisfaction_metrics["overall_satisfaction"]
        result = await service.track_customer_interaction(interaction)
        assert result is True
        # satisfaction_metrics deve ter sido atualizado
        after = service.customers[cid].satisfaction_metrics["overall_satisfaction"]
        assert after != before or after >= 0  # foi recalculado

    @pytest.mark.asyncio
    async def test_track_interaction_exception_returns_false(self, service):
        """Lines 780-782: exception handler retorna False."""
        from modules.crm.services.crm_360_service import (
            CustomerInteraction,
            InteractionDirection,
            InteractionType,
        ...
        # Corromper o journey para forçar AttributeError
        service.customers[cid].journey = None
        ...
        interaction = CustomerInteraction(
            ...
            channel="phone",
            touchpoint_category=TouchpointCategory.SUPPORT,
            subject="Suporte",
            description="Chamada",
            sentiment_score=0.5,
        )
    ...
    @pytest.mark.asyncio
    async def test_segment_customers_vip_branch(self, service):
        """Line 815: new_segment = CustomerSegment.VIP quando segment_score >= 15."""
        from modules.crm.services.crm_360_service import CustomerSegment

        customers = await service.get_all_customers()
        cid = customers[0].id
        # monthly_revenue/1000 * 0.5: com 50000 → 25; sat 9.5*0.3=2.85; eng 0.95*10*0.2=1.9 → 29.75 ≥ 15
        service.customers[cid].financial_metrics["monthly_revenue"] = 50000
        service.customers[cid].satisfaction_metrics["overall_satisfaction"] = 9.5
        service.customers[cid].engagement_metrics["feature_adoption_rate"] = 0.95

        segments = await service.segment_customers()
        assert CustomerSegment.VIP in segments
        assert cid in segments[CustomerSegment.VIP]

    @pytest.mark.asyncio
    async def test_segment_customers_premium_branch(self, service):
        """Line 817: new_segment = CustomerSegment.PREMIUM quando 10 <= score < 15."""
        from modules.crm.services.crm_360_service import CustomerSegment

        customers = await service.get_all_customers()
        cid = customers[1].id
        # revenue=18000 → 18*0.5=9; sat=8*0.3=2.4; eng=0.5*10*0.2=1 → 12.4 ≥ 10 e < 15
        service.customers[cid].financial_metrics["monthly_revenue"] = 18000
        service.customers[cid].satisfaction_metrics["overall_satisfaction"] = 8.0
        service.customers[cid].engagement_metrics["feature_adoption_rate"] = 0.5

        segments = await service.segment_customers()
        assert CustomerSegment.PREMIUM in segments

    @pytest.mark.asyncio
    async def test_get_predictive_insights_all_customers(self, service):
        """Lines 839-843: retornar todos os insights quando customer_id é None."""
        all_insights = await service.get_predictive_insights(customer_id=None)
        assert isinstance(all_insights, list)
        customers = await service.get_all_customers()
        single = await service.get_predictive_insights(customer_id=customers[0].id)
        assert len(all_insights) >= len(single)

    @pytest.mark.asyncio
    async def test_generate_health_score_not_found_returns_zero(self, service):
        """Line 849: retorna 0.0 quando customer não encontrado."""
        score = await service.generate_customer_health_score("nonexistent-xyz-999")
        assert score == 0.0

    @pytest.mark.asyncio
    async def test_get_recommendations_not_found_returns_empty(self, service):
        """Lines 921-922: retorna [] quando customer não encontrado."""
        recs = await service.get_customer_recommendations("nonexistent-xyz-999")
        assert recs == []

    @pytest.mark.asyncio
    async def test_get_recommendations_low_health_triggers_improvement(self, service):
        """Line 930: recomendação health_improvement para clientes com score < 70."""
        customers = await service.get_all_customers()
        cid = customers[0].id
        # Degradar todas as métricas
        service.customers[cid].satisfaction_metrics["overall_satisfaction"] = 2.0
        service.customers[cid].engagement_metrics["feature_adoption_rate"] = 0.05
        service.customers[cid].financial_metrics["payment_history_score"] = 0.2

        recs = await service.get_customer_recommendations(cid)
        assert any(r.get("type") == "health_improvement" for r in recs)
