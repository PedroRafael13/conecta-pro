...
- crm_360_service.py (get_all_customers filters, search_customers filters)
...
# ============================================================
# GAP 7: crm_360_service.py — filters
# ============================================================


@pytest.mark.asyncio
async def test_get_all_customers_segment_filter():
    """Cover line 746-747: get_all_customers with segment filter."""
    from modules.crm.services.crm_360_service import CRM360Service, CustomerSegment

    svc = CRM360Service()
    # Get VIP customers
    vips = await svc.get_all_customers(segment=CustomerSegment.VIP)
    for c in vips:
        assert c.segment == CustomerSegment.VIP


@pytest.mark.asyncio
async def test_get_all_customers_stage_filter():
    """Cover line 749-750: get_all_customers with stage filter."""
    from modules.crm.services.crm_360_service import CRM360Service, CustomerStage

    svc = CRM360Service()
    loyal = await svc.get_all_customers(stage=CustomerStage.LOYAL)
    for c in loyal:
        assert c.stage == CustomerStage.LOYAL


@pytest.mark.asyncio
async def test_get_all_customers_both_filters():
    """Cover both segment and stage filters combined."""
    from modules.crm.services.crm_360_service import CRM360Service, CustomerSegment, CustomerStage

    svc = CRM360Service()
    result = await svc.get_all_customers(segment=CustomerSegment.VIP, stage=CustomerStage.LOYAL)
    for c in result:
        assert c.segment == CustomerSegment.VIP
        assert c.stage == CustomerStage.LOYAL
...
@pytest.mark.asyncio
async def test_search_customers_with_segment_filter():
    """Cover lines 980-981: search_customers with segment filter."""
    from modules.crm.services.crm_360_service import CRM360Service, CustomerSegment

    svc = CRM360Service()
    # Search with a broad query and restrict by segment
    results = await svc.search_customers(
        query="Condomínio",
        filters={"segment": CustomerSegment.VIP},
    )
    for c in results:
        assert c.segment == CustomerSegment.VIP
...
@pytest.mark.asyncio
async def test_search_customers_with_stage_filter():
    """Cover lines 982-983: search_customers with stage filter."""
    from modules.crm.services.crm_360_service import CRM360Service, CustomerStage

    svc = CRM360Service()
    results = await svc.search_customers(
        query="Condomínio",
        filters={"stage": CustomerStage.ADVOCATE},
    )
    for c in results:
        assert c.stage == CustomerStage.ADVOCATE


@pytest.mark.asyncio
async def test_search_customers_with_min_revenue_filter():
    """Cover lines 984-988: search_customers with min_revenue filter."""
    from modules.crm.services.crm_360_service import CRM360Service

    svc = CRM360Service()
    results = await svc.search_customers(
        query="Condomínio",
        filters={"min_revenue": 999999},  # Very high, should filter most out
    )
    for c in results:
        assert c.financial_metrics["monthly_revenue"] >= 999999


@pytest.mark.asyncio
async def test_search_customers_no_filters():
    """Cover search_customers without filters."""
    from modules.crm.services.crm_360_service import CRM360Service

    svc = CRM360Service()
    results = await svc.search_customers(query="Condomínio")
    assert isinstance(results, list)


@pytest.mark.asyncio
async def test_search_customers_no_match():
    """Cover search_customers with no matching query."""
    from modules.crm.services.crm_360_service import CRM360Service

    svc = CRM360Service()
    results = await svc.search_customers(query="XYZNONEXISTENT")
    assert results == []
