...
class ContractTerms(BaseModel):
    ...
    penalty_rate_percent: Decimal = Field(default=Decimal("0.5"), ge=Decimal("0"), le=Decimal("5"))
...
class SupplierQualification(BaseModel):
    ...
    def overall_score(self) -> float:
        ...
        weights = {"technical": 0.35, "commercial": 0.25, "compliance": 0.25, "financial": 0.15}
        return (
    ...
    def is_qualified(self) -> bool:
        ...
        min_scores = {"technical": 60, "commercial": 50, "compliance": 70, "overall": 65}
        return (
            self.technical_score >= min_scores["technical"]
            and self.commercial_score >= min_scores["commercial"]
            and self.compliance_score >= min_scores["compliance"]
            and self.overall_score >= min_scores["overall"]
            and self.risk_rating in [SupplierRisk.LOW, SupplierRisk.MEDIUM]
        )
...
class ProcurementEntity(BaseModel):
    ...
    bidding_modality: BiddingModality | None = None
    ...
    approved_value: Money | None = None
    ...
    def validate_procurement(self) -> "ProcurementEntity":
        ...
        if duration_days > max_days:
            raise ValueError(f"Contratacao {self.urgency.value} deve ser concluida em ate {max_days} dias")
        ...
        if self.urgency in [ProcurementUrgency.EMERGENCY, ProcurementUrgency.CRITICAL]:
            ...
            if self.estimated_value > max_value:
                raise ValueError(f"Contratacoes {self.urgency.value} nao podem exceder {max_value}")

        # Valida justificativa para urgencias
        if self.urgency.requires_justification() and not self.justification:
            raise ValueError(f"Contratacao {self.urgency.value} requer justificativa")
    ...
    def advance_workflow(self, user_id: str, notes: str | None = None) -> bool:
        ...
        if current_stage < 11:
            ...
            self._add_audit_entry(
                ...
                user_id=user_id,
                details={"from_stage": current_stage, "to_stage": self.workflow_stage, "notes": notes},
            )
            return True

        return False

    def approve_budget(self, approved_value: Money, approver_id: str, notes: str | None = None) -> bool:
        ...
        self._add_audit_entry(
            ...
            details={
                ...
                "variance_percent": float((approved_value.amount / self.estimated_value.amount - 1) * 100),
                "notes": notes,
            },
        )
    ...
    def select_supplier(self, supplier_id: UUID, user_id: str, justification: str) -> bool:
        """Seleciona fornecedor para a contratacao."""
        # Busca qualificacao do fornecedor
        supplier_qual = next((sq for sq in self.supplier_qualifications if sq.supplier_id == supplier_id), None)
        ...
        if self.estimated_value.amount > max_value:
            raise ValueError(f"Valor da contratacao excede limite para risco {supplier_qual.risk_rating}")
    ...
    def add_supplier_qualification(self, qualification: SupplierQualification, user_id: str) -> None:
        """Adiciona qualificacao de fornecedor."""
        # Remove qualificacao anterior do mesmo fornecedor
        self.supplier_qualifications = [
            sq for sq in self.supplier_qualifications if sq.supplier_id != qualification.supplier_id
        ]
    ...
    def cancel(self, user_id: str, reason: str) -> bool:
        ...
        self._add_audit_entry(
            action="cancelled", user_id=user_id, details={"previous_status": previous_status, "reason": reason}
        )
    ...
    def suspend(self, user_id: str, reason: str) -> bool:
        ...
        self._add_audit_entry(
            action="suspended", user_id=user_id, details={"previous_status": previous_status, "reason": reason}
        )
    ...
    def calculate_risk(self) -> Literal["low", "medium", "high", "critical"]:
        ...
        if self.selected_supplier_id:
            supplier_qual = next(
                (sq for sq in self.supplier_qualifications if sq.supplier_id == self.selected_supplier_id), None
            )
            if supplier_qual:
                supplier_risk = {
                    SupplierRisk.LOW: 0,
    ...
    def _add_audit_entry(self, action: str, user_id: str, details: dict[str, Any]) -> None:
