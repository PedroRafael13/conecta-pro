...
with open('/opt/conecta-pro/backend/modules/scheduler/services/intelligent_scheduler_service.py', 'r') as f:
    lines = f.readlines()
...
for i, line in enumerate(lines):
    if 'forbidden_start, forbidden_end = conditions["forbidden_hours"]' in line:
        ...
        lines.insert(i+2, '                    forbidden_start = forbidden_period[0][0] + forbidden_period[0][1] / 60\n')
        lines.insert(i+3, '                    forbidden_end = forbidden_period[1][0] + forbidden_period[1][1] / 60\n')
        break
