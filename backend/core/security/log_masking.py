    ...
    def mask_dict(cls, data: dict[str, Any], sensitive_keys: set = None) -> dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if any(sensitive in key_lower for sensitive in sensitive_keys):
                ...
                result[key] = [
                    cls.mask_dict(item, sensitive_keys) if isinstance(item, dict) else item for item in value
                ]
            else:
                result[key] = value
...
class SecureLoggingMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            # Sanitiza query string para log seguro
            query_string = scope.get("query_string", b"").decode()
            if query_string:
                scope["_sanitized_query"] = LogMasker.mask_sensitive_string(query_string)
...
if __name__ == "__main__":
    ...
    # CPF
    cpf_tests = ["123.456.789-00", "12345678900", "111.222.333-44"]
    print("\nCPF:")
    for cpf in cpf_tests:
        print(f"  {cpf} → {LogMasker.mask_cpf(cpf)}")

    # Email
    email_tests = ["usuario@example.com", "ab@example.com", "nome.sobrenome@empresa.com.br"]
    print("\nEmail:")
    for email in email_tests:
        print(f"  {email} → {LogMasker.mask_email(email)}")

    # Telefone
    phone_tests = ["(11) 98765-4321", "11987654321", "(21) 3456-7890"]
    print("\nTelefone:")
