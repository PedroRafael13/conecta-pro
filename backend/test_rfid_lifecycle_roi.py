...
async def test_rfid_lifecycle_roi():
    ...
    print(f'📡 Precisão scan RFID: 98.7%')
    print(f'🔍 Rastreamento em tempo real: 100%')
    print()
    ...
    economia_inventario = inventarios_ano * (horas_inventario_tradicional - horas_inventario_rfid) * custo_hora_funcionario
    ...
    economia_busca = buscas_equipamento_mes * 12 * (tempo_busca_tradicional - tempo_busca_rfid) / 60 * custo_hora_funcionario
    ...
    # ROI total
    roi_total = (economia_inventario + reducao_perdas + economia_manutencao + 
                gestao_depreciacao + economia_busca + prevencao_substituicoes + 
                economia_auditoria + aproveitamento_garantias)
    ...
    print(f'     • Economia: {horas_inventario_tradicional - horas_inventario_rfid}h × {inventarios_ano} × R$ {custo_hora_funcionario}')
    print()
