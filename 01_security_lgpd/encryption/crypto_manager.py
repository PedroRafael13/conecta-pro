...
Description: Gerenciador central de criptografia com suporte a AES-256, RSA e Fernet
...
class CryptoManager(CryptoManagerInterface):
    """
    ...
    Example:
        ...
        >>> result = manager.encrypt(b"dados sensíveis", key)
        >>> plaintext = manager.decrypt(result, key)
    """
    ...
    def __init__(self, config: Optional[CryptoConfig] = None):
        ...
        logger.info("CryptoManager inicializado com algoritmo padrão: %s",
    ...
    def generate_key(self, algorithm: Optional[EncryptionAlgorithm] = None) -> bytes:
        ...
        Raises:
            KeyError: Se algoritmo não suportado.
        """
        ...
        try:
            if algo in [EncryptionAlgorithm.AES_256_GCM, EncryptionAlgorithm.AES_256_CBC]:
                ...
                # Para RSA, retornamos a chave privada serializada
                ...
                raise KeyError(f"Algoritmo não suportado: {algo}", algo.value)
            ...
            raise KeyError(f"Falha ao gerar chave: {str(e)}", algo.value if algo else None)
    ...
    def decrypt(
        ...
        """
        # Converte dicionário para EncryptionResult se necessário
    ...
    def _encrypt_rsa(self, plaintext: bytes, key: bytes) -> EncryptionResult:
        ...
        # Carrega chave privada para extrair pública
        private_key = serialization.load_pem_private_key(
            key, password=None, backend=self._backend
        )
        public_key = private_key.public_key()

        # RSA tem limite de tamanho baseado no key size
        max_size = (self.config.rsa_key_size // 8) - 66  # OAEP overhead
...
_default_manager: Optional[CryptoManager] = None


def get_crypto_manager(config: Optional[CryptoConfig] = None) -> CryptoManager:
    """
    ...
    Returns:
        CryptoManager: Instância do gerenciador.
    """
    global _default_manager
    if _default_manager is None:
        _default_manager = CryptoManager(config)
